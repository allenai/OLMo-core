...
from olmo_core.version import VERSION, VERSION_SHORT

# -- Project information -----------------------------------------------------

project = "OLMo-core"
copyright = f"{datetime.today().year}, Allen Institute for Artificial Intelligence"  # noqa: DTZ002
...
class ShutupSphinxAutodocTypehintsFilter(logging.Filter):
    def filter(self, record: logging.LogRecord) -> bool:
        if "Cannot resolve forward reference" in record.msg:
            return False
        return "Failed guarded type import" not in record.msg
