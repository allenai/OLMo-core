...
set -uo pipefail
S3=s3://ai2-llm/checkpoints/prasanns/flop_scaling
...
CMD+="\$AWS s3 sync $S3/bases/ $WEKA/bases/ --only-show-errors; echo base_files=\$(ls $WEKA/bases/q4b-dense-cpt-fixmark/model_and_optim 2>/dev/null | wc -l); "
...
exec gantry run --name "fs-sync-s3-weka-$(date +%m%d%H%M)" -w ai2/flex2 -b ai2/oe-other \
