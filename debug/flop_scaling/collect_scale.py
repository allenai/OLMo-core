...
OUT = c.OUT
rows = []
for scale, tag in [("0.8b", "s08b"), ("2b", "s2b"), ("9b", "s9b"), ("27b", "s27b")]:
...
if os.path.exists(f"{OUT}/results35.csv"):
    for r in csv.DictReader(open(f"{OUT}/results35.csv")):
        if r["task"] in ("oolong", "contradiction") and r["arm"] in ("dense", "kv17", "kv33", "ffnmoe-t10") and r["budget"] in ("20M", "80M", "14M", "56M"):
...
order = {"0.8b": 0, "2b": 1, "4b": 2, "9b": 3, "27b": 4}; aorder = {"dense": 0, "kv17": 1, "kv33": 1, "ffnmoe-t10": 2}
