...
for li in ("1", "2"):
    ...
    m.kvr_routers[li].w.weight.data.normal_(0, 0.5); m.kvr_routers[li].w.bias.data.fill_(0.5)
...
print(f"[4] joint cost {jb['last_cost']:.3f} target {jb['last_target']:.3f}; grads: skip {m2.bskip_routers['0'].w.bias.grad.item():.2e} kv {m2.kvr_routers['0'].w.bias.grad.item():.2e} ffn {m2.blocks['2'].feed_forward._nffn_router.w.bias.grad.abs().sum().item():.2e}")
print("SMOKE OK")
