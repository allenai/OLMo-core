...
JOBS="
lmx-full-mixs64M-s2-4b|full|-
lmx-full-mixs160M-s2-4b|full|-
lmx-full-p32k4000-4b|full|-
lmx-slm-mixs64M-s2-4b|slm|16k,32k
lmx-slm-mixs160M-s2-4b|slm|16k,32k
lmx-full-q32k16000-qd-4b|slm|32k,64k
lmx-full-qmixs64M-qd-4b|slm|2k,8k,16k,32k
lmx-full-qmixs160M-qd-4b|slm|2k,8k,16k,32k
lmx-full-qmixs320M-qd-4b|slm|2k,8k,16k,32k
lmx-slm-qmixs320M-qd-4b|slm|2k,8k,16k,32k
lmx-full-nmixs16M-nq-4b|slm|2k,8k,16k,32k
lmx-full-nmixs32M-nq-4b|slm|2k,8k,16k,32k
lmx-full-nmixs48M-nq-4b|slm|2k,8k,16k,32k
lmx-slm-nmixs48M-nq-4b|slm|2k,8k,16k,32k
lmx-full-nqD32k4000-4b|slm|32k,64k
lmx-full-nqD64k2000-4b|slm|32k,64k
"
...
case "$RUN" in *qmix*|*q32k*) export EVAL500_ROOT=/data/prasann/qdmatch_lengthmix/eval_rungs; LT=qdmatch_nq;; *nmix*|*nqD*) export EVAL500_ROOT=/data/prasann/nq_lengthmix/eval_rungs; LT=nq;; *) export EVAL500_ROOT=/data/prasann/outlier_lengthmix/eval500_local/outlier; LT=outlier;; esac
...
OLMO_LANDMARK_SPARSE_DECODE=1 $PY -m torch.distributed.run --nproc_per_node=2 --master_port=$((20000 + SLURM_JOB_ID % 10000)) $REPO/src/scripts/ctc_eval/eval/eval_lc_native.py \
  ...
  --max-length 72000 --max-test-samples 600 --batch-size 1 --skip-ruler --skip-gen \
  --landmark-mem-id 248200 --landmark-pad-id 248203 --eos-token-id 248044 \
  --prompt-format chat --query-position after \
  --ladder --ladder-tasks $LT --ladder-rungs '"$RUNGS"'
cp "$DST/eval_outlier_multirung.json" /scratch/users/prasann/stl_eval_results/${RUN}-chat_shp_multirung.json
cp "$DST"/eval_outlier_multirung*generations* /scratch/users/prasann/stl_eval_results/${RUN}-chat_shp.generations.jsonl 2>/dev/null || true
rm -rf "$DST"
rm -rf "$DST"
echo EVAL-OK'
}
for i in $(seq 1 220); do
    ...
    case "$ST" in
      ...
      TRAIN)
        ...
        if [[ "$S" == "DONE:0" ]]; then
          ...
          echo "GAVE_UP:train-$S" > "$SF"; echo "[shp] $RUN train ended badly: $S"
        fi ;;
      PUSH:*)
        P=${ST#PUSH:}; S=$(state "$P")
  ...
  N=$(grep -l '^EVAL$' "$STATE"/*.shp.state 2>/dev/null | wc -l)
  echo "[shp tick $i] $N/16 chained $(date '+%T')"
  [ "$N" -ge 16 ] && break
  sleep 300
done
echo "[shp] exiting"
