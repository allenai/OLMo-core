...
JOBS="
lmx-full-p64k1500-4b|full
lmx-full-mixs64k96M-4b|full
lmx-slm-mixs64k96M-4b|slm
"
...
'"$SPD"'$PY -m torch.distributed.run --nproc_per_node=2 --master_port=$((20000 + SLURM_JOB_ID % 10000)) $REPO/src/scripts/ctc_eval/eval/eval_lc_native.py \
  --model-path "$DST" --out "$DST/eval_outlier64k_multirung.json" --tokenizer Qwen/Qwen3.5-0.8B \
  --max-length 72000 --max-test-samples 600 --batch-size '"$BS"' --skip-ruler --skip-gen \
  --landmark-mem-id 248200 --landmark-pad-id 248203 --eos-token-id 248044 \
  --prompt-format chat --query-position after \
  --ladder --ladder-tasks outlier --ladder-rungs 3k,8k,16k,32k,64k # ,8k
cp "$DST/eval_outlier64k_multirung.json" /scratch/users/prasann/stl_eval_results/${RUN}-chat_qdmatch_multirung.json
rm -rf "$DST"
echo EVAL-OK'
}
...
for i in $(seq 1 100); do
  ...
  N=$(grep -l '^EVAL$' "$STATE"/*.k64.state 2>/dev/null | wc -l)
  echo "[qdchaser tick $i] $N/3 chained $(date '+%T')"
  [ "$N" -ge 3 ] && break
  sleep 300
done
