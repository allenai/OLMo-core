...
JOBS="
lmx-full-nq2k1250-4b|full
lmx-full-nq2k2500-4b|full
lmx-full-nq2k5000-4b|full
lmx-full-nq2k10000-4b|full
lmx-full-nq2k20000-4b|full
lmx-full-nq8k1000-4b|full
lmx-full-nq8k2000-4b|full
lmx-full-nq8k4000-4b|full
lmx-full-nq8k8000-4b|full
lmx-slm-nq2k20000-4b|slm
lmx-slm-nq8k8000-4b|slm
"
...
export EVAL500_ROOT=/data/prasann/nq_lengthmix/eval_rungs
...
'"$SPD"'$PY -m torch.distributed.run --nproc_per_node=2 --master_port=$((20000 + SLURM_JOB_ID % 10000)) $REPO/src/scripts/ctc_eval/eval/eval_lc_native.py \
  --model-path "$DST" --out "$DST/eval_nq_multirung.json" --tokenizer Qwen/Qwen3.5-0.8B \
  ...
  --ladder --ladder-tasks nq --ladder-rungs 2k,8k
cp "$DST/eval_nq_multirung.json" /scratch/users/prasann/stl_eval_results/${RUN}-chat_nq_multirung.json
rm -rf "$DST"
echo EVAL-OK'
}
...
for i in $(seq 1 100); do
  ...
  N=$(grep -l '^EVAL$' "$STATE"/*.nql.state 2>/dev/null | wc -l)
  echo "[qdchaser tick $i] $N/11 chained $(date '+%T')"
  [ "$N" -ge 11 ] && break
  sleep 300
done
