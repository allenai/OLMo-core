...
def main():
    ...
    # The text-only export has no image/video processor (it was never a VL checkpoint) --
    # but vLLM resolves any Qwen3_5 architecture to the VL wrapper class, whose init still
    # probes for one (used only to size the (unused) multimodal budget for text prompts).
    # Pull those from the base snapshot, same as the vision-tower weights in make_vl_weights.py.
