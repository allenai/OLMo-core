...
<div class="note"><b>2026-09-08 afternoon — parity on all four tasks.</b> A slot may stand in for a document's free text but never for the tokens the question matches exactly (claim ids, user ids, dates). Keeping each document's <i>header</i> real (<code>Claim N:</code> for contradiction, <code>Date: … || User: … || Instance:</code> for oolong; <code>--prefix-real stopK</code>) and pooling only the body reproduces full attention: contradiction Δ −0.015 ± 0.02 at keep 1/36 (≈4.8×), oolong Δ −0.009 ± 0.04 at keep 1/3 (≈1.4×; its header is half the line). Contradiction also closes with the document <i>after</i> each gold claim real (<code>goldnbr1</code>, leak-free <code>+runs</code>: Δ −0.002 at keep 1/36, ≈11×) — the right neighbour matters, the left does not. Rows from sneetches are a different held-out 24 than the Beaker rows (contradiction full 0.071 vs 0.042, oolong 0.507 vs 0.466). Trainer flag: <code>--st-header-stop-id 25 --st-header-stop-count {1|3}</code>.</div>
<ul>
<li><b>Slot logit bias</b> (none / +log L / +log L + c / constant c): calibration is task-dependent; retrieval tasks want ≤ 0, oolong wants ≥ log L + 2; no single bias helps all four; flat at keep 0. <i>Done, all four tasks, 24 rows.</i></li>
...
<li><b>Fitted log-mass slot</b> and <b>G slots per document</b> (2 / 4 / 8): move nothing on any task. <i>Done.</i></li>
<li><b>Keep policies</b> (overlap / length / short / first / last / hard negatives): content-based ≈ random per token kept; positional (first/last = one contiguous run of slots) is catastrophic on contradiction and nq, helps outlier. <i>Done.</i></li>
<li><b>GDN no-write</b> (slots attention-only): helps contradiction only at low keep, hurts everywhere else. <i>Done.</i></li>
<li><b>Slot RoPE position</b> (centre / start / end): no effect. <i>Done.</i></li>
<li><b>Header real, body pooled</b> and <b>gold-neighbour runs</b>: parity (see note). <i>Done on sneetches rows; Beaker confirmation on the eval-bundle rows in probe <code>header</code> / <code>nbr-runs</code>.</i></li>
<li>Next: training arms with header-real (contradiction 56M keep 1/36 and 1/12, oolong 80M keep 1/3) against the dense ladder; a <code>gold_plus_random_runs</code> keep mode for the 11× arm.</li>
</ul>
