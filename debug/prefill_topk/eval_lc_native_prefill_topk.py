"""
COPY of ``src/scripts/ctc_eval/eval/eval_lc_native.py`` with **prefill-wide top-k landmark
retrieval** bolted on. Kept separate so the production eval script is untouched.

Landmark/compressive-landmark eval normally applies hard top-k block retrieval only at *decode*
(``GenerationConfig.landmark_top_k_fraction``, default 0.1); the prefill still soft-gates every
prompt token over ALL past blocks. The ``--prefill-topk*`` flags below additionally route the prefill
through :mod:`olmo_core.nn.attention.landmark_prefill_topk`, so every query position -- prompt tokens
included -- sees only its top-k blocks. Diffing the two runs measures how much accuracy the
"honestly sparse" landmark model loses. Everything else (prompts, grader, decode) is unchanged from
the production script, so the numbers are directly comparable to results-hub rows.
...
def main():
    ...
    # ---- prefill-wide top-k landmark retrieval (this copy's whole reason to exist) ----
    ap.add_argument("--prefill-topk", type=int, default=None,
                    help="landmark models: apply hard top-k block retrieval to EVERY prefill query "
                         "(not just the decoded ones), keeping this many past blocks. Takes "
                         "precedence over --prefill-topk-fraction. Unset (and no fraction) = the "
                         "production behaviour: dense soft-gated prefill, top-k at decode only.")
    ap.add_argument("--prefill-topk-fraction", type=float, default=None,
                    help="as --prefill-topk but k = ceil(fraction * num_prompt_blocks), the same "
                         "rule GenerationConfig.landmark_top_k_fraction uses at decode.")
    ap.add_argument("--prefill-nonselected-mass", type=float, default=None,
                    help="compressive landmark only: alpha reserved for the NON-selected blocks' "
                         "landmark tokens during prefill. Default = each layer's own "
    ...
    gen_cfg = GenerationConfig(eos_token_id=tok.eos_token_id, pad_token_id=tok.pad_token_id,
                               ...
                               landmark_top_k_fraction=(args.decode_topk_fraction or None))
    ...
    if args.prefill_topk is not None or args.prefill_topk_fraction is not None:
        ...
        n_patched = enable_prefill_topk(
            ...
            nonselected_mass=args.prefill_nonselected_mass,
            query_tile=args.prefill_query_tile,
        )
        prefill_topk_info = {
            "top_k": args.prefill_topk,
            "top_k_fraction": args.prefill_topk_fraction,
        ...
        print(f"[prefill-topk] patched {n_patched} landmark layers: {prefill_topk_info}", flush=True)
    else:
        print("[prefill-topk] OFF (dense soft-gated prefill; top-k at decode only)", flush=True)
    print(f"[decode-topk] fraction={gen_cfg.landmark_top_k_fraction}", flush=True)
    ...
    summary = {"model_path": args.model_path, "ruler": {}, "contradiction": {}, "nq": {},
               "prefill_topk": prefill_topk_info,
               "decode_topk_fraction": gen_cfg.landmark_top_k_fraction}
