...
from corpus_reasoning.lib.data_format_smoothed import build_prompt
from corpus_reasoning.lib.attn_smoothing import (
    alpha_from_step, attach_row_mask_pre_hook,
    install_smoothed_attention, parse_smoothing_config, setup_smoothing_tokens,
)
from corpus_reasoning.lib.longlora_attn import (
    install_s2_attn, parse_longlora_config, LONGLORA_MODULES_TO_SAVE,
)
...
class ChunkedDataset(Dataset):
    def __init__(self, data_path, tokenizer, max_len, doc_start_id, doc_end_id,
                 ...
                 unified_prompt=False, cot_mode="label", wrap_question=False,
        ...
        self.wrap_question = wrap_question
        # Per-example mix: with prob standard_mix_prob, skip wrap_documents()
        # so the example has no doc-boundary tokens. All-FREE chunk_ids hit
        # the universal `q_free | kv_free` clause in every pattern's mask
        # (chunked / hierarchical / etc.), collapsing that example to plain
        # causal attention while the rest stay on the configured pattern.
    ...
    def __getitem__(self, idx):
        ...
        prompt, output = build_prompt(
            ...
            wrap_question=self.wrap_question,
        )
...
class SdpaChunkedCollator:
    ...
    def __call__(self, features):
        ...
        # Hierarchical pattern: defer mask construction to the per-layer SDPA
        # wrapper. The collator emits only a 2D pad mask; the wrapper rebuilds
        # the per-layer (B, 1, S, S) mask from chunk_ids each forward call.
        if self.pattern.name == "hierarchical_anchor":
...
def main():
    ...
    parser.add_argument("--attn", choices=["flex", "sdpa"], default=None,
                        help="Attention backend. Defaults to cfg['attn_backend'] "
                             "or 'sdpa' if unset (this script is dispatched only "
                             "for attn_smoothing / hierarchical_anchor configs, "
                             "both of which require SDPA — flipping the default "
                             "would silently break them). For non-smoothing "
                             "chunked training, use train_chunked_fast.py whose "
                             "default is 'flex'.")
    ...
    if args.attn is None:
        # Default kept at sdpa here — train_chunked_smoothed.py only
        # gets dispatched for attn_smoothing / hierarchical_anchor
        # configs (both override _attn_implementation and require SDPA);
        # the fast-path default flip lives in train_chunked_fast.py.
        args.attn = cfg.get("attn_backend", "sdpa")
    ...
    smoothing = parse_smoothing_config(cfg)
    smoothing_state = {"alpha": 0.0} if smoothing else None
    longlora = parse_longlora_config(cfg)
    if longlora is not None:
        if smoothing is not None:
            raise ValueError(
                "longlora and attn_smoothing both override the "
                "_attn_implementation slot — only one can be active at a time."
            )
        if attention_pattern.name not in ("standard",):
            raise ValueError(
                f"longlora expects attention_pattern=standard "
                f"(plain causal training); got {attention_pattern.name!r}. "
                f"S^2-Attn is a training-time swap on top of full causal "
                f"attention, not a chunked-mask variant."
            )
        print(f"[longlora] enabled, group_size={longlora['group_size']}")
    q_start_id, q_end_id = None, None
    if smoothing:
        # Only register Q_START/Q_END when we need to wrap question spans;
        # otherwise keep the tokenizer identical to the baseline so an
        # alpha=0 run can be numerically identical to an unpatched run.
        if smoothing["smooth_question"]:
            q_start_id, q_end_id = setup_smoothing_tokens(tokenizer)
        print(f"[smoothing] alpha_init={smoothing['alpha_init']} "
              f"decay_end_fraction={smoothing['decay_end_fraction']} "
              f"smooth_q={smoothing['smooth_question']} "
              f"smooth_a={smoothing['smooth_answer']}")
    ...
    # is on the native flex_attention path or the sdpa path.
    flex_state = {}
    if flex_via_state:
        if smoothing:
            raise ValueError(
                "flex_chunked (sdpa-fallback flex path) and attn_smoothing "
                "both override the _attn_implementation slot — only one "
                "can be active at a time. Use --attn sdpa with smoothing, "
                "or disable smoothing for the flex path."
            )
        if attention_pattern.name == "hierarchical_anchor":
            raise ValueError(
                "flex_chunked and hierarchical_anchor both override the "
                "_attn_implementation slot — only one can be active at a "
                "time. hierarchical_anchor requires --attn sdpa."
            )
        install_flex_chunked_attention(model, flex_state)

    if smoothing:
        install_smoothed_attention(model, smoothing_state)

    if longlora is not None:
        if flex_via_state:
            raise ValueError(
                "longlora and flex_chunked both override the "
                "_attn_implementation slot — use --attn sdpa with longlora."
            )
        install_s2_attn(model, group_size=longlora["group_size"])

    hierarchical_state = {}
    if attention_pattern.name == "hierarchical_anchor":
        if smoothing:
            raise ValueError(
                "hierarchical_anchor and attn_smoothing both override the "
                "_attn_implementation slot — only one can be active at a time."
            )
        if args.attn != "sdpa":
            raise ValueError(
                f"hierarchical_anchor currently supports --attn sdpa only "
                f"(got {args.attn!r})."
            )
        num_layers = int(model.config.num_hidden_layers)
        install_hierarchical_sdpa_attention(model, hierarchical_state)
        attach_hierarchical_pre_hook(
            model, hierarchical_state,
            doc_start_id=doc_start_id, doc_end_id=doc_end_id,
            pattern=attention_pattern, num_transformer_layers=num_layers,
        )
        print(
            f"[hierarchical] num_layers={num_layers} "
            f"num_anchors={attention_pattern.num_anchors} "
            f"stride_base={attention_pattern.stride_base}"
        )
    ...
    if cfg.get("adapter") == "lora":
        ...
        # LongLoRA's "LoRA+" recipe: embed_tokens and all RMSNorms are fully
        # trainable on top of LoRA on q/k/v/o. modules_to_save flips
        # requires_grad and saves the deltas alongside the LoRA adapter.
        if longlora is not None:
            lora_kwargs["modules_to_save"] = LONGLORA_MODULES_TO_SAVE
            print(f"[longlora] modules_to_save={LONGLORA_MODULES_TO_SAVE}")
    ...
    wrap_question = bool(smoothing and smoothing["smooth_question"])
    ...
    if standard_mix_prob > 0.0 and standard_attention:
        raise ValueError(
            ...
            "(otherwise every example would already be standard)."
        )
    ...
    if standard_mix_prob > 0.0:
        ...
        print(f"  standard_mix_prob={standard_mix_prob} -> "
              f"{n_std}/{len(dataset)} examples present as plain causal")
    ...
    if smoothing:
        attach_row_mask_pre_hook(
            model, smoothing_state,
            q_start_id=q_start_id, q_end_id=q_end_id,
            smooth_question=smoothing["smooth_question"],
            smooth_answer=smoothing["smooth_answer"],
            pad_token_id=tokenizer.pad_token_id,
        )
        _smoothing = smoothing  # closure capture

        class AlphaCallback(TrainerCallback):
            def on_step_begin(self, args, state, control, **kwargs):
                a = alpha_from_step(
                    state.global_step, max(state.max_steps or 0, 1),
                    _smoothing["alpha_init"], _smoothing["decay_end_fraction"],
                )
                smoothing_state["alpha"] = a

            def on_log(self, args, state, control, logs=None, **kwargs):
                if logs is not None:
                    logs["attn_smoothing_alpha"] = smoothing_state.get("alpha", 0.0)

        trainer.add_callback(AlphaCallback())
