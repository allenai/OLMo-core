...
| `launch_long_context_evals.sh` | 2026-07-13 | Pre-Berkeley gantry/weka HELMET+RULER eval launcher; depends on sibling checkouts `../ai2-helmet` / `../olmo-cookbook` that no longer exist. Local evals: see `local_cluster.md`; Beaker evals: the eval-ladder sbatch/gantry scripts in `src/scripts/train/sft/`. |
