...
%
% ⚠ THE TRIM WAS WRONG IN TWO WAYS AT ONCE, AND THEY MASKED EACH OTHER.
% attn_v2.pdf is a 720 x 404.88pt page whose actual ink sits at x in [243.5, 484.0] and
% y in [13.0, 392.1] -- i.e. 240.5 x 379.1pt of content floating in wide white margins.
%   1. trim=300 30 300 20 keeps only x in [300, 420]. That is a 120pt window INSIDE the content,
%      not around it: it cuts 56pt of row labels off the left and 64pt of matrix off the right.
%   2. There was no `clip`, so the cut material was still painted. LaTeX therefore reserved a box
%      for a 120pt-wide crop while drawing 240pt of ink -- the figure came out at exactly TWICE
%      the width it had reserved, overhanging ~60pt on each side and running into the column.
% Both together are why scaling it looked erratic: `width` controls the 120pt phantom box, and the
% visible figure was always 2x whatever you asked for.
% Correct margins, measured from the rendered page: trim=244 13 236 13 with `clip`. Now the box
% and the ink are the same object, and `width` finally means what it says. 0.85\linewidth is then
% just a fit choice -- \linewidth fills the column but pushes the caption onto the page number.
\begin{wrapfigure}{r}{0.5\textwidth}
    \centering
    \includegraphics[width=0.85\linewidth, trim=244 13 236 13, clip]{figures/attn_v2.pdf}
...
We further propose and apply a new trick---\textbf{mask mixing}: during chunked attention training, we mix in a random $p$ percentage of full attention mask training batches into training, and \emph{then use chunked attention for evaluation}. $p=0$ is just chunked attention, and $p=1$ is just full attention training. We find mask-mixing surprisingly beneficial on various tasks (Table~\ref{tab:maskmix}), potentially due to its quadratic train-time attention complexity. \claude{Its value tracks \cname{} as sharply as everything else in this paper: averaged over the ladder it is worth $+0.011$ on \ctc{N} retrieval (BEIR FiQA, $0.648 \rightarrow 0.658$, a 1.7\% relative gain), but on the high-\cname{} tasks it is often the difference between learning the task and not learning it at all --- Contradiction goes from $0.000$ to $0.791$ and Reordering from $0.000$ to $0.182$, while Outlier (scale-$k$) improves $7.1\times$ ($0.083 \rightarrow 0.588$).} While not the focus of this work, we encourage future work to explore it further.
