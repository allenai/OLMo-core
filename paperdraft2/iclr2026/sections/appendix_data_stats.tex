...
\begin{itemize}
\item \textbf{NQ / HotpotQA:} both use the 100-word DPR Wikipedia corpus \citep{Karpukhin2020DensePR}. Negatives are mined by BM25 with simple text-overlap filters to avoid spurious near-duplicates of the gold passage. HotpotQA uses the bridge subset. \claude{The per-$N$ HotpotQA data listed here carries a
natural-language CoT generated with gemini-2.5-flash and grounded in the gold paragraphs and
answer; the suite's token-ladder HotpotQA data does not, and the suite runs are trained and scored
without a CoT.}
...
% app:amazon-reviews was never defined anywhere. The Amazon outlier task is described in the
% "Task Suite Data Details" appendix, whose label is app:data-details.
\item \textbf{Outlier (Amazon Reviews):} see Appendix~\claude{\ref{app:data-details}}.
