...
checkpoint-native v3 health ledger. Eligibility permits at most eight finite gradient-only guard
...
`rustin/vision-ssmax-direct-alignment-v2`. Replace every placeholder with an exact path/ref. Both
...
`perception-direct-v2` root; failed artifacts from the superseded revision remain immutable.
...
PYTHONPATH=src python src/scripts/eval/vision_alignment_ssmax_perception_direct_manifest.py \
  --spec /weka/.../perception-direct-v2/ssmax_head_qknorm/manifest-spec.json \
  --output /weka/.../perception-direct-v2/ssmax_head_qknorm/manifest.json
```
...
PYTHONPATH=src python src/scripts/beaker_launch_vision_ssmax_evidence.py launch \
  ...
  --manifest /weka/.../perception-direct-v2/ssmax_head_qknorm/manifest.json \
  --expected-manifest-sha256 SHA256 \
  --step 4000 \
  --work-dir /weka/.../perception-direct-v2/ssmax_head_qknorm/work/step4000 \
  --output /weka/.../perception-direct-v2/ssmax_head_qknorm/step4000-evaluation.json
```
