...
| **CTC-SUITE-HIGH** / **CTC-SUITE-LOW** | `make_fig_suite_ladders.py` | `ctc_suite_high`, `ctc_suite_low` | `data/ctc_suite_grid_data.csv` | Appendix reference pair: the dense-vs-chunked ladder for **every** suite task, split by class — 10 high-CTC panels and 12 low-CTC panels (the same 12/10 split CTC-GRID's Average panel reports, read from the data rather than hardcoded). These are the individual tasks behind the class averages in the main figures. Both use a 4-wide grid so the panels are the same size and shape and the two classes can be compared directly; 5×2 fits the 10 high panels but at `\textwidth` the ticks stop being readable. **Axes are shared within a figure on purpose** — the metrics differ per task so absolute scores are not commensurable, but the dense-chunked *gaps* are, and per-panel autoscaling would make a 0.02 gap and a 0.6 gap look identical. Ladders that stop at 16k (re-order, outlier fix-k, absence) are unevaluated cells, not measured zeros, and are printed to stdout for the caption. Writes into **both** `paperdraft/figures/` and `paperdraft2/figures/`. |
...
| **TABLE-DATASTATS** | `make_table_datastats.py` | `table_datastats.tex` (both drafts) | `data/ctc_suite_rung_stats.json` + `data/ctc_suite_grid_data.csv` | LaTeX table (`tab:data-stats`): source corpus, CTC class, metric, **measured** documents-per-example at each of the five rungs, median chars/doc and eval size, for all 22 suite tasks. **Replaces the hand-written per-$N$ table**, which described a regime (NQ `k=20..200`, contradiction `N=20..500`, eval sizes 400/488/200) that no current experiment uses, covered 7 of 22 tasks, and contradicted `tab:hyperparams`'s own caption on where the contradiction ladder starts. Doc counts are medians over real examples of the exact rung file each result was graded on, resolved through the CSV's `ladder`+`task_dir` columns — several tasks are served from a non-default tree (`nq`→`eval_rungs_ext32k`, `msmarco`→`eval_rungs_tokenaccurate`, `outlier_fixk`→`eval_rungs_fixedM`). ⚠ Two rows are flagged in-table because measuring found their ladders are not clean length sweeps: † **OBLIQ** is non-monotone — its 8k rung file (71.8 MB) is *larger* than its 32k one (66.5 MB) and its 16k rung is smaller than both, and the score curve tracks the corpus rather than the label (0.608 → 0.277 → 0.327); ‡ **MS MARCO rerank**'s pool saturates at 20/40/70/100 docs, so its "16k" rung is ~34k characters, and no `eval_data` path was recorded for its graded runs so the 32k rung's provenance is unconfirmed. Both feed the low-CTC average in CTC-GRID. Sizes in **characters, not tokens** — rung labels are build-time token targets that are individually wrong in both directions (see `debug/ctc_modelscale/measure_all_rungs.py`). |
