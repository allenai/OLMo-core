...
from olmo_core.nn.parametrization import MupScalingStrategy
    ...
    *,
    ...
    if (
        ...
        raise NotImplementedError(
            f"Conversion of parametrization models to HF is not yet supported for parametrization models not using {MupScalingStrategy.constant_inputs} strategy."
        )
    if (
        (feed_forward := model_config.block.feed_forward) is not None
        and feed_forward.parametrization is not None
        and feed_forward.parametrization.scaling_strategy != MupScalingStrategy.constant_inputs
    ):
        raise NotImplementedError(
            f"Conversion of parametrization models to HF is not yet supported for parametrization models not using {MupScalingStrategy.constant_inputs} strategy."
        )
