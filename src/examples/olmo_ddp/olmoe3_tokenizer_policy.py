...
def require_correct_tokenizer(path, *, runtime=False):
    ...
    if digest != PRETOKENIZER_SHA256:
        raise ValueError(f"Incorrect production tokenizer at {path}; retokenization/retraining required")
    ...
    if runtime:
        ...
        for text in ("12345678901234567890", "def f():\n    return 123456\n", "中文 123456"):
            if tok.encode(text, add_special_tokens=False) != reference.encode(
