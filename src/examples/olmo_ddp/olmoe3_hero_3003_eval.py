"""Bounded step179000 stable-PT evaluation: frozen workers, HF fallback, and base evals.

This campaign does not modify training, checkpoint retention, or inference kernels.
Each arm has one conversion, one qualification, and three base-evaluation jobs. Failed
...
CAMPAIGN = "olmo35-small-stable3003t-20260914"
BRANCH = "codex/hero-3t-evals-20260914"
STEP = 179000
ROOT = MOUNT / "scratch" / CAMPAIGN
AUTOMATION = MOUNT / "uploader/automation" / CAMPAIGN
ARMS = ("emo", "non-emo")
STAGES = ("convert", "qualify", "gen_mc", "math", "code")
...
def run_worker(args):
    ...
    if args.mode == "ruler":
        ...
        sys.argv = [ruler.__file__, "--milestone", "3003t", "--arm", args.arm, "--instances", "4"]
        ruler.main()
        return
    ...
    if args.mode == "convert":
        ...
        for module in (download, stage, convert):
            module.SCRATCH, module.TARGETS = ROOT, {3003: STEP}
...
def advance(control, arm, specs):
    ...
    for stage in STAGES:
        ...
        if state == "STATUS_SUCCEEDED" and stage not in ("convert", "qualify"):
            if stage == "ruler":
                ruler.verify_success(model_path("3003t", arm))
            else:
...
def watch(check=False):
    ...
    with Beaker.from_env(check_for_upgrades=False) as b:
        ...
        with (AUTOMATION / "LOCK").open("a") as lock:
            ...
            log("MATCHED_3003_WATCHER_ARMED", **plan)
            while True:
                snapshot = {}
                for arm in ARMS:
                    try:
                ...
                if snapshot != previous:
                    log("MATCHED_3003_STATUS", arms=snapshot)
                    previous = snapshot
...
def controller_spec(template, commit, check=False):
    ...
    spec[
        "description"
    ] = f"{CAMPAIGN}: two stable PT179000 arms -> conversion/qualification -> OLMoBase only; RULER excluded. No training, retention or deletion changes."
...
def launch(check, apply):
    ...
    with Beaker.from_env(check_for_upgrades=False) as b:
        ...
        if not apply:
            log("MATCHED_3003_PLAN", name=name, source_commit=commit)
            return
        if not check:
            assert (
