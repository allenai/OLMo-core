"""Clean-pin deployment: real-image CPU gate, then a no-resource Phobos LC watcher."""
...
from olmoe3_hero_lc_control import mount_lc, training_spec
from olmoe3_hero_lc_eval import eval_specs, ruler_spec
...
def build_spec(beaker, commit, stage, gate=None):
    ...
    if stage == "validate":
        t["arguments"] = [
            ...
            "python src/examples/olmo_ddp/olmoe3_hero_lc.py --validate-only && "
            "python src/examples/olmo_ddp/olmoe3_hero_lc.py --data-probe",
        ]
    else:
        assert gate
        t["arguments"] = [
            ...
            "python -u src/examples/olmo_ddp/olmoe3_hero_lc_control.py",
        ]
    if stage == "validate":
        mount_lc(t)
    replace_env(
    ...
    with Beaker.from_env(check_for_upgrades=False) as b:
        for run in runs():
            ...
            plans = dict(
                ...
                ruler=ruler_spec(b, run, commit),
                **eval_specs(b, run, commit),
            )
