...
def model_path(milestone, arm):
    """Only this campaign's two final 100B LC exports."""
    from olmoe3_hero_lc_plan import END, EVAL_ROOT

    assert milestone == "lc100b" and arm in ("emo", "non-emo")
    return EVAL_ROOT / arm / f"step{END}/hf"
...
def main():
    ...
    parser.add_argument("--milestone", choices=("lc100b",), required=True)
    ...
    runtime.FAST_MODELS = {model_path(m, a) for m in ("lc100b",) for a in ("emo", "non-emo")}
