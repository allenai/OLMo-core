...
| MT |16,777,216|64|2,125|2.2e-4 (20% of stable PT), linear to zero, no warmup|0,2,4; every250;2125:12|
...
PT ends at111.853699072B tokens. Revised MT processes35.651584B; LC remains35.641884672B;
the ladder formula requests35,641,421,562 tokens, rounded up to whole batches.
The September19 revision restarts MT from the finished PT checkpoints with fresh
optimizer/data, in distinct `-mt-16mi-linear0` runs; old MT outputs are preserved.
MT uses the hero batch/topology and the20% LR rule. LC settings are provisional:
**LC and SFT are blocked until the user reviews MT evaluation and approves LC.**
EMO is ON only during PT/decay and OFF for MT/LC/SFT.
...
stage completion gates evaluation. MT performs its2→4-step full-state resume
check within its main allocation before continuing to2125. LC is explicitly held.
...
- Controller: `uploader/automation/olmo35-qkgain-20260919/mt16mi-linear0`.
- Restored reference: original controller root `sources/emo/step6000/olmo-core`.
