#!/usr/bin/env python3
"""
Build **shared-corpus** ("efficient") variants of the v2 eval rungs. EVAL ONLY.

Motivation: a v2 rung is 500 examples over 500 *independent* corpora, so a 32k rung costs 500 full
...
Usage::
    ...
    python -m corpus_reasoning.data.build_shared_corpus_evals --task oolong --rung 32768
    python -m corpus_reasoning.data.build_shared_corpus_evals --task nq --rung 8192
    python -m corpus_reasoning.data.build_shared_corpus_evals --task contradiction --rung 32768 \\
        --tail-frac 0.05 0.25
"""
import argparse
import collections
...
EVAL_RUNGS = "/scratch/users/prasann/ctc_suite_staged/eval_rungs"
OUT_ROOT_DEFAULT = "/scratch/users/prasann/ctc_suite_staged/eval_rungs_shared"
CR_DATA = "/scratch/users/prasann/corpus-reasoning/data"
...
def build_oolong_shared(rung, out_root, **_):
    ...
    """
    src = f"{CR_DATA}/oolong_test_synth_ctx{rung}_spliteval.jsonl"
    rows = load_jsonl(src)
...
def build_multiplexed(task, rung, out_root, queries_per_corpus=None, seed=SEED,
    ...
    """
    rows = load_jsonl(f"{EVAL_RUNGS}/{task}/rung_{rung}.jsonl")
    ...
    save_jsonl(path, out)
    return path, {
        "source": f"{EVAL_RUNGS}/{task}/rung_{rung}.jsonl",
        "rows": len(out),
...
def build_outlier_shared(task, rung, out_root, tail_frac=0.05,
                         queries_per_corpus=None, seed=SEED, scatter_gold=False, **_):
