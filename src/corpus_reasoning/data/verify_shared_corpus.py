...
def main():
    ...
    p.add_argument("--index-base", type=int, default=None,
    ...
    for cid, members in groups.items():
        ...
        for i, r in members:
            ...
            shas.add((n, actual))
        if len(shas) > 1:
            errors.append(f"corpus {cid}: {len(shas)} distinct prefixes across {len(members)} rows "
    ...
    print(f"rows            : {len(rows)}  (eval_size={len(rows)}"
          f"{'  ⚠ <500, quote its SE inline' if len(rows) < 500 else ''})")
    print(f"corpora         : {len(groups)}  queries/corpus="
          f"{sorted({len(v) for v in groups.values()})}")
    print(f"ndocs           : {len(rows[0]['documents'])}  shared_prefix_len="
          f"{rows[0]['shared_prefix_len']}  tail={sorted(set(tail_sizes))}")
    shared_frac = rows[0]['shared_prefix_len'] / len(rows[0]['documents'])
    print(f"shared doc frac : {shared_frac:.3f}   (~{1/max(1e-9, 1-shared_frac):.1f}x prefill "
          f"saving if the tail dominates cost)" if shared_frac < 1
          else "shared doc frac : 1.000   (fully shared corpus)")
    if total_gold:
        print(f"gold placement  : {in_prefix}/{total_gold} in shared prefix, "
              f"{in_tail}/{total_gold} in per-query tail")
    if pair_total:
        print(f"pair split      : {pair_split_ok}/{pair_total} pairs have exactly one member in "
              f"the prefix and its partner in the tail")
        if pair_split_ok != pair_total:
            errors.append(f"pair split violated on {pair_total - pair_split_ok} pairs "
    ...
    if in_tail and tail_sizes:
        ...
        print(f"guess-tail ctrl : f1≈{k/avg_tail:.3f}  (score of blindly answering with {k:.1f} "
