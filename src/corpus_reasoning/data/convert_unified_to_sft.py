...
EOS = 151643  # Qwen3 lineage default; OVERRIDE via --eos for other tokenizers (Qwen3.5: 248044).
...
def tokenize_instance(tok, user_content, answer, max_seq_len, eos=EOS):
...
def main():
    ...
    ap.add_argument(
        "--task", required=True, help="build_prompt task name (e.g. retrieval, rerank, outlier)"
    )
    ...
    ap.add_argument(
        ...
        type=int,
        default=EOS,
        help="Answer-terminator token id appended with loss=True. MUST match the "
        "training family's eos (Qwen3: 151643, Qwen3.5: 248044) -- the Qwen3 "
        "default is WRONG for Qwen3.5 shards.",
    )
    args = ap.parse_args()
    ...
    for i, line in enumerate(open(args.input)):
        ...
        prompt, answer = build_prompt(
            ex, task=args.task, use_alpaca=False, query_position=args.query_position
        )
        r = tokenize_instance(tok, prompt, answer, args.max_seq_len, eos=args.eos)
        if r is None:
            n_skip += 1
            continue
        ids, mask = r
        tok_chunks.append(ids)
    ...
    toks = np.concatenate(tok_chunks)
    masks = np.concatenate(mask_chunks)
