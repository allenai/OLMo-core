"""Convert (aligned/fixed) NQ retrieval JSONL -> olmo-core SFT shards matching
...
Prompt comes from scripts.lib.data_format.build_prompt(task="retrieval") — byte-identical to the
eval — rendered through the Qwen3 chat template. Loss mask = assistant turn only (offset-based).

    python scripts/data/convert_nq_to_sft.py \
      --input data/nq_train_k20_hn19_2500_aligned.jsonl \
      --out-dir /scratch/users/prasann/cpt_data/nq_aligned_k20_qwen --max-seq-len 8192
"""
import argparse
import json
import os
...
def main():
    ...
    for i, line in enumerate(open(args.input)):
        if args.limit and n >= args.limit:
            break
        ex = json.loads(line)
        prompt, answer = build_prompt(ex, task="retrieval", use_alpaca=False,
    ...
    meta = {"input": args.input, "num_instances": n, "num_skipped": n_skip,
            ...
            "tokenizer": args.tokenizer, "eos": EOS, "task": "retrieval"}
