...
DEPRECATED / REDUNDANT for data from the current ``generate_nq_training_data.py``.
That builder now draws **gold, hard negatives, AND random distractors all from the
single ``wikipedia-dpr-100w`` index**, so every doc already shares one surface format
and ~100-word length. There is no cross-source gap left to fix — measured gold/hard/
random median lengths differ by <5% and pre- vs post-align tokens/doc are ~identical
(157 -> 154). **Build pipelines should SKIP this step.** It is kept only for older data
whose gold/random came from NQ HF contexts (the mismatch described below).
