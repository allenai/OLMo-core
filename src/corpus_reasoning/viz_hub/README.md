...
> **Two sites, two projects — do not cross the streams.** There are two viz builds
> on two branches, each with its OWN Pages project so neither clobbers the other:
>
> | branch / checkout | content | Pages project → URL |
> |---|---|---|
> | corpus-reasoning `main` (this OLMo-core submodule) | Overview / Data / Experiments **+ results-hub "Results" tab** | `corpus-reasoning-hub` → `corpus-reasoning-hub.pages.dev` |
> | `prasann/singletask-ladder` (standalone `/scratch/users/prasann/corpus-reasoning`) | **"Headline · 32k matrix" mixing-results tables** + Doc-Chunked | `corpus-reasoning-viz` → `corpus-reasoning-viz.pages.dev` |
>
> `deploy_cloudflare.sh` on `main` defaults to `corpus-reasoning-hub` and **refuses**
> to deploy to `corpus-reasoning-viz` (override: `ALLOW_VIZ_CLOBBER=1`). Deploying the
> mixing site over `corpus-reasoning-viz` is exactly the "mixing results disappeared"
> regression — rebuild+deploy that one from the standalone singletask-ladder checkout.
...
3. (optional) `export CF_PAGES_PROJECT=corpus-reasoning-hub` to name the project
   (default for this `main` build; see the two-project note above).
