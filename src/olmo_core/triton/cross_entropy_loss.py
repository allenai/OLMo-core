...
class CrossEntropyLoss(torch.autograd.Function):
    ...
    def forward(
        ...
        with torch.cuda.device(logits.device.index):
            cross_entropy_fwd_kernel[(n_rows,)](  # type: ignore
                losses,  # data ptrs
                lse,
                z_losses,
                logits,
        ...
        if world_size > 1:
            ...
            if world_size > 1:
                ...
                dist.all_gather_into_tensor(lse_allgather, lse, group=process_group)
                handle_losses = dist.all_reduce(
                    losses, op=dist.ReduceOp.SUM, group=process_group, async_op=True
                )
                lse = torch.logsumexp(lse_allgather, dim=0)
                handle_losses.wait()  # type: ignore
    ...
    def backward(ctx, grad_losses, grad_z_losses):
        ...
        # ValueError: Pointer argument (at 0) cannot be accessed from Triton (cpu tensor?)
        with torch.cuda.device(logits.device.index):
            cross_entropy_bwd_kernel[grid](  # type: ignore
                dlogits,  # data ptrs
                grad_losses,
                logits,
                lse,
