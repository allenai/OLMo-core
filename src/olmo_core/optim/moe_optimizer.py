"""Sharded AdamW optimizer state for OLMoDDP models."""
...
class OLMoDDPOptimizerConfig(Config):
    ...
    def _build_partitioned_groups(
        ...
        *,
        ...
        # Checkpoint and scheduler group order is dense groups followed by EP groups.
...
def assign_full_tensor_to_dtensor(dst: DTensor, src: torch.Tensor) -> None:
    """Distribute a full tensor into the destination DTensor's mesh and placements."""
...
class OLMoDDPOptimizer:
    ...
    def print_memory_summary(self):
        """Log model, gradient, optimizer-state, and FP8-cache memory estimates."""
        total_params = 0
    ...
    def _maybe_debug_nan_inf_grad_norm(
        ...
        """Report nonfinite norms when ``OLMO_DDP_DEBUG_NONFINITE_GRAD`` is enabled.

        Parameter and component diagnostics are collected only for nonfinite norms on selected
        ranks. ``OLMO_DDP_DEBUG_NONFINITE_GRAD_TOPK`` limits log entries;
        ``OLMO_DEBUG_DUMP_DIR`` enables full diagnostic reports.
        """
    ...
    def step(self, closure: Optional[Callable[[], float]] = None) -> Optional[float]:
        ...
        if getattr(self, "_use_reduce_scatter_grads", False):
            # Optimizer-owned reduction is disabled when MultiGroupDDP owns gradient reduction.
        ...
        for param_group in self.param_groups:
            for name, param in param_group["named_params"].items():
                ...
                if all(
                    ...
                    main_grad_local = model_grad_fp32  # replicated reduced gradient
    ...
    def _step_foreach(self, closure=None) -> None:
        ...
        for group in self.param_groups:
            ...
            # Retain BF16 state tensors for copying back the FP32 updates.
            ...
            def maybe_copy_back_16bit_states():
                # FP32 state updates are in-place; BF16 states use temporary FP32 copies.
            ...
            for name, model_p in group["named_params"].items():
                ...
                if self.states_dtype == torch.bfloat16:
                    # Use FP32 temporaries for the AdamW update.
    ...
    def zero_grad(self, set_to_none=True):
        """Require gradient clearing through the train module's model buffers."""
        raise RuntimeError("Clear model gradients through the train module, not the optimizer.")

    def unsharded_state_dict(self) -> dict:
        """Reject unsharded state export; use :meth:`state_dict` for distributed checkpoints."""
        raise NotImplementedError("Unsharded optimizer state export is not supported")
    ...
    def state_dict(self) -> dict:
        """Export distributed optimizer state and rolling skip-step statistics.

        EP checkpoint conversion may release live shard storage. Call :meth:`load_state_dict`
        with the exported state after saving and before continuing training.
        """
        sd = {}
        for param_group in self.param_groups:
            for name, param in param_group["named_params"].items():
                    ...
                    for suffix in all_suffixes:
                        ...
                        if suffix != "step":
                            ...
                            if not self._shares_tensor_storage(ckpt_local, state_local):
                                # Release separate shard storage until the synchronous save
                                # finishes. The train module reloads it from this checkpoint view.
...
# Compatibility names for serialized optimizer configurations.
