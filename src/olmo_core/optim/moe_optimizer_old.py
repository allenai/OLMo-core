...
class MoEFusedV2Optimizer(Optimizer):
    ...
    def _build_segments_by_rank(self, tag: str):
        ...
        segments_by_rank: List[List[Tuple[str, torch.nn.Parameter, int, int]]] = [[] for _ in range(ws)]
        # segments_by_rank[rank]     = List of (param_name, param, start_in_param, length)
        for rank in range(ws):
            s_start, s_end = shard_ranges[rank] # marks the start/end of this rank's shard in global coords
            for i, (n, p) in enumerate(order.items()):
        ...
        for tag in ("ep_dp", "dp"):
            ...
            def _fill_rank_piece(r: int, need: int, dst: torch.Tensor, dst_off: int) -> None:
                ...
                while wrote < need:
                    ...
                    if take > 0:
                        assert p.grad is not None, f"Param grad is None for param {p} on rank {my_rank} for tag {tag}"
    ...
    def unsharded_state_dict(self) -> dict:
        ...
        for tag in ("dp", "ep_dp"):
            ...
            ws , r = self._world_and_rank(pg)
            for buf_name in ("main", 
                             "exp_avg", 
                             "exp_avg_sq"
                             ):
                sharded_buf = getattr(self, f"_flat_{buf_name}_{tag}") # Sharded over DP or EP_DP
                all_global_bufs_cpu = []
                global_buf = torch.empty_like(sharded_buf) # 1 rank's local shard size, receive other ranks' shards
                for src_rank in range(ws):
                    print(f"Broadcasting/Receiving {buf_name} shard for tag {tag} from src_rank {src_rank} (total {ws})")
                    ...
                    if r == src_rank:
                        ...
                        dist.broadcast(global_buf, src=dist.get_global_rank(pg, src_rank), group=pg) # send
                    else:
                        dist.broadcast(global_buf, src=dist.get_global_rank(pg, src_rank), group=pg) # recv
                
                    all_global_bufs_cpu.append(global_buf.cpu()) # put each rank's shard to cpu, and later concat them

                unsharded_buf_cpu = torch.cat(all_global_bufs_cpu, dim=0) # a unsharded buffer on cpu, all ranks in the pg have the same copy
                assert unsharded_buf_cpu.numel() == N, f"Unsharded buffer size {unsharded_buf_cpu.numel()}, expected {N}"
                ...
                for param_name, param in order.items(): # for each param in this tag
                    p_start, p_end = self.param_ranges_by_tag[tag][param_name] # get start/end in the global unsharded buffer
                    per_param_buf = unsharded_buf_cpu[p_start:p_end]
                    unsharded_state_dict[f"{param_name}.{buf_name}"] = per_param_buf.reshape(param.shape)
    ...
    def _as_dtensor(self, local: torch.Tensor, tag: str, global_numel: int) -> DTensor:
        if tag == 'dp':
            ...
            return DTensor.from_local(local, self.dp_mesh['dp'], placements=[Shard(0)], )
        elif tag == 'ep_dp':
