...
def _validate_direct_evaluation_receipt(
    ...
    try:
        ...
        paired._validate_state(receipt["state"], arm=TRAINING_ROLE, step=step)
        paired._validate_text_result(
            receipt["text_sentinel"], manifest=manifest, arm=TRAINING_ROLE, step=step
        )
    except paired.SSMaxPerceptionEvidenceError as error:
        raise SSMaxPerceptionDirectEvidenceError(str(error)) from error
    attention = _mapping(
...
def build_promotion_report(
    *,
    ...
    canonical_text = evaluation_payloads[0]["text_sentinel"]
    ...
    for step in REQUIRED_STEPS:
        ...
        if evaluation_payloads[step]["text_sentinel"] != canonical_text:
            deviations.append({"kind": "native_text_sentinel_changed", "step": step})
