...
class GradientDumperCallback(Callback):
    ...
    def pre_optim_step(self):
        ...
        if self.save_first_n is None:
            # Save sharded gradients using distributed checkpoint
            ...
            log.info(f"Saved sharded gradients for step {self.step} to '{sharded_dir}'")
        else:
            ...
            for name, p in model.named_parameters():
                ...
                if not is_distributed() or get_rank() == 0:
                    ...
                    slice_filepath = sampled_dir / slice_filename
                    save_file({"gradient": sliced_grad}, str(slice_filepath))
                    log.info(f"Saved first {actual_n} of '{name}' to '{slice_filepath}'")

            log.info(f"Saved sampled gradients for step {self.step} to '{sampled_dir}'")
