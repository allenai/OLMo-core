...
- The batch also carries ``images``, ``image_masks``, ``pooled_patches_idx``
  which flow through to ``MultimodalTransformer.forward`` as ``**model_kwargs``.
...
class MultimodalTransformerTrainModule(TransformerTrainModule):
    ...
    def _move_image_kwargs_to_device(self, batch: Dict[str, Any]) -> None:
        ...
        ``labels`` but doesn't know about our ``images`` / ``image_masks`` /
        ...
        for key in ("images", "image_masks", "pooled_patches_idx"):
