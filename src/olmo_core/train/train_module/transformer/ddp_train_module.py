"""Distributed training and checkpointing for OLMoDDP models."""
...
def cpu_mesh_like(gpu_mesh: DeviceMesh) -> DeviceMesh:
    """Create a CPU device mesh with the same rank layout and dimension names."""
    return DeviceMesh(
        "cpu",
        gpu_mesh.mesh.clone(),
...
FlatSavePlanner = DefaultSavePlanner
...
class OLMoDDPTrainModule(TrainModule):
    """Train an OLMoDDP model with sharded optimizer state and optional EP, CP, or PP."""
    ...
    def __init__(
        ...
        if not self.eval_only:
            ...
            # MultiGroupDDP reduces normal parameters; FP8WeightStore gradients use a separate
            # synchronization path in the optimizer.
        ...
        *,
        ...
        if ep is None:  # EP not used
            ...
            with torch.device("cpu"):
                ...
                if pp is not None:
                    pp_world_size_for_layout = pp.degree
    ...
    def _pp_dry_run_mode(self) -> str:
        """Select local-stage warmup or full pipeline warmup from the runtime setting."""
    ...
    def _run_independent_pp_dry_run(
        ...
        **kwargs,
        ...
        """Warm up each local pipeline stage while preserving its training microbatch shape."""
        assert self.pp_enabled
        assert self._pp_stages is not None
    ...
    def train_batch(self, batch: Dict[str, Any], dry_run: bool = False):
        ...
        with nvtx.annotate("record_metrics"):
            if self.pp_enabled:
                ...
                # Skip-step decisions require the same globally averaged loss on every rank.
                if is_distributed():
    ...
    def optim_step(self):
        ...
        # The optimizer copies updated master parameters to the model and refreshes FP8 caches.
        optim.step()
    ...
    def _debug_dump_kind_enabled(self, kind: str) -> bool:
        return self._debug_env_flag(f"OLMO_DEBUG_DUMP_{kind.upper()}")
        ...
        with contextlib.ExitStack() as stack:
            if isinstance(self.model_parts[0], FSDPModule):
                ...
                if (
                    ...
                    stack.enter_context(self.ddp_no_sync(self.model_parts))
            yield
    ...
    def _prepare_batch(
        ...
        has_doc_lens = "doc_lens" in batch and "max_doc_lens" in batch
        if has_doc_lens:
    ...
    def parallelize_and_init_model(
        ...
        *,
        ...
        # DDP wrapping requires initialized parameters.
        if eval_only:
    ...
    def reduce_send_recv(self, x: Optional[torch.Tensor] = None) -> torch.Tensor:
        """Average a final-stage scalar across data ranks and propagate it through the pipeline."""
        assert (
        ...
        # Match scalar propagation to the pipeline schedule's rank-completion order.
