...
class OLMoDDPTrainModule(TrainModule):
    def __init__(
        ...
        if (
            ...
            and not dp_config.only_allreduce_last_microbatch
        ):
            raise OLMoConfigurationError(
        ...
        if not self.eval_only:
            ...
            if self.use_reduce_scatter:
                sharded_params = (
                    self.optim.normal_params_with_sharded_optimizer_state()
                )
                unconfigured_params = set(sharded_params)
                for model_part in self.model_parts:
                    if not isinstance(
                        model_part, MultiGroupDistributedDataParallel
                    ):
                        raise RuntimeError(
                    ...
                    model_part.configure_reduce_scatter_params(
                        local_sharded_params
                    )
        ...
        for part_idx, model in enumerate(self.model_parts):
            for name, param in model.named_parameters():
                ...
                if reduced_grad_shard is not None:
                    entry["reduced_grad_shard"] = self._debug_tensor_payload(
                        reduced_grad_shard
                    )
                else:
                ...
                if any(
                    key in entry
                    for key in ("grad", "main_grad_fp32", "reduced_grad_shard")
                ):
    ...
    def _log_ep_no_sync_symm_buffer_summary(
        ...
        *,
        ...
        if torch.cuda.is_available():
            ...
            stat = lambda name: int(cuda_stats.get(name, 0))
        ...
        if unique_infos:
            ...
            for owner, nbytes in sorted(
                ...
                lines.append(
                    f"    {owner}: {self._gib(nbytes):.4f} GiB "
                    f"({owner_counts[owner]} tensors)"
                )
