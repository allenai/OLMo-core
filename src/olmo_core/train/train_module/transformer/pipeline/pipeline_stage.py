...
class CustomPipelineStage:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Retain stage inputs and outputs for communication and backward.
        self.fwd_cache: dict[
            int, tuple[torch.Tensor, torch.Tensor]
        ] = {}  # mb_idx -> (stage_input, stage_output)

        # Retain input gradients until they are sent to the previous stage.
        ...
        # The schedule returns detached final-stage losses to the train module.
    ...
    def forward_one_chunk(
        ...
        if self.is_last:
            ...
            logits, loss, ce_loss, z_loss = output
            # Backward expects a one-element tensor rather than a scalar loss.
            stage_output_with_graph = loss.unsqueeze(0)
            detached_lm_output = LMOutputWithLoss(
                logits=logits,  # The fused LM head does not materialize logits.
                loss=loss.detach(),
            ...
            stage_output_with_graph = output
    ...
    def backward_one_chunk(
        ...
        """
        Run the full backward pass once for a microbatch.

        The schedule sets ``last_backward`` to synchronize gradients across data-parallel
        groups after the final backward pass.
        """
        ...
        if self.is_last and not self.is_first:
            # Release the final-stage autograd graph after backward.
    ...
    def forward_maybe_with_nosync(self, last_forward: bool = False, *args, **kwargs):
        # Synchronization is controlled by the last backward, not the forward position.
    ...
    def backward_maybe_with_nosync(
        ...
        if isinstance(self.submod, DistributedDataParallel):
            if last_backward:
                # The no-sync forward requires explicit reducer preparation on the last backward.
            ...
            if last_backward:
                ...
                # Accumulate gradients without synchronizing intermediate microbatches.
                flip_requires_sync(self.submod, False)
    ...
    def clear_runtime_states(self) -> None:
        """Hook for releasing schedule-specific runtime state."""
        pass
