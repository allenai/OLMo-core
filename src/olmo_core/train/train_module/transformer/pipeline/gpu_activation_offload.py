...
class GPUActivationOffloader:
    """
    Offload saved-for-backward CUDA tensors to another GPU by pipeline group.

    All captured CUDA tensors, including parameters, remain owned by their group until
    :meth:`manual_release_group` is called.
    """
    ...
    def __init__(
        ...
        # Use a dedicated copy stream on each source device.
    ...
    def _pack_hook(self, group: str, t: torch.Tensor):
        # Leave non-CUDA tensors in place.
        if not t.is_cuda:
            return t

        key = id(t)
        if key in self._saved_tensors_by_group.get(group, {}):
            # A tensor can be saved more than once within a group.
            return key

        if t.device.index != torch.cuda.current_device():
            raise RuntimeError(
                f"Tensor device {t.device} does not match current device {torch.cuda.current_device()}"
            )
    ...
    def _unpack_hook(self, group: str, handle):
        ...
        # Reload must complete before autograd accesses a captured tensor.
        if t.device.index != torch.cuda.current_device():
            raise RuntimeError(
                f"Tensor device {t.device} does not match current device {torch.cuda.current_device()}"
            )
    ...
    def async_offload(self, group: str) -> Optional[torch.cuda.Event]:
        ...
        # Finish source-stream work before copying and replacing the saved tensors.
    ...
    def async_reload(self, group: str) -> Optional[torch.cuda.Event]:
        ...
        with torch.cuda.stream(self._stream):
            for key, t in _saved_tensors.items():
                if t.device.index == torch.cuda.current_device():
                    raise RuntimeError(
                        f"Tensor device {t.device} already on current device {torch.cuda.current_device()}"
                    )
