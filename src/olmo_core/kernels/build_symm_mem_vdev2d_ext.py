"""Build the NVSHMEM extension with CMake or setuptools CUDA device linking."""
...
def build_extension(*, inplace: bool, verbose: bool, force: bool, backend: str = "cmake") -> None:
    """Build the symmetric-memory extension with the selected build backend.

    :param inplace: Install the extension in the package directory.
    :param verbose: Enable verbose compiler output.
    :param force: Remove the selected backend's build directory before building.
    :param backend: ``cmake`` or ``setuptools``.
    """
...
def main() -> None:
    """Parse command-line options and build the symmetric-memory extension."""
