"""CUDA-backed NVSHMEM collectives and rowwise MoE dispatch primitives."""
...
def _load_cuda_extension():
    ...
    if auto_build:
        ...
        # Each node-local package directory has one builder. All ranks wait for installation
        ...
        if not distributed or get_local_rank() == 0:
            try:
                ...
                build_error = e
        if distributed:
            # Builders also reach the barrier on failure so their peers can exit the wait.
            barrier()
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Or set OLMO_SYMM_VDEV2D_AUTO_BUILD=1 to build on first use."
        ) from e
...
def all_to_all_vdev_2d_nblocks(
    ...
    *,
    ...
    """Exchange variable-length 2D splits with explicit alignment and launch-grid settings."""
    ext = _load_cuda_extension()
...
def all_to_all_vdev_2d_offset_nblocks(
    ...
    *,
    ...
    """Exchange variable-length 2D splits given input and output offsets."""
    ext = _load_cuda_extension()
...
def rowwise_dispatch_put(
    ...
    *,
    ...
    """Put routed input rows into peer buffers, optionally applying routing probabilities."""
    ext = _load_cuda_extension()
...
def rowwise_build_compact_route_records(
    ...
    *,
    ...
    """Build compact route records and wave offsets from rank, row and expert assignments."""
...
def rowwise_dispatch_put_compact(
    ...
    *,
    ...
    """Dispatch the input rows belonging to one wave of compact route records."""
    ext = _load_cuda_extension()
...
def rowwise_dispatch_put_compact_weighted(
    ...
    *,
    ...
    """Dispatch a compact routing wave with routing probabilities applied to its rows."""
    ext = _load_cuda_extension()
...
def rowwise_inverse_route_meta_put_compact(
    ...
    *,
    ...
    """Put inverse-route metadata into peer buffers using compact route records."""
    ext = _load_cuda_extension()
...
def rowwise_build_inverse_route_meta_from_global_records(
    ...
    *,
    ...
    """Build local inverse-route metadata from globally gathered compact records."""
    ext = _load_cuda_extension()
...
def rowwise_dispatch_put_scaled(
    ...
    *,
    ...
    """Quantize input rows to MXFP8 and dispatch their values and scales."""
    # Unwritten rows need initialization only when downstream operations consume padding.
...
def rowwise_combine_get(
    ...
    *,
    ...
    """Fetch expert output rows and combine them with optional routing probabilities."""
    ext = _load_cuda_extension()
...
def rowwise_combine_put(
    ...
    *,
    ...
    """Put expert output rows into their source ranks' gathered-route buffers."""
    ext = _load_cuda_extension()
...
def rowwise_reduce_gathered_routes(
    ...
    """Reduce gathered expert rows using routing probabilities and an optional rank mask."""
    ext = _load_cuda_extension()
...
def rowwise_reduce_gathered_routes_unweighted(
    ...
    """Sum gathered expert rows with an optional rank mask."""
    ext = _load_cuda_extension()
...
def rowwise_combine_get_scaled(
    ...
    *,
    ...
    """Fetch MXFP8 expert outputs, dequantize them and reduce their routed contributions."""
...
def rowwise_combine_get_fused(
    ...
    *,
    ...
    """Fetch and combine expert rows in one kernel, optionally saving gathered outputs."""
    ext = _load_cuda_extension()
...
def rowwise_gather_get(
    ...
    *,
    ...
    """Fetch peer expert rows without reducing their routed contributions."""
    ext = _load_cuda_extension()
...
def rowwise_dispatch_put_pair(
    ...
    *,
    ...
    """Dispatch quantized row values and scales with a shared routing map."""
    ext = _load_cuda_extension()
...
def rowwise_dispatch_put_scaled_weighted(
    ...
    *,
    ...
    """Weight, quantize and dispatch rows without a route-expanded high-precision tensor."""
    if zero_unwritten or os.getenv("OLMO_ROWWISE_FP8_DISPATCH_INIT_OUT", "0") == "1":
        out_q.zero_()
        out_scales.fill_(1.0)
...
def rowwise_gather_get_pair(
    ...
    *,
    ...
    """Fetch quantized row values and scales with a shared routing map."""
    ext = _load_cuda_extension()
