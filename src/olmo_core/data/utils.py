    ...
    if batch_size <= num_microbatch_instances:
        ...
        for key, value in batch.items():
            if isinstance(value, torch.Tensor):
                ...
                raise TypeError(f"unexpected item in batch: '{key}={value}'")
        return [
...
def iter_document_indices(
    data_path: PathOrStr,
    *,
    local_cache: PathOrStr | None = None,
    use_array_if_local: bool | None = None,
    ...
    """
    if (
        use_array_if_local is None
        and eos_token_id is not None
        and dtype is not None
        and not is_url(data_path)
    ):
...
def get_document_indices(
    data_path: PathOrStr, local_cache: PathOrStr | None = None
) -> list[tuple[int, int]]:
    """
...
def load_array_slice(
    ...
    start_idx: int,
    end_idx: int,
    dtype: type[np.uint8] | type[np.uint16] | type[np.uint32] | type[np.uint64] | type[np.bool_],
) -> np.ndarray:
    """
...
def bucket_documents(
    ...
    *,
    ...
    eos_token_id: int,
    dtype: type[np.uint8] | type[np.uint16] | type[np.uint32] | type[np.uint64],
    indices_dtype: type[np.uint8] | type[np.uint16] | type[np.uint32] | type[np.uint64] = np.uint32,
) -> tuple[int, int]:
    """
...
class SegmentTreeNode:
    ...
    children: tuple["SegmentTreeNode", "SegmentTreeNode"] | None = None
    ...
    def update(self, weight: int | None = None):
...
def pack_documents_into_instances(
    *paths: PathOrStr,
    ...
    dtype: type[np.uint8] | type[np.uint16] | type[np.uint32] | type[np.uint64],
    bos_token_id: int | None = None,
    indices_dtype: type[np.uint8] | type[np.uint16] | type[np.uint32] | type[np.uint64] = np.uint64,
    long_doc_strategy: LongDocStrategy = LongDocStrategy.truncate,
) -> tuple[list[list[int]], np.ndarray, int]:
    """
