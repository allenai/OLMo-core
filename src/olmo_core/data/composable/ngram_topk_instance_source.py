"""
Wrap a base :class:`InstanceSource` with per-position KN top-k fields.

For each instance, we add ``ngram_token_ids`` (S, K) and
``ngram_log_probs`` (S, K). The IDs are the top-k next-token candidates from
the Kneser-Ney-smoothed ngram table. The values are natural-log
full-vocabulary ngram probabilities for those candidates, used by the
product-of-experts training loss as a sparse logit bias.

The per-position probe uses
:class:`olmo_core.data.ngram_topk.NgramTopKSource`,
...
class NgramTopKInstanceSource(InstanceSource):
    """
    An :class:`InstanceSource` that wraps another one and augments each
    instance with per-position KN top-k product-of-experts fields.
    ...
    def __getitem__(self, idx: int) -> Instance:
        ...
        # For position i (whose ngram expert predicts input_ids[i+1]) the
        ...
        out = dict(inst)
        out["ngram_token_ids"] = ids  # (S, K) int32 / int64
        out["ngram_log_probs"] = values  # (S, K) float32
        return out
...
@dataclass
class NgramTopKInstanceSourceConfig(InstanceSourceConfig):
    """Config for :class:`NgramTopKInstanceSource`."""
    ...
    def build(self, work_dir: PathOrStr) -> NgramTopKInstanceSource:
        base = self.source.build(work_dir)
        return NgramTopKInstanceSource(
            base,
...
NgramTopKInstanceSource.Config = NgramTopKInstanceSourceConfig  # type: ignore[attr-defined]
