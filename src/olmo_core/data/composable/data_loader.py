    ...
    def get_instance(self, idx: int) -> Dict[str, Any]:
        ...
        for source in self.sources_for_this_epoch:
            ...
            if source_start_offset <= idx < source_end_offset:
                ...
                # KN top-k fields from NgramTopKInstanceSource. Pass through
                # preserving dtype. We use ``torch.as_tensor``
                # directly because this module's local ``as_tensor`` (composable
                # .utils.as_tensor) is hard-wired to cast non-bool numpy arrays
                # via ``astype(np.int_)``, which silently truncates negative
                # log-probs to sign-confused integers.
                if (s_ids := instance.get("ngram_token_ids")) is not None:
