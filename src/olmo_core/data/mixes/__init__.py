...
class DataMix(DataMixBase):
    ...
    OLMo_mix_0625_fixed = "OLMo-mix-0625-fixed"
    OLMo_midtrain_mix = "OLMo-midtrain-final-macroanneal"
    ...
    def build(self, base_dir: str, tokenizer: str) -> Tuple[List[str], List[str]]:
        ...
        elif self == DataMix.OLMo_mix_0625 or self == DataMix.OLMo_mix_0625_fixed or self == DataMix.OLMo_midtrain_mix:
            ...
            with mix_path.open() as f:
                for line_num, line in enumerate(f):
                    ...
                    if "{TOKENIZER}" in path:
                        # Standard case: replace placeholder with tokenizer
                        path = path.replace("{TOKENIZER}", tokenizer_id)
                    # If no {TOKENIZER}, use path as-is (already has correct tokenizer path or doesn't need one)
