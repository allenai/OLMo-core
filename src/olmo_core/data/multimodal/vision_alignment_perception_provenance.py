...
_BUILDER_NAME = "build_vision_alignment_perception_provenance"
_BUILDER_VERSION = 1
...
_FINEVISION_MATERIALIZER = "materialize_vision_alignment_finevision.py"
...
def _builder_script_path() -> Path:
    return Path(__file__).resolve().parents[3] / "scripts" / "data" / f"{_BUILDER_NAME}.py"


def _materializer_script_path() -> Path:
    return Path(__file__).resolve().parents[3] / "scripts" / "data" / _FINEVISION_MATERIALIZER
...
def validate_finevision_materialization(
    ...
    The implementation deliberately reproduces the materializer's output fingerprint rather
    than importing its implementation. The copied receipt pins that script's exact current
    bytes, while the source spec supplies the absolute paths whose shard, receipt, and
    ``dataset_info.json`` bytes are checked before an audited-alignment adapter can be built.
    ...
    :raises ValueError: If any receipt, code, path, byte, count, or fingerprint differs.
    """
    ...
    materializer_path = _materializer_script_path()
    if not materializer_path.is_file() or _sha256_file(materializer_path) != materializer_sha:
        raise ValueError("FineVision materializer bytes differ from the current code")
    ...
    cache_paths = [copied_path, external_manifest, complete_path, materializer_path]
...
class PerceptionProvenanceManifest:
    ...
    def validate_image_path_signatures(self) -> None:
        ...
        """
        for record in self.image_path_signatures:
...
def load_perception_provenance_manifest(
    ...
    *,
    ...
    the exhaustive default. ``require_complete=False`` is reserved for the builder's
    prepublication self-check before it writes the commit marker.
    """
    ...
    if (
        ...
        or _count(builder["version"], name="provenance builder.version", minimum=1)
        != _BUILDER_VERSION
    ):
        raise ValueError("Perception provenance builder identity differs")
    builder_sha = _sha256(builder["script_sha256"], name="provenance builder.script_sha256")
    builder_path = _builder_script_path()
    if not builder_path.is_file() or _sha256_file(builder_path) != builder_sha:
        raise ValueError("Perception provenance builder bytes differ from the current code")
    ...
    if (
        ...
        != VISION_ALIGNMENT_PERCEPTION_SOURCE_REGISTRY_VERSION
        or root["source_registry_sha256"] != vision_alignment_perception_source_registry_sha256()
        or root["source_implementation_inventory"]
        != vision_alignment_perception_implementation_inventory()
    ):
        raise ValueError("Perception source implementation identity differs")
