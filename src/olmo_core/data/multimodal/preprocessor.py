...
class MultimodalPreprocessor:
    ...
    def __call__(
        ...
        :returns: Dict with keys ``input_tokens``, ``loss_masks``, ``images``,
            ``image_masks``, ``pooled_patches_idx``. For text-only examples the
            three image fields are zero-sized arrays so a collator can still
            stack them.
        """
        cfg = self.cfg
        ...
        if image is not None:
            ...
            image_masks = mc_out.image_masks
            ...
            image_masks = np.zeros((0, 0), dtype=np.float32)
