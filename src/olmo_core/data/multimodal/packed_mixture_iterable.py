...
from .packing import REF_CURSOR_KEY, iter_dynamic_packs
...
class _CountingRefIter:
    """Wrap a ref iterator, exposing how many refs have been pulled from it.

    The packer consumes a *data-dependent* number of refs per pack, so the only way to
    know where a worker's stream stopped is to count as it goes. The count is what makes
    an O(1) resume possible: refs are cheap to regenerate (pure RNG arithmetic), while
    the examples they point at are not (image decode + preprocess).
    """
    ...
    def __iter__(self) -> "_CountingRefIter":
        return self
    ...
    def skip(self, n: int) -> None:
        """Advance ``n`` refs without loading the examples they point at."""
        for _ in range(n):
            next(self._it)
        self.count += n
...
class PackedMixtureIterableDataset(torch.utils.data.IterableDataset):
    ...
    def __init__(
        ...
        *,
        ...
        # Refs each worker fast-forwards past on resume. Skipping happens on the *ref*
        # stream (cheap RNG arithmetic) rather than by rebuilding batches, which is what
        # makes resume O(1) in wall-clock instead of O(resume_step).
    ...
    def _packed_stream(
        self, worker_id: int, num_workers: int, counter: "_CountingRefIter"
    ) -> Iterator[Dict[str, Any]]:
        # `counter` wraps the ref stream so __iter__ can stamp each pack with the number
        # of refs consumed so far. Skipping is done here, on refs, *before* the example
        # stream: the refs are regenerated by RNG (fast) and the images they point at are
        # never touched.
        if self.skip_refs:
            counter.skip(self.skip_refs)
        example_stream = self._example_stream(counter)
    ...
    def __iter__(self) -> Iterator[Dict[str, Any]]:
        ...
        packs_yielded = 0
        counter = _CountingRefIter(self._partitioned_ref_iter(worker_id, num_workers))
        for pack in self._packed_stream(worker_id, num_workers, counter):
            packs_yielded += 1
            if packs_yielded == 1:
                log.info(
                    "PackedMixtureIterableDataset worker=%d yielded first pack (skipped %d refs)",
                    worker_id,
                    self.skip_refs,
                )
            # Stamp the worker's ref cursor so the main process can persist it and resume
            # here next time instead of replaying the stream.
            pack[REF_CURSOR_KEY] = counter.count
            yield pack
