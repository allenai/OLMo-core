...
if TYPE_CHECKING:
    # olmo_core.nn imports this module while initializing.
...
class PipelineParallelConfig(Config):
    ...
    """
    Pipeline activation/gradient P2P backend. ``nccl`` uses two-sided torch.distributed
    P2P; ``nccl_rma`` and ``nccl_rma_ack`` use NCCL PutSignal/WaitSignal with their
    respective acknowledgment policies. ``nccl_device`` is not implemented.
    """
    ...
    def uses_separate_p2p_group(self) -> bool:
        """Whether activation and gradient transfers use a dedicated process group."""
    ...
    def build_p2p_process_group(self, device_mesh: DeviceMesh) -> Optional[dist.ProcessGroup]:
        """Build a dedicated NCCL P2P group, or return ``None`` to reuse the pipeline group."""
        self._validate_p2p_nccl_ctas()
        if self.p2p_backend == PipelineP2PBackend.nccl_device:
            raise OLMoConfigurationError(
                "p2p_backend='nccl_device' is reserved but not implemented yet"
            )
...
def draw_pipeline_timeline(
    ...
    """Plot per-rank pipeline actions, using two time units for full backward passes."""
    ...
    total_w = [sum(2 if is_bwd(cell) else 1 for cell in pp_order[r]) for r in ranks]
    assert all(
        w == total_w[0] for w in total_w
    ), f"Not all rows have the same total width: {total_w}"
    row_width = total_w[0]
    ...
    for iy, r in enumerate(ranks):
        ...
        y = y * 2
        x_offset = 0
        for op in pp_order[r]:
            if op is None:  # idle
                stage_id, kind, mb = None, None, None
            else:
                stage_id, kind, mb, _, _ = op
    ...
    # Limit labels to approximately 20 ticks and retain the unit grid.
...
def get_pipeline_bubble_stats(pp_order: Dict[int, List[Any]]) -> tuple[int, int, float]:
    """Return the number of idle slots, total slots and idle fraction across all ranks."""
    ranks = sorted(pp_order.keys())
    if not ranks:
        return 0, 0, 0.0
...
def format_pp_schedule_text(
    ...
    *,
    ...
    """Format schedule metadata, utilization statistics and per-rank action sequences."""
...
def debug_save_pp_schedule(
    ...
    """Save a schedule timeline and an optional text summary."""
    import matplotlib.pyplot as plt
...
class PipelineSchedule:
    ...
    def __init__(
        ...
        *,
        ...
        # Plotting requires matplotlib from the optional dev dependencies.
    ...
    def step(
        ...
        **kwargs,
        ...
        if forward_only:
            ...
            self.schedule_impl.reset_n_microbatches(input_ids.size(0))
        elif (
        ...
        step_output = self.schedule_impl.step(
            *args, target=target, forward_only=forward_only, **kwargs
        )
