...
def _fp32_post_grad_acc_hook(param: torch.Tensor):
    """Accumulate into the FP32 master gradient and release the parameter gradient."""
    g = param.grad
    if g is None:
        return
...
class OLMoDDPModel(olmo_core.nn.transformer.Transformer):
    """
    Transformer with multi-group data parallelism and optional expert parallelism.

    Uses :class:`~olmo_core.nn.ddp.block.OLMoDDPTransformerBlock` for both routed and
    shared-only feed-forward layers.
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.has_grad_accum_fp32_buffer = False
        self._compile_requested = False

        super().__init__(*args, **kwargs)
        self.ep_enabled = False
        self._ep_modules = []
        # Single-world symmetric-memory paths require identical allocation order
        # even when pipeline stages own different numbers of MoE blocks.
    ...
    def named_required_ddp_blocks(self) -> Iterator[tuple[str, OLMoDDPTransformerBlock]]:
        """Iterate over blocks, rejecting block types incompatible with OLMo DDP."""
        for block_key, block in self.blocks.items():
    ...
    def ep_no_sync_blocks(self) -> Iterator[OLMoDDPTransformerBlock]:
        """Iterate over routed blocks using OLMo symmetric memory."""
        for _, block in self.named_ep_no_sync_blocks():
            yield block

    def named_fp8_blocks(self) -> Iterator[tuple[str, object]]:
        """Iterate over keys and blocks with FP8 weights in attention or experts."""
        for block_key, block in self.blocks.items():
    ...
    def purge_cuda_events(self):
        """Release CUDA events so pipeline partitioning can deep-copy the model."""
        for layer in self.ddp_blocks():
            layer.purge_cuda_events()

    def install_cuda_events(self):
        """Install CUDA events after pipeline partitioning."""
        for layer in self.ddp_blocks():
    ...
    def count_ep_no_sync_blocks(self) -> int:
        """Return the number of routed blocks using OLMo symmetric memory."""
        return sum(1 for _ in self.ep_no_sync_blocks())

    def iter_ep_no_sync_symm_tensor_infos(self) -> Iterator[EpNoSyncSymmTensorInfo]:
        """Iterate over allocated symmetric tensors for buffer accounting."""
    ...
    def count_non_rowwise_ep_no_sync_blocks(self) -> int:
        """Return the number of symmetric-memory blocks using non-rowwise buffers."""
    ...
    def refresh_rowwise_fp8_cache(self) -> None:
        """Refresh each block's cached rowwise-quantized weights."""
        for block in self.ddp_blocks():
            block.refresh_rowwise_fp8_cache()

    def named_fp8_weight_stores(self) -> Iterator[tuple[str, object]]:
        """Iterate over fully qualified names and expert or attention FP8 stores."""
    ...
    def prewarm_deepep_v2_runtimes(
        ...
        *,
        ...
        """Prepare DeepEP buffers for the maximum local microbatch token count."""
    ...
    def apply_ddp(
        ...
        """Reject composable DDP; this model requires :meth:`apply_dp`."""
        raise OLMoConfigurationError(
            "OLMoDDPModel requires multi-group DDP; use apply_dp instead of apply_ddp."
        )
    ...
    def apply_dp(
        ...
        """Wrap the model in multi-group DDP with separate dense and expert reductions.

        Parameters use BF16; gradient accumulation and reduction precision are independently
        configurable. ``root_module`` may include a vision encoder and connector around this LM.
        """
        ...
        # Expert sharding is marked by apply_ep().
        self._ep_modules = [m for m in module.modules() if getattr(m, "_ep_sharded", False)]
        ep_sharded_params = set()
        for m in self._ep_modules:
            for n, p in m.named_parameters():
                ep_sharded_params.add(p)

        # Parameters are BF16; optimizer master parameters and gradient buffers
        # have separate precision policies.
    ...
    def prepare_experts_for_ddp(self, world_mesh: DeviceMesh):
        """No-op: :meth:`apply_ep` performs expert sharding for this model."""
    ...
    def init_weights(
        ...
        *,
        ...
        # Match the parameter dtype used by apply_dp().
    ...
    def attach_fp32_accum(self):
        """Install FP32 gradient accumulation hooks on trainable parameters."""
    ...
    def set_main_grads_to_none(self):
        """Release FP32 master gradients, including gradients in FP8 weight stores."""
        for p in self.parameters():
            if not p.requires_grad:
                continue
    ...
    def forward_tbo(
        ...
        **kwargs,
        ...
        """Run the expert-parallel model using two-batch overlap."""
    ...
    def forward_embed(self, input_ids: torch.Tensor) -> torch.Tensor:
        """Embed input tokens or pass through inputs on stages without embeddings."""
        h = self.embeddings(input_ids) if self.embeddings is not None else input_ids
        if self.embeddings is not None and self.embed_scale is not None:
            h = h * self.embed_scale
        if self.embedding_norm is not None:
