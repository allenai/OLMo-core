...
class OLMoDDPTransformerBlockConfig(TransformerBlockConfig):
    ...
    The norm placement must match the source architecture when loading a checkpoint.
    """
    ...
    def build(
        ...
        *,
        ...
        if kwargs.pop("masked_dropout"):
            raise OLMoConfigurationError(
                "'masked_dropout' is not supported by OLMoDDPTransformerBlock"
            )
...
class OLMoDDPTransformerBlock(olmo_core.nn.transformer.block.TransformerBlockBase):
    ...
    def invalidate_rowwise_fp8_cache(self) -> None:
        """Invalidate cached rowwise-quantized weights after an update."""
        _invalidate_rowwise_fp8_cache(self)

    def refresh_rowwise_fp8_cache(self) -> None:
        """Refresh rowwise-quantized expert weights."""
    ...
    def named_fp8_weight_stores(self) -> Iterator[tuple[str, FP8WeightStore]]:
        """Iterate over names and FP8 stores for routed and shared experts."""
        if self.routed_experts is not None:
    ...
    def release_fp8_weight_anchor_storage(self) -> None:
        """Release parameter-anchor storage when FP8 weight stores own the weights."""
        if self.routed_experts is not None:
    ...
    def sync_fp8_weight_store_grads_from_anchor(self) -> None:
        """Transfer parameter-anchor gradients into FP8 weight stores."""
        if self.routed_experts is not None:
    ...
    def purge_cuda_events(self):
        """Release CUDA events so the block can be deep-copied."""
    ...
    def compute_metrics(
        ...
        """Collect router and expert-dispatch metrics, optionally resetting their accumulators."""
        from olmo_core.train.common import ReduceType
    ...
    def reset_metrics(self):
        """Reset router and expert-dispatch metric accumulators."""
        if self.routed_experts_router:
    ...
    def get_dense_stream(self, for_x1=False) -> torch.cuda.Stream:
        """Return the dense-compute stream for the requested TBO microbatch."""
        if for_x1:
    ...
    def combined_forward_rowwise_nvshmem_tbo(
        ...
        **kwargs,
        ...
        """Run a rowwise two-batch-overlap stage for the model's TBO forward path."""
