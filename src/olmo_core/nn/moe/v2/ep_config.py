...
class ExpertParallelPath(StrEnum):
    """Communication backend used to dispatch tokens to routed experts."""

    # Synchronized all-to-all EP. Does not require symmetric memory.
    sync_1d = "sync_1d"
    # Symmetric-memory all-to-all with explicit expert permutation and unpermutation.
    no_sync_1d = "no_sync_1d"
    # OLMo rowwise NVSHMEM with static buffers and device-side routing metadata.
    rowwise_nvshmem = "rowwise_nvshmem"
    # Experimental NVSHMEM dispatch, expert GEMM, and combine overlap within a batch.
...
class ExpertParallelSchedule(StrEnum):
    """Execution schedule for expert-parallel blocks."""
...
class DeepEPConfig(Config):
    """Buffer allocation and communication settings for the DeepEP backend."""
    ...
    # Received rows per expert must be packed without padding (alignment 1).
    ...
    def validate(self) -> None:
        """Normalize weighting and validate communication resource counts."""
...
class ExpertParallelConfig(Config):
    """Expert dispatch capacity, buffer sharing, and backend-specific settings."""
    ...
    def validate(self) -> None:
        """Normalize settings and reject incompatible backend and scheduling options."""
        ...
        # Block/train dispatch does not support the two-batch-overlap schedule.
    ...
    def no_sync(self) -> bool:
        """Whether the backend avoids synchronized all-to-all dispatch."""
        return self.path != ExpertParallelPath.sync_1d

    @property
    def is_no_sync_1d(self) -> bool:
        """Whether the backend uses symmetric-memory all-to-all dispatch."""
        return self.path == ExpertParallelPath.no_sync_1d

    @property
    def is_rowwise(self) -> bool:
        """Whether the backend uses rowwise NVSHMEM dispatch."""
        return self.path in {
    ...
    def is_deepep(self) -> bool:
        """Whether the backend uses DeepEP communication buffers."""
        return self.path == ExpertParallelPath.deepep_v2

    @property
    def uses_olmo_symm(self) -> bool:
        """Whether dispatch requires OLMo-managed symmetric memory."""
        return self.no_sync and not self.is_deepep

    @property
    def uses_rowwise_buffers(self) -> bool:
        """Whether dispatch requires rowwise symmetric buffers."""
        return self.is_rowwise

    @property
    def rowwise_transport(self) -> Optional[str]:
        """Return the rowwise transport, or ``None`` for non-rowwise backends."""
        if self.path in {
            ExpertParallelPath.rowwise_nvshmem,
            ExpertParallelPath.rowwise_wave,
        }:
