...
def _rowwise_buffer_debug_print(message: str) -> None:
    if not _rowwise_buffer_debug_enabled():
        return
    rank = dist.get_rank() if dist.is_available() and dist.is_initialized() else "?"
    print(
        (
            "[OLMO_ROWWISE_BUFFER_DEBUG] "
...
def _alloc_ep_symm_tensor(
    *,
    ...
    if olmo_symm_mem.is_enabled():
        ...
        _rowwise_buffer_debug_print(f"symm_alloc:exit shape={shape} dtype={dtype} device={device}")
        return symm_tensor
...
class EpNoSyncSymmTensorInfo:
    """Description of a symmetric-memory allocation used by expert parallelism."""
...
def get_ep_no_sync_group_name(block: "OLMoDDPTransformerBlock") -> str:
    """Return the initialized symmetric-memory process-group name for a block."""
...
def ep_no_sync_slot_for_lane(block: "OLMoDDPTransformerBlock", lane_id: int) -> int:
    """Map an overlap lane to its symmetric-buffer slot."""
    if lane_id < 0:
...
def resolve_ep_no_sync_chunk_reorder_backend() -> str:
    """Resolve the chunk-reordering backend from the process environment."""
...
def get_or_init_ep_no_sync_symm_tensor(
    ...
    *,
    ...
    """Reuse or allocate a named symmetric tensor with the requested shape and dtype."""
    if _symm_mem is None and not olmo_symm_mem.is_enabled():
        raise RuntimeError("EP no-sync requires torch.distributed._symmetric_memory")
...
def acquire_ep_no_sync_dispatch_out_lease(
    ...
    *,
    ...
    """Acquire a lifetime-managed dispatch-output buffer."""
...
def acquire_ep_no_sync_fp8_dispatch_out_lease(
    ...
    *,
    ...
    """Acquire a lifetime-managed FP8 dispatch-output buffer."""
...
def acquire_ep_no_sync_combine_out_lease(
    ...
    *,
    ...
    """Acquire a lifetime-managed token-combination output buffer."""
...
def acquire_ep_no_sync_combine_gather_lease(
    ...
    *,
    ...
    """Acquire a lifetime-managed buffer for gathered expert routes."""
...
def acquire_ep_no_sync_fp8_combine_gather_lease(
    ...
    *,
    ...
    """Acquire a lifetime-managed FP8 buffer for gathered expert routes."""
...
def prewarm_ep_no_sync_rowwise_dispatch_out_leases(
    ...
    *,
    ...
    """Allocate rowwise dispatch-output leases before compiled execution."""
...
def prewarm_ep_no_sync_rowwise_lifetime_leases(
    ...
    *,
    ...
    """Allocate rowwise communication leases before compiled execution."""
    num_slots = _rowwise_lifetime_lease_prewarm_slots() if num_slots is None else num_slots
    if num_slots == 0:
        return
...
def get_ep_no_sync_rowwise_fp8_buffers(
    ...
    *,
    ...
    """Get or allocate rowwise FP8 communication buffers for a block."""
    ...
    if not need_dispatch_out:
        ...
        dispatch_out_q = dispatch_out_lease.tensor("dispatch_out_q")  # type: ignore[union-attr]
        dispatch_out_scales = dispatch_out_lease.tensor("dispatch_out_scales")  # type: ignore[union-attr]
...
def get_cached_ep_no_sync_buffers(
    ...
    *,
    ...
    """Return existing no-sync communication buffers matching the requested geometry."""
...
def get_cached_ep_no_sync_rowwise_fp8_buffers(
    ...
    *,
    ...
    """Return existing rowwise FP8 buffers matching the requested geometry."""
    if getattr(block, "_ep_no_sync_shared_pool", None) is not None:
...
def use_ep_no_sync_rowwise_symm_dispatch_in(block: "OLMoDDPTransformerBlock") -> bool:
    """Resolve whether rowwise dispatch inputs use symmetric memory."""
...
def use_ep_no_sync_rowwise_symm_combine_out(block: "OLMoDDPTransformerBlock") -> bool:
    """Resolve whether rowwise combination outputs use symmetric memory."""
    return _resolve_rowwise_symm_option(
        block,
        attr_name="ep.rowwise_symm_combine_out",
        env_name="OLMO_MOE_ROWWISE_SYMM_COMBINE_OUT",
...
def use_ep_no_sync_rowwise_symm_combine_gather(block: "OLMoDDPTransformerBlock") -> bool:
    """Resolve whether gathered expert routes use symmetric memory."""
    return _resolve_rowwise_symm_option(
        block,
        attr_name="ep.rowwise_symm_combine_gather",
        env_name="OLMO_MOE_ROWWISE_SYMM_COMBINE_GATHER",
...
def acquire_ep_no_sync_rowwise_lifetime_leases(
    ...
    *,
    ...
    """Acquire the requested rowwise buffers with autograd-safe lifetimes."""
...
def get_ep_no_sync_buffers(
    ...
    *,
    ...
    """Get or allocate no-sync communication buffers for the requested slot."""
    ...
    if need_dispatch_out:
        ...
        if lease_dispatch_out:
            ...
            dispatch_out = dispatch_out_lease.tensor("dispatch_out")  # type: ignore[union-attr]
    ...
    if need_combine_out:
        ...
        if lease_combine_out:
            ...
            combine_out = combine_out_lease.tensor("combine_out")  # type: ignore[union-attr]
    ...
    if need_combine_gather:
        ...
        if lease_combine_gather:
            ...
            combine_gather = combine_gather_lease.tensor("combine_gather")  # type: ignore[union-attr]
    ...
    _rowwise_buffer_debug_print(
        f"get_buffers:exit block={block.block_idx} slot={resolved_slot_idx}"
    )
    buffers = _NoSyncSymmBuffers(
        dispatch_in=dispatch_in,
        dispatch_in_rank_splits=dispatch_in_rank_splits,
...
def iter_ep_no_sync_symm_tensors(block: "OLMoDDPTransformerBlock") -> Iterator[torch.Tensor]:
    """Iterate the symmetric tensors owned or shared by a block."""
...
def iter_ep_no_sync_symm_tensor_infos(
    ...
    """Iterate allocation descriptions for the symmetric tensors used by a block."""
...
def compute_ep_no_sync_rank_capacity(block: "OLMoDDPTransformerBlock", num_out_tokens: int) -> int:
    ...
    """Compute destination-rank row capacity from token count and routing fan-out."""
    return max(
        1,
