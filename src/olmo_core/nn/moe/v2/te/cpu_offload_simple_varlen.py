...
Variant that supports variable number of tensors per group/layer with
...
class GroupCommitFunction(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, tensor, cpu_offload_handler: "CpuOffloadHandler"):
    ...
    def backward(ctx, *grad_output):
        cpu_offload_handler: CpuOffloadHandler = ctx.cpu_offload_handler
...
class CpuOffloadHandler:
        ...
        with torch.cuda.stream(self.h2d_stream):  # type: ignore
            ...
            for tensor_tag, state in entries:
                if isinstance(state, tuple):
                    ...
                    # Already on GPU (not offloaded); nothing to do
                    pass
    ...
    @staticmethod
    def reload(state, copy_buffer: Optional[torch.Tensor] = None):
        ...
        # Shapes/dtypes should match thanks to _ensure_bank_for_group
...
def tensor_need_offloading_checker_activations(tensor):
    # In the simplified version we offload everything saved-for-backward.
    # You can switch to hasattr(tensor, "activation_offloading") for opt-in.
    return True
