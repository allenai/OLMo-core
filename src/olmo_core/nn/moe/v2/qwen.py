...
def _require_linear_attention_kwargs(
    *,
    ...
    assert (
        ...
        and linear_num_value_heads is not None
        and linear_key_head_dim is not None
        and linear_value_head_dim is not None
        and linear_conv_kernel_dim is not None
    )
    return (
...
) -> tuple[TransformerBlockConfig | dict[str, TransformerBlockConfig], list[str] | None]:
    if len(set(layer_types)) == 1:
        return blocks[layer_types[0]], None
    return dict(blocks), list(layer_types)
    ...
    return OLMoDDPTransformerBlockConfig(
        sequence_mixer=AttentionConfig(
            ...
            rope=RoPEConfig(
                ...
                # core's RoPEConfig takes a partial-rotary factor instead of an absolute dim:
                # int(head_dim * factor) leading dims are rotated, the rest pass through.
                partial_rotary_factor=attention_rotary_dim / attention_head_dim,
                full_precision=True,
            ),
            qk_norm=layer_norm,
