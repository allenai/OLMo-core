...
def reset_ep_no_sync_rowwise_metrics(block: OLMoDDPTransformerBlock) -> None:
    """Reset accumulated rowwise dispatch metrics."""
...
def add_ep_no_sync_rowwise_metrics(
    ...
    """Add rowwise dispatch metrics to the block metric accumulator."""
    if (
...
def should_accumulate_ep_no_sync_rowwise_metrics(
    ...
    """Whether this forward should contribute rowwise dispatch metrics."""
...
def accumulate_ep_no_sync_rowwise_metrics(
    ...
    *,
    ...
    """Accumulate rowwise capacity and drop metrics without retaining their autograd graphs."""
    if rank_capacity <= 0:
        return

    drop_sum = drop_token_cnt.detach().to(dtype=torch.float32)
    if isinstance(num_out_tokens, int):
        # Scalar token counts use the same device and shape as the drop counter.
