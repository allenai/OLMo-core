...
def record_named_saved_activation(tensor: torch.Tensor, name: str) -> None:
    """Record a tensor's diagnostic name when saved-activation tracing is active."""
...
def maybe_dump_ep_no_sync_saved_activations(
    ...
    *,
    ...
    """Run one no-sync forward with saved-activation tracing when the diagnostic is enabled.

    Returns the diagnostic forward's output, or ``None`` when no diagnostic is requested.
    """
