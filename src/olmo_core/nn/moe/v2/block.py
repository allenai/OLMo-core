...
class MoEFusedV2TransformerBlockConfig(TransformerBlockConfig):
    ...
    def flops_per_seq(self, d_model: int, seqlen: int) -> int:
        ...
        from olmo_core.nn.transformer.flops import (
...
class MoEFusedV2TransformerBlock(olmo_core.nn.transformer.block.TransformerBlockBase):
    ...
    def apply_compile(self):
        ...
        self._permute_routed_experts_unpermute = torch.compile(
    ...
    def router_forward(
        ...
        return router(local_x, scores_only, loss_div_factor=loss_div_factor)  # scalar
    ...
    def async_copy_to_cpu(self, gpu_buf):
    ...
    def combined_forward_no_ep(
        ...
        **kwargs,
        ...
        # + attention norm
        # + residual connection
        attn_res_out: torch.Tensor = block_inp + self.attention_norm(
            self.attention(block_inp, **kwargs)
        )
        ...
        local_batch_size_per_global_routed_expert_cpu, copy_stream, dtoh_event = async_copy_to_cpu(
            local_batch_size_per_global_routed_expert, event=self._dtoh_event
        )
        ...
        if torch.is_grad_enabled():
            with nvtx.annotate("attach_auxiliary_loss", color="blue"):
                if routed_expert_router_aux_loss is not None:
                    ...
                    attn_res_out = attach_auxiliary_loss(
                        attn_res_out, routed_expert_router_aux_loss
                    )
        ...
        if self.shared_experts is not None:
            ...
            wait_stream_no_compile(
                this_stream=self.get_dense_stream(), other_stream=torch.cuda.current_stream()
            )
        ...
        if self.shared_experts is not None:
            ...
            shared_out_factor = self.shared_experts.num_experts / (
                self.routed_experts_router.top_k + self.shared_experts.num_experts
            )
            routed_out_factor = self.routed_experts_router.top_k / (
                self.routed_experts_router.top_k + self.shared_experts.num_experts
            )
        ...
        with nvtx.annotate("Sync token count", color="green"):
            with torch.no_grad():
                ...
                (
                    ...
                    copy_stream,
                    dtoh_event,
                ) = async_copy_to_cpu(parallel_batch_size_per_local_expert, event=self._dtoh_event)
    ...
    def _permute_routed_experts_unpermute(
        self, global_x, global_x_local_expert_indices, parallel_batch_size_per_local_expert_cpu
    ):
        assert self.routed_experts is not None
        ...
        **kwargs,
        ...
        with nvtx.annotate("TBO-1", color="orange"):
            if x1_is_fresh:
                ...
                reversed_local_x_permutation_mapping1 = x1_ctx[
                    "reversed_local_x_permutation_mapping1"
                ]
                local_x_global_routed_expert_weights1 = x1_ctx[
                    "local_x_global_routed_expert_weights1"
                ]
                ...
                if last_block.shared_experts is not None:
                    ...
                    shared_out_factor1 = last_block.shared_experts.num_experts / (
                        last_block.routed_experts_router.top_k
                        + last_block.shared_experts.num_experts
                    )
                    routed_out_factor1 = last_block.routed_experts_router.top_k / (
                        last_block.routed_experts_router.top_k
                        + last_block.shared_experts.num_experts
                    )
            ...
            if torch.is_grad_enabled():  # only when grad enabled
                with nvtx.annotate("attach_auxiliary_loss", color="blue"):
                    if routed_expert_router_aux_loss1 is not None:
                        attn_res_out1 = attach_auxiliary_loss(
                            attn_res_out1, routed_expert_router_aux_loss1
                        )
            ...
            with nvtx.annotate("Permute local tokens", color="green"):
                ...
                num_out_tokens1 = (
                    routing_map1.size(0) * self.routed_experts_router.top_k
                )  # dropless
