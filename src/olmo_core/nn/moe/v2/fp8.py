...
class MoERowwiseFP8ScaleMode(StrEnum):
    """Exponent rounding used to quantize rowwise MXFP8 scales."""

    floor = "floor"  # OCP-style conversion
...
class MoERowwiseFP8Config(Config):
    """Rowwise MXFP8 storage, accumulation, and recomputation settings."""
    ...
    def validate(self) -> None:
        """Validate block geometry and the process-wide MXFP8 scale mode."""
    ...
    def assert_runtime_supported(self) -> None:
        """Require CUDA hardware and grouped-matrix operators compatible with MXFP8."""
        self.validate()
...
def normalize_rowwise_fp8_config(
    ...
    """Resolve a rowwise FP8 config instance, mapping, or disabled configuration."""
    if value is None:
        return None
...
def reset_shared_rowwise_fp8_cache(block: OLMoDDPTransformerBlock) -> None:
    """Invalidate cached shared-expert projections and their weight versions."""
...
def invalidate_rowwise_fp8_cache(block: OLMoDDPTransformerBlock) -> None:
    """Invalidate routed- and shared-expert quantized projection caches."""
...
def refresh_shared_rowwise_fp8_cache(block: OLMoDDPTransformerBlock) -> None:
    """Refresh cached shared-expert projections from the current weights."""
...
def refresh_rowwise_fp8_cache(block: OLMoDDPTransformerBlock) -> None:
    """Refresh routed- and shared-expert quantized projection caches."""
...
def shared_experts_forward1_rowwise_fp8(
    ...
    *,
    ...
    """Compute the shared-expert up/gate projection using rowwise FP8."""
...
def shared_experts_forward_rowwise_fp8(
    ...
    *,
    ...
    """Compute shared-expert outputs using rowwise FP8 projections."""
    assert block.shared_experts is not None
    _require_swiglu_shared_experts(block.shared_experts)
    if x.ndim != 3:
        raise RuntimeError(
...
def shared_experts_forward2_rowwise_fp8(
    ...
    *,
    ...
    """Compute the shared-expert down projection using rowwise FP8."""
    assert block.shared_experts is not None
