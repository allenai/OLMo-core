...
class MoERouterConfigV2(Config):
    ...
    """Multiply selected expert weights by ``top_k``."""
    expert_weight_scale: Optional[float] = None
    original_top_k: Optional[int] = None
    """Reference top-k for scaling weights when the routing fan-out changes."""
    use_recompute_fp32_cast: bool = False
    """Recompute the fp32 router-input cast in backward to reduce saved activations."""
...
class MoERouterV2(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # Keep mutable metrics outside module buffers so distributed wrappers do not move
        # or reduce them and compiled forwards can update their storage in place.
    ...
    def forward(
        ...
        *,
        ...
        if self.gating_function in (
            ...
            # Keep the load-balancing normalization finite when all sigmoid scores underflow.
        ...
        # The one-forward recomputation cache is incompatible with pipeline parallelism.
