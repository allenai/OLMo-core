...
class _RowwiseFP8DispatchExpertsCombineAutograd(torch.autograd.Function):
    ...
    def forward(  # type: ignore[override]
        ...
        if bool(recompute_swiglu):
            # Quantize immediately after projection while up_gate is cache-resident.
    ...
    def backward(ctx, grad_out: torch.Tensor):  # type: ignore[override]
        ...
        # Up/gate dgrad needs an FP8 LHS; wgrad uses the bf16 SwiGLU gradient directly
        # to avoid a quantize/dequantize round trip.
        _rowwise_debug_op_enter(
