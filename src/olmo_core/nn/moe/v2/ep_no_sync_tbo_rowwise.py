...
def ep_no_sync_rowwise_tbo_stage_a(
    ...
    **kwargs,
    ...
    """Compute attention and routing metadata for one overlap lane."""
    self = block
...
def ep_no_sync_rowwise_tbo_stage_d_launch(
    ...
    """Launch rowwise token dispatch on the communication stream."""
    self = block
...
def ep_no_sync_rowwise_tbo_stage_shared_launch(
    ...
    """Launch shared experts once for an overlap lane."""
    self = block
...
def ep_no_sync_rowwise_tbo_stage_e(
    ...
    """Wait for dispatch and execute routed experts for one lane."""
    self = block
    assert self.routed_experts is not None
...
def ep_no_sync_rowwise_tbo_stage_c_launch(
    ...
    """Launch rowwise combination of completed expert outputs."""
    del block
...
def ep_no_sync_rowwise_tbo_stage_tail(
    ...
    """Wait for communication and complete the block residual path."""
    self = block
    a_state = pending_ctx.a_state
...
def combined_forward_ep_no_sync_tbo_rowwise(
    ...
    **kwargs,
    ...
    """Interleave two rowwise expert-parallel lanes and return the pending lane context."""
    self = block
    if x1_is_fresh:
        pending_prev = None
    else:
