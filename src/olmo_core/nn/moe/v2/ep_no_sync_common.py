...
def padded_local_expert_splits_for_capacity(
    ...
    *,
    ...
    """Pad local expert group sizes to the allocated row capacity."""
...
def rowwise_stage_debug_enabled() -> bool:
    """Whether rowwise stage diagnostics are enabled for this rank."""
...
def rowwise_stage_debug_print(label: str, **fields: object) -> None:
    """Print a stage diagnostic when enabled for this rank."""
...
def rowwise_stage_debug_sync(label: str, device: torch.device | None = None) -> None:
    """Synchronize the device when rowwise stage synchronization checks are enabled."""
...
def build_keep_reorder(
    ...
    """Build the token reorder map for retained expert routes."""
...
def restore_drop_unpermute_1d(
    ...
    *,
    ...
    """Restore dropped routes and unpermute expert outputs into token order."""
    self = block
