...
class RoutedExperts(nn.Module):
    ...
    def forward(self, x: torch.Tensor, batch_size_per_expert: torch.Tensor) -> torch.Tensor:
        ...
        batch_size_per_expert_tensor = batch_size_per_expert.to(
            dtype=torch.int64
        )  # NOTE: int64 required for grouped_gemm
        ...
        if self.num_local_experts == 1 and use_matmuls:
            ...
            up_gate = gmm_no_compile(
                x, w_up_gate, batch_size_per_expert_tensor, trans_b=True
            )  # -> (BS, 2H)
        ...
        if self.num_local_experts == 1 and use_matmuls:
            ...
            down = gmm_no_compile(
                h, w_down, batch_size_per_expert_tensor, trans_b=False
            )  # -> (BS, H)
    ...
    def act_and_down(
        ...
        down = gmm_no_compile(
            h, self.w_down, batch_size_per_expert_tensor, trans_b=False
        )  # -> (BS, H)
        return down
