...
class KdaFwdStateKernel:
    ...
    def _setup_attributes(self):
        ...
        self.vp_smem_layout = sm100_utils.make_smem_layout_b(mma_oi, self.tile_oi, self.io_dtype, 1)
        ...
        self.num_simt_load_bytes = cute.size_in_bytes(
    ...
    def kda_cute_fwd(
        self,
        tma_qg: cute.CopyAtom,
        mQG: cute.Tensor,
        tma_w: cute.CopyAtom,
        mW: cute.Tensor,
        tma_kgt: cute.CopyAtom,
        mKGT: cute.Tensor,
        tma_aqk: cute.CopyAtom,
        mAqk: cute.Tensor,
        tma_u: cute.CopyAtom,
        mU: cute.Tensor,
        ...
        sQG = storage.smem_qg.get_tensor(
            self.qg_smem_layout.outer, swizzle=self.qg_smem_layout.inner
        )
        ...
        sAqk = storage.smem_aqk.get_tensor(
            self.aqk_smem_layout.outer, swizzle=self.aqk_smem_layout.inner
        )
        ...
        sVp = storage.smem_vp.get_tensor(
            self.vp_smem_layout.outer, swizzle=self.vp_smem_layout.inner
        )
        sVp_epi = storage.smem_vp.get_tensor(
            self.vp_epi_layout.outer, swizzle=self.vp_epi_layout.inner
        )
        sVpd = storage.smem_vpd.get_tensor(
            self.vpd_smem_layout.outer, swizzle=self.vpd_smem_layout.inner
        )
        sVpd_epi = storage.smem_vpd.get_tensor(
            self.vp_epi_layout.outer, swizzle=self.vp_epi_layout.inner
        )
        ...
        def make_mma_to_simt_pipe(ptr, consumer_threads):
            return pipeline.PipelineUmmaAsync.create(
                num_stages=1,
                producer_group=pipeline.CooperativeGroup(pipeline.Agent.Thread),
                consumer_group=pipeline.CooperativeGroup(pipeline.Agent.Thread, consumer_threads),
                barrier_storage=ptr,
                defer_sync=True,
            )
        ...
        if warp_idx == self.tma_warp_id:
            ...
            for c in cutlass.range(NT, unroll=1):
                ...
                cute.copy(
                    tma_kgt, tKGTg[None, c], tKGTs[None, mmain_producer.index], tma_bar_ptr=bar
                )
                cute.copy(
                    tma_aqk, tAqkg[None, c], tAqks[None, mmain_producer.index], tma_bar_ptr=bar
                )
                ...
                cute.copy(
                    tma_gd, tGdg[None, c], tGds[None, simtin_producer.index], tma_bar_ptr=sbar
                )
        ...
        # ==========================================================================
            ...
            for c in cutlass.range(NT, unroll=1):
                ...
                simtin_pipe.consumer_release(simtin_mma_consumer, pipeline.PipelineOp.TCGen05Mma)
        ...
        # ==========================================================================
            ...
            tRS_rVp = cute.make_rmem_tensor(cute.slice_(tRS_sVp.shape, (None, None, None, 0)), io)
            ...
            tDHrGd = cute.make_rmem_tensor(cute.slice_(tDHsGd.shape, (None, None, None, 0)), f32)
            ...
            for c in cutlass.range(NT, unroll=1):
                ...
                simtin_pipe.consumer_release(simtin_consumer, pipeline.PipelineOp.AsyncThread)
        ...
        # ==========================================================================
            ...
            tRS_rO = cute.make_rmem_tensor(cute.slice_(tRS_sO.shape, (None, None, None, 0)), io)
            ...
            for c in cutlass.range(NT, unroll=1):
                ...
                for i in cutlass.range(cute.size(tTR_rOH), unroll_full=True, vectorize=True):
...
def kda_cute(
    ...
    o, ht, _, _, _ = kda_cute_fwd_with_residuals(q, k, v, g, beta, h0, scale, chunk_size)
    return o, ht


def kda_cute_fwd_with_residuals(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    g: torch.Tensor,
    ...
    """cumsum + intra are fla's (disable_recompute=True so intra stores qg); scan+o is cute.

    Returns (o, ht, g2, Aqk, Akk) — the backward's residuals, same rounding as fla's own
    forward since every saved tensor comes from fla's kernels.
    """
    ...
    g2 = chunk_local_cumsum(g=g, scale=FLA_RCP_LN2, chunk_size=chunk_size)
    ...
    o, ht = kda_cute_fwd_call(qg, kg, w, u, Aqk, g2, h0, float(scale))
    return o, ht, g2, Aqk, Akk
