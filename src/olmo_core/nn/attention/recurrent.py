...
class GatedDeltaNet2(SequenceMixer):
    """
    The layer implementation for `Gated Delta Networks 2 <https://github.com/NVlabs/GatedDeltaNet-2>`_.

    Modified from: https://github.com/fla-org/flash-linear-attention/blob/cbb0a72efb55c18ca0ef4f298298317573ad2cb3/fla/layers/gdn2.py

    GDN2 decouples the erase and write operations of the delta rule with channel-wise
    gates: a key-side erase gate ``b``, a value-side write gate ``w``, and a channel-wise
    decay ``g`` (low-rank projected), replacing GDN v1's per-head scalar beta and decay.
    ...
    :param allow_neg_eigval: Allow negative eigenvalues. Default: ``False``. If set to ``True``, the
        erase gate ``b`` will be multiplied by 2. See reference: `Unlocking State-Tracking in Linear
        RNNs Through Negative Eigenvalues <https://arxiv.org/abs/2411.12537>`_.
    :param conv_size: The kernel size of the short convolution. Default: 4.
    :param conv_bias: Whether to use bias in the short convolution. Default: ``False``.
    :param norm_eps: The epsilon value for the normalization layer. Default: 1e-5.
    :param dtype: The default data type to use for parameters.
    ...
    def __init__(
        ...
        *,
        ...
        self.w_k = nn.Linear(d_model, self.key_dim, bias=False, dtype=dtype, device=init_device)
        self.w_v = nn.Linear(d_model, self.value_dim, bias=False, dtype=dtype, device=init_device)
        # Low-rank channel-wise decay projection.
        ...
        self.o_norm = FusedRMSNormGated(
            self.head_v_dim, eps=norm_eps, activation="sigmoid", device=init_device
        )  # type: ignore
        self.w_out = nn.Linear(self.value_dim, d_model, bias=False, dtype=dtype, device=init_device)
    ...
    def forward(
        ...
        **kwargs,
        ...
        """
        Apply GDN2 sequence mixing to the input.
        ...
        # shape: (batch_size, seq_len, n_heads * head_k_dim), float32
        g = F.softplus(self.w_f(x).float() + self.dt_bias)
        # Per-head decay scaling, applied before any CP exchange so no head slicing
        # of A_log is needed on the head-parallel ranks.
        g = (
            -self.A_log.float().exp().unsqueeze(-1) * g.view(B, -1, self.n_heads, self.head_k_dim)
        ).flatten(2)
        ...
        gate = self.w_g(x).view(B, T_og, -1, self.head_v_dim)

        # shape: (batch_size, seq_len, d_model)
        return self.w_out(self.o_norm(o, gate).view(B, T_og, -1))
    ...
    def apply_cp(
        ...
        # Ulysses CP requires divisibility by CP world size for:
        # 1. n_heads - q/k/g/b are exchanged as flat key_dim channel tensors, and each
        #    rank's contiguous channel slice must correspond to whole heads (for the
        #    per-head view, in-kernel l2norm, and the pre-applied A_log scaling)
        # 2. n_v_heads - for head partitioning in the recurrent kernel
        # (key_dim and value_dim divisibility for the conv layers follows from these.)
    ...
    def init_weights(
        ...
        *,
        ...
        for w in (self.w_q, self.w_k, self.w_v, self.w_b, self.w_w, *self.w_f, *self.w_g):
            init_linear(w, std=std, generator=generator)
        for w in (self.q_conv1d, self.k_conv1d, self.v_conv1d):
            init_linear(w, std=std, generator=generator)
    ...
    def num_flops_per_token(self, seq_len: int) -> int:
        """
        Compute FLOPs per token for GDN2.

        This accounts for:
        - Linear projections (w_q, w_k, w_v, w_f, w_b, w_w, w_g, w_out)
        - Short convolutions (q, k, v)
        - GDN2 recurrent computation
        - Gated RMS normalization
        """
        del seq_len
        # Linear projection FLOPs (2 ops per multiply-add)
        ...
        # GDN2 recurrent computation per token, per element of the (n_v_heads, head_k_dim,
        # head_v_dim) state:
        # - Channel-wise state decay Diag(exp(g)) S
        # - Erase readout (b * k)^T S
        # - Erase rank-1 update k ((b * k)^T S)
        # - Write outer product k (w * v)^T
        # - Query-state matmul q S
        # Each is 2 FLOPs per element (multiply-add or similar). The elementwise gate
        # products (b * k, w * v) are O(head_dim) per head and negligible.
        state_size = self.n_v_heads * self.head_k_dim * self.head_v_dim
...
class GatedDeltaNet2Config(SequenceMixerConfig[GatedDeltaNet2]):
    ...
    GVA is preferred over GQA for linear RNNs like GDN2 because the recurrent state
    has shape ``(n_v_heads, head_k_dim, head_v_dim)``. Unlike softmax attention where
    the KV cache grows with sequence length (motivating GQA to reduce it), the linear
    RNN state is constant size regardless of sequence length. Since there's no memory
    scaling issue to solve, we instead can opt to increase the state size to improve the model's
    ...
    def num_params(self, d_model: int) -> int:
        ...
        params += d_model * key_dim  # w_k
        params += d_model * value_dim  # w_v
        params += d_model * head_v_dim  # w_f[0]
        params += head_v_dim * key_dim  # w_f[1]
        params += d_model * key_dim  # w_b
        params += d_model * value_dim  # w_w
        params += d_model * head_v_dim  # w_g[0]
        params += head_v_dim * value_dim  # w_g[1]
        params += value_dim  # w_g[1] bias
        params += value_dim * d_model  # w_out
