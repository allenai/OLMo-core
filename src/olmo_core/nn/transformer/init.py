...
class InitMethod(StrEnum):
    ...
    def _init_weight(
        ...
        *,
        ...
        parametrization: Optional[ParametrizationBase] = None,
    ):
        std = ParametrizationBase.scale_init_std(parametrization, std)
        _apply_init(
        ...
        *,
        ...
        if isinstance(m, Attention) or hasattr(m, "w_q"):
            ...
            for name, w in zip(("w_q.weight", "w_k.weight", "w_v.weight"), (m.w_q, m.w_k, m.w_v)):
                self._init_linear(
                    w, std=std, generator=generator, parametrization=m.parametrizations.get(name)
                )
        ...
        parametrizations: Optional[Dict[str, ParametrizationBase]] = getattr(
            m, "parametrizations", None
        )
        self._init_linear(
            m.w_out,
            std=std,
            generator=generator,
            parametrization=parametrizations.get("w_out.weight") if parametrizations else None,
        )
    ...
    def init_feed_forward(
        ...
        *,
        ...
        self._init_linear(
            m.w1, std=std, generator=generator, parametrization=m.parametrizations.get("w1.weight")
        )
        ...
        self._init_linear(
            m.w3, std=std, generator=generator, parametrization=m.parametrizations.get("w3.weight")
        )
        ...
        self._init_linear(
            m.w2, std=std, generator=generator, parametrization=m.parametrizations.get("w2.weight")
        )
    ...
    def init_feed_forward_moe(
        ...
        *,
        ...
        self._init_weight(
            ...
            std=std,
            generator=generator,
            parametrization=router.parametrizations.get("router"),
        )

        mlp = cast(Union[MoEMLP, DroplessMoEMLP], m.experts.mlp)
        self._init_weight(
            mlp.w1, std=std, generator=generator, parametrization=mlp.parametrizations.get("w1")
        )
        self._init_weight(
            mlp.w2, std=std, generator=generator, parametrization=mlp.parametrizations.get("w2")
        )
        self._init_weight(
            mlp.w3, std=std, generator=generator, parametrization=mlp.parametrizations.get("w3")
        )
