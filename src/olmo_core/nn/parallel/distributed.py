...
class MultiGroupDistributedDataParallel(Module):
    """
    A data-parallel module wrapper that all-reduces gradients across one or more process groups.

    Unlike :class:`torch.nn.parallel.DistributedDataParallel`, gradients are accumulated into
    flat, pre-allocated **bucket views** (each parameter's ``.grad`` is a view into a contiguous
    bucket), and different parameters may be assigned to different process groups via
    :data:`param_process_group_fn` — useful when subsets of the model are replicated over
    different device meshes. Bucketed all-reduces are launched (overlapped with the backward
    pass) as each bucket fills, and :meth:`finalize_grad_reduce` must be called after
    ``loss.backward()`` to wait for them to complete.

    Optionally, gradients can be accumulated and/or reduced in fp32 for numerical stability with
    ...
    :param module: The module to wrap. All parameters must be materialized (no meta/uninitialized
        params) and the module must not have buffers when using :data:`param_process_group_fn`.
    ...
    :param accumulate_grads_in_fp32: Accumulate gradients into an fp32 buffer (requires
        :data:`reduce_grads_in_fp32`).
    :param reduce_grads_in_fp32: All-reduce gradients in fp32 regardless of parameter dtype.
    """
    ...
    def finalize_grad_reduce(self):
        ...
        # In some cases (e.g. imbalanced MoE routing) a param's post-accumulate hook is never called
        # this step, so its grad_ready is never set. Mark those ready so their bucket can reduce.
        # Do NOT zero the bucket view here: it already holds the correct value — zero if the param
        # was never touched (buffers are zeroed at the start of the accumulation window), or the
        # accumulated grad if it was touched in an earlier `no_sync()` micro-batch but not the final
        # (synced) one. Zeroing would discard that accumulated grad.
