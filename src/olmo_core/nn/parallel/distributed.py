...
class MultiGroupDistributedDataParallel(Module):
    ...
    def __init__(
        ...
        self._reduce_scatter_pack_scratch: Dict[
        ...
        def flush_current_bucket() -> None:
            ...
            if current_reduce_scatter:
                for param in current_params:
                    ...
                    local_ranges.append(
                        (local_numel, local_numel + param_local_numel)
                    )
    ...
    def configure_reduce_scatter_params(
        self, params: set[torch.nn.Parameter]
    ) -> None:
        ...
        if not self.use_reduce_scatter:
            if params:
                raise RuntimeError(
                    "Cannot configure reduce-scatter parameters when "
                    "use_reduce_scatter=False."
                )
            return
        ...
        if self._grad_reduce_hooks:
            raise RuntimeError(
                "Cannot rebuild gradient buckets while reductions are in flight."
            )
        ...
        if bucket.reduce_scatter:
            ...
            for (full_start, full_end), (local_start, local_end) in zip(
                ...
                packed[:, local_start:local_end].copy_(
                    bucket.flat_storage[full_start:full_end].view(
                        world_size, local_numel
                    )
                )
