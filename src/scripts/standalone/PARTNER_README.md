...
These are new, separate files. The original `README.md`, `standalone_model.py`,
`fused_model.py` and `distributed_fused_benchmark.py` are preserved byte-for-byte
from standalone commit `00ea561418c54968ea5043279e9450a8c45b97e7`.
Use the `partner_*` files below for the updated family; the original filenames
continue to describe the legacy model.
...
- `partner_model.py` is a readable, unfused PyTorch reference implementation
...
- `partner_fused_model.py` builds `tiny`, `small`, `medium`, `large` or a separate `30m` smoke using
...
- `partner_distributed_fused_benchmark.py` is the runnable CUDA benchmark. It applies
...
pip install torch
python partner_model.py --model-size tiny
python partner_model.py --model-size large
```
...
```bash
python partner_fused_model.py --model-size 30m
python partner_fused_model.py --model-size tiny --no-emo --global-load-balancing
python partner_fused_model.py --model-size large
```
...
torchrun --standalone --nproc-per-node=1 \
  partner_distributed_fused_benchmark.py \
  --model-size 30m \
  --ep-degree 1 \
  --sequence-length 512 \
