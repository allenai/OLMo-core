"""ICL overlap full-attention ladder on v02, align-fix + data-loader seed=42.

Identical to ``icl_overlap_v02_full_attn_ladder.py`` (which is already the
align-fix version, using ``PerFileChunkedInstanceSource`` for the ICL
side) except the ``ComposableDataLoaderConfig.seed`` is set to 42
(matching the seed used by the old-pipeline
``NumpyDataLoaderConfig``). Paired with
``baseline_v02_seed42_full_attn_ladder.py``.

Motivation: isolate whether the integer seed alone (i.e., a particular
random trajectory) is responsible for any of the observed
old-pipeline-vs-new-pipeline TE gap on C4 CE. If this run lands near
the default-seed align-fix run (+0.017 TE at 2×C), seed is not the
variable.
"""
import argparse
import logging
