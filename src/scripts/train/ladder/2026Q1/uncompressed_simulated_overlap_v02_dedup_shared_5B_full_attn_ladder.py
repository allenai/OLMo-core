...
shared-bloom dedup canary.

Same setup as ``uncompressed_simulated_overlap_v02_full_attn_ladder`` but
trained on the 5B-token shared-bloom dedup canary instead of the original
500B no-dedup ucm-sim run. Trains directly on the dedup-canary data as if
it were a normal pretraining mix (i.e. as a *suffix-sampled baseline*),
not as a treatment with overlap structure. Lets us check how close the
doc-deduped distribution is to the baseline distribution by comparing a
model trained on it against the baseline ladders at matched compute.

Data source (5.9B actual tokens / 5.0B compressed-equivalent budget):
``/weka/oe-training-default/ai2-llm/suffix-arrays/preprocessed/dolma2-0625-v02/uncompressed-simulated-overlap-v02-8192-wo-replace-dedup-by-doc-shared-5B/allenai/dolma2-tokenizer/``.

At 190M / 2xC (~7.6B tokens) the dataset cycles ~1.3x, which is
intentional — we're measuring distribution similarity, not testing
overfitting.
"""
import argparse
import logging
...
UNCOMPRESSED_SIM_OVERLAP_PATHS = [
    "/weka/oe-training-default/ai2-llm/suffix-arrays/preprocessed/dolma2-0625-v02/uncompressed-simulated-overlap-v02-8192-wo-replace-dedup-by-doc-shared-5B/allenai/dolma2-tokenizer/*.npy",
]
