"""SA random-branch K=8 N=1 J=256 50B — random-branch-in-anchor MASK.

Per notes/random_branch_anchor_design.md: a baseline-style anchor with
K=8 random k-positions, each carrying N=1 walked SA-neighbors (pure
scatter, no chain) with one J=256-token head. Heads loss-masked.
Overlap shared with anchor (vis_limit=max_seq_len). This is the
pure-scatter format baseline.

Built via build_sa_random_branch_anchor.py with --K 8 --N-per-kpos 1
--J 256 --baseline-anchor-mult 0.6. Measured comp_ratio=1.0235.
...
BASE = (
    ...
    "dolma2-0625-v02/sa-random-branch-K8-N1-J256-50B/allenai/dolma2-tokenizer"
)
