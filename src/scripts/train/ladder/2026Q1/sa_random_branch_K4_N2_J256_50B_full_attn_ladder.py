"""SA random-branch K=4 N=2 J=256 50B — random-branch-in-anchor MASK.

Per notes/random_branch_anchor_design.md: a baseline-style anchor with
K=4 random k-positions, each carrying N=2 walked SA-neighbors as a
tree-mask chain with one J=256-token head per walked. Heads
loss-masked. Overlap shared with anchor (vis_limit=max_seq_len).

Built via build_sa_random_branch_anchor.py with --K 4 --N-per-kpos 2
--J 256 --baseline-anchor-mult 0.625. Measured comp_ratio=1.1072.
...
BASE = (
    ...
    "dolma2-0625-v02/sa-random-branch-K4-N2-J256-50B/allenai/dolma2-tokenizer"
)
