...
**FAST-COMPRESSIVE-LANDMARK** CPT model (block-128 variant) on a THREE-WAY MIX of raw
...
This is the BLOCK-128 counterpart of ``Qwen3-4B-compressive-cptmix-5task-32k-SFT.py`` (block 64).
Differences, both forced by the larger landmark block:

  1. Model: ``fast_compressive_landmark=True, mem_freq=127`` (block 128), with
     ``nonselected_landmark_mass=0.1``. Init from the block-128 compressive CPT base on weka
     (``qwen3-4b-compressive-landmark-block128/step2385``), NOT the block-64 or dense/plain-landmark
...
the existing CPT mix rather than replacing it.
    ...
    PYTHONPATH=src python src/scripts/train/sft/Qwen3-4B-compressive-block128-cptmix-5task-32k-SFT.py \\
        dry_run q4b-comp-block128-cptmix-5task-32k ai2/jupiter
    PYTHONPATH=src python src/scripts/train/sft/Qwen3-4B-compressive-block128-cptmix-5task-32k-SFT.py \\
        launch  q4b-comp-block128-cptmix-5task-32k ai2/neptune --launch.num_nodes=2
"""
...
# ---------------------------------------------------------------------------
# Landmark geometry (block-128 variant)
# ---------------------------------------------------------------------------
MEM_FREQ = 127
BLOCK_SIZE = MEM_FREQ + 1  # 128
SEQUENCE_LENGTH = (
    40960  # landmark-token-space window; divisible by BLOCK_SIZE; >= max ladder40k doc
)
CONTENT_LEN = SEQUENCE_LENGTH // BLOCK_SIZE * MEM_FREQ  # 40640 content tokens (pre-landmark)
...
CP_DEGREE = 8
NUM_NODES = 1  # 1 node x 8 GPUs, cp_degree=8 -> NUM_NODES DP replicas
...
# Block-128 compressive-landmark CPT base (model+optim) on weka. Loaded weights-only
# (load_optim_state=False).
BASE_CHECKPOINT = (
    "/weka/oe-training-default/ai2-llm/checkpoints/"
    "qwen3-4b-compressive-landmark-block128/step2385/model_and_optim"
)
