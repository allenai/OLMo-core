"""
This script can be used to launch an SFT run for the Qwen3 4B model on Beaker.
...
DEFAULT_NUM_NODES = 1
GPUS_PER_NODE = 8
MAX_RANK_MICROBATCH_SIZE_TOKENS = 16_384  # 4B needs CP at 32k seq_len; each GPU handles 16k via ring attention
...
class SFTConfig(Config):
    """
    Custom config class for the Qwen3 4B SFT run.
    """
    ...
    def build(
        ...
        *,
        ...
        # Manual config to match our modified tokenizer where eos_token was changed
        # from <|im_end|> (151645) to <|endoftext|> (151643) for olmo chat template
        # compatibility. See open-instruct/scripts/data/prepare_modified_tokenizer.py.
        tokenizer_config = TokenizerConfig(
            vocab_size=151936,
            eos_token_id=151643,  # <|endoftext|> — changed from default <|im_end|> (151645)
            pad_token_id=151645,  # <|im_end|>
        ...
        model = TransformerConfig.qwen3_4B(
        ...
        config = SFTConfig(
            ...
            .with_callback(
                ...
                WandBCallback(
                    name=run_name,
                    entity="ai2-llm",
                    project=f"{user_name}-Qwen3-4B-sft",
                    enabled=False,
                    cancel_check_interval=10,
                ),
            ),
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="SFT the Qwen3 4B model.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python %(prog)s dry_run test /path/to/ckpt ai2/cluster --dataset_path=/path/to/dataset
  python %(prog)s launch run01 /path/to/ckpt ai2/jupiter-cirrascale-2 --seq_len=4096 --num_nodes=1 --dataset_path=/path/to/dataset --launch.priority=high --launch.follow=false
""",
    )
