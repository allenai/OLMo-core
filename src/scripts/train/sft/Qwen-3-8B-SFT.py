"""
This script can be used to launch an SFT run for the Qwen3 8B model on Beaker.
Run the script without any arguments to see usage info. See the README for more details.

IMPORTANT NOTES for training on multi-turn data:
Tokenize with the data transformation function last_turn_tulu_tokenize_and_truncate_v1. 
Dataset must be split by assistant turn with thinking masked in historical assistant turns (n assistant turns -> n messages)

Example 1 — Multi-step (non_conversation)                                                                                                 
                                                                                                                                        
A single user prompt followed by multiple tool-call rounds. Each cumulative prefix up to an assistant turn becomes its own training       
record. Only the last assistant turn retains <think>.                                                                                     
                
Record ending at turn 1 (3 msgs):                                                                                                         
[0] system:    "You are a helpful assistant..."                                                                                         
[1] user:      "Update TASK-003 in PROJ-001 to reflect that the missing interaction guidance..."
[2] assistant: tool_calls=[get_task_by_id(...)]          ← no <think> (historical)                                                      
                                                                                                                                        
Record ending at turn 2 (5 msgs):                                                                                                         
[0] system:    "You are a helpful assistant..."                                                                                         
[1] user:      "Update TASK-003 in PROJ-001..."
[2] assistant: tool_calls=[get_task_by_id(...)]          ← no <think> (historical)                                                      
[3] tool:      {"success": true, "data": {"task_id": "TASK-003", ...}}
[4] assistant: tool_calls=[is_user_active(...)]          ← no <think> (historical)                                                      
                
Record ending at turn 3 (7 msgs):                                                                                                         
[0] system:    "You are a helpful assistant..."
[1] user:      "Update TASK-003 in PROJ-001..."                                                                                         
[2] assistant: tool_calls=[get_task_by_id(...)]          ← no <think> (historical)
[3] tool:      {"success": true, "data": {"task_id": "TASK-003", ...}}                                                                  
[4] assistant: tool_calls=[is_user_active(...)]          ← no <think> (historical)
[5] tool:      {"success": true, "data": {"user_id": "USR-002", "is_active": true}}                                                     
[6] assistant: <think>I realize that the attachment or comment section...</think>
                tool_calls=[update_task_title(...)]       ← <think> preserved (last turn)                                                
                                                                                                                                                                                                                                                                              
Example 2 — Multi-turn (conversation)                                                                                                     
                                    
Multiple user messages interleaved with tool calls. Same split-per-assistant-turn logic; history accumulates across user turns.
                                                                                                                                        
Record ending at turn 1 (3 msgs):
[0] system:    "You are a helpful assistant..."                                                                                         
[1] user:      "Hi! Could you rename TASK-003 to 'Accessibility review for signup and keyboard flow'?"
[2] assistant: tool_calls=[update_task_title(...)]       ← no <think> (historical)                                                      
                                                                                                                                        
Record ending at turn 2 (5 msgs):                                                                                                         
[0] system:    "You are a helpful assistant..."                                                                                         
[1] user:      "Hi! Could you rename TASK-003..."
[2] assistant: tool_calls=[update_task_title(...)]       ← no <think> (historical)
[3] tool:      {"success": true, "message": "Task title updated successfully."}                                                         
[4] assistant: "Done — TASK-003 has been renamed. Anything else?"
                                                            ← no <think> (historical)                                                      
...
[2] assistant: tool_calls=[update_task_title(...)]       ← no <think> (historical)
[3] tool:      {"success": true, "message": "Task title updated successfully."}                                                         
[4] assistant: "Done — TASK-003 has been renamed. Anything else?"  ← no <think> (historical)                                            
[5] user:      "Great, thanks! Could you reassign TASK-003 from USR-004 to USR-002?"
[6] assistant: <think>I need to follow the instructions for task reassignment...</think>                                                
                tool_calls=[assign_task_to_user(...)]     ← <think> preserved (las

Example tokenization command:
PYTHONPATH=/weka/oe-adapt-default/shaktis/general-tool-use/open-instruct python scripts/data/convert_sft_data_for_olmocore.py \
      --tokenizer_name_or_path /weka/oe-training-default/ai2-llm/shaktis/tokenizer/Qwen3-8B \
      --dataset_transform_fn last_turn_tulu_tokenize_and_truncate_v1 sft_tulu_filter_v1 \
      --dataset_mixer_list /weka/oe-training-default/ai2-llm/shaktis/data/envscaler_complex_qwen3_8b_alpaca_think_masked_history.jsonl 1.0 \
      --output_dir /weka/oe-training-default/ai2-llm/shaktis/data/envscaler_complex_qwen3_8b_alpaca_think_masked_history \
      --max_seq_length 32768 \
      --chat_template_name qwen123 \
      --add_eos_separator
"""
...
DEFAULT_NUM_NODES = 1
GPUS_PER_NODE = 8
MAX_RANK_MICROBATCH_SIZE_TOKENS = 16_384  # max tokens this config can handle on an A100
...
class SFTConfig(Config):
    ...
    def build(
        ...
        *,
        ...
        # from_hf returns bos_token_id=151643 (<|endoftext|>) which never appears in chat data.
        # Override to 151644 (<|im_start|>): conversation boundaries are <|im_end|><|im_start|>
        # (no newline), while turn boundaries within a conversation are <|im_end|>\n<|im_start|>.
        # This lets iter_document_indices detect the 9022 conversation boundaries correctly.
        # TODO: why use dataclasses.replace here instead of just creating a new TokenizerConfig? The from_hf method does some important stuff that we don't want to lose, right?
        tokenizer_config = dataclasses.replace(
            TokenizerConfig.from_hf("Qwen/Qwen3-8B"), bos_token_id=151644
        )
        ...
        model = TransformerConfig.qwen3_8B(
        ...
        dp_config = TransformerDataParallelConfig(
            ...
            // (bs_config.cp_degree or 1), # try to keep communication w/in a node
        )
        ...
        config = SFTConfig(
            ...
            trainer=TrainerConfig(
                save_folder=f"{root_dir}/checkpoints/{user_name}/qwen-sft/{run_name}",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="SFT the 8B model.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  uv run python /weka/oe-adapt-default/shaktis/OLMo-core/src/scripts/train/sft/Qwen-3-8B-SFT.py dry_run \
      test \
      /weka/oe-training-default/ai2-llm/shaktis/olmocore/Qwen3-8B/model_and_optim \
      ai2/jupiter \
      --dataset_path /weka/oe-training-default/ai2-llm/shaktis/data/envscaler-qwen-3-8b-no-think \
      --budget ai2/oe-adapt \
