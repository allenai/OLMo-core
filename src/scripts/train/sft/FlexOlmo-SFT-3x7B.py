...
def _get_default_global_batch_size(model_name: str) -> int:
    """
    Return a default global_batch_size (in tokens) that divides evenly for common
    node counts of each model family.

    For NxEP models the data-parallel world size is
        dp_world_size = (num_nodes * GPUS_PER_NODE) / ep_degree

    The global batch size must be divisible by dp_world_size * sequence_length.

    - ep_degree=5 (5x7b): needs num_nodes multiple of 5 → dp=8   → 80 * seq works
    - ep_degree=4 (4x7b): needs num_nodes multiple of 1 → dp=8*N/4 → 80 * seq works for N≥1
    - ep_degree=3 (3x7b): needs num_nodes multiple of 3 → dp=8   → 48 * seq keeps grad-accum
                           reasonable (48 sequences → 6 per rank with dp=8)
    - ep_degree=2 (2x7b): dp=4*N → 80 * seq works
    """
    if model_name and "3x7b" in model_name:
        # 48 is divisible by 8 (dp_world_size with 3 nodes) and keeps grad-accum sane.
        # With dp=8 that's 6 sequences/rank at seq_len=16384 → 98304 tokens/rank.
        return 48 * DEFAULT_SEQUENCE_LENGTH
    # Default for everything else (2x7b, 4x7b, 5x7b, dense models, etc.)
    return 80 * DEFAULT_SEQUENCE_LENGTH
...
class SFTRouterConfig(Config):
    ...
    def build(
        ...
        *,
        ...
        # ----------------------------------------------------------------
        # Compute world_size once for dynamic num_replicas later
        # ----------------------------------------------------------------
        world_size = num_nodes * GPUS_PER_NODE
        ...
        elif model_name == "olmoe-2x7b-only-router":
            # MoE model configuration for router SFT
            model = TransformerConfig.olmoe_nx7b(  # Use MoE configuration
                vocab_size=tokenizer_config.padded_vocab_size(),
                num_experts=2,
        ...
        elif model_name == "olmoe-2x7b-unfrozen-lm-head":
            # MoE model configuration for router SFT
            model = TransformerConfig.olmoe_nx7b(  # Use MoE configuration
                vocab_size=tokenizer_config.padded_vocab_size(),
                num_experts=2,
        ...
        elif model_name == "olmoe-2x7b-unfrozen-lm-head-embed":
            # MoE model configuration for router SFT
            model = TransformerConfig.olmoe_nx7b(  # Use MoE configuration
                vocab_size=tokenizer_config.padded_vocab_size(),
                num_experts=2,
        ...
        elif model_name == "olmoe-2x7b-unfrozen":
            # MoE model configuration for router SFT
            model = TransformerConfig.olmoe_nx7b(  # Use MoE configuration
                vocab_size=tokenizer_config.padded_vocab_size(),
                num_experts=2,
        ...
        elif model_name == "olmoe-2x7b-unfrozen-lm-head-embed-1-active":
            # MoE model configuration for router SFT
            model = TransformerConfig.olmoe_nx7b(  # Use MoE configuration
                vocab_size=tokenizer_config.padded_vocab_size(),
                num_experts=2,
        ...
        elif model_name == "olmoe-2x7b-expert-bias":
            # MoE model configuration for router SFT
            model = TransformerConfig.olmoe_nx7b_with_expert_bias(  # Use MoE configuration
                vocab_size=tokenizer_config.padded_vocab_size(),
                num_experts=2,
                top_k=2,  # Override default of 1
                lb_loss_weight=0.0,
        ...
        # =============================================================
            ...
            # 3x7b router-only: freeze everything except router
            ...
            freeze_experts = "none"
            rank_microbatch_size = 4096
        elif model_name == "olmoe-3x7b-1-active":
            model = TransformerConfig.olmoe_nx7b(
                vocab_size=tokenizer_config.padded_vocab_size(),
                num_experts=3,
                top_k=1,
            ...
            freeze_experts = "none"
            rank_microbatch_size = 4096
        elif model_name == "olmoe-3x7b-unfrozen":
            # 3x7b fully unfrozen: train everything
            model = TransformerConfig.olmoe_nx7b(
                vocab_size=tokenizer_config.padded_vocab_size(),
                num_experts=3,
                top_k=3,
            ...
            freeze_experts = "none"
            rank_microbatch_size = 2048
        elif model_name == "olmoe-3x7b-unfrozen-lm-head":
            # 3x7b: unfreeze router + experts + lm_head
            model = TransformerConfig.olmoe_nx7b(
                vocab_size=tokenizer_config.padded_vocab_size(),
                num_experts=3,
                top_k=3,
            ...
            freeze_experts = "none"
            rank_microbatch_size = 2048
        elif model_name == "olmoe-3x7b-unfrozen-lm-head-embed":
            # 3x7b: unfreeze router + experts + lm_head + embeddings
            model = TransformerConfig.olmoe_nx7b(
                vocab_size=tokenizer_config.padded_vocab_size(),
                num_experts=3,
                top_k=3,
        ...
        # =============================================================
        # 5x7b model variants (unchanged)
        # =============================================================
        elif model_name == "olmoe-5x7b":
            model = TransformerConfig.olmoe_nx7b(  # Use MoE configuration
                vocab_size=tokenizer_config.padded_vocab_size(),
                num_experts=5,
                top_k=5,  # Override default of 1
                lb_loss_weight=0.0,
                z_loss_weight=0.001,
                use_flash=True,
                freeze_params=[
            ...
            if num_nodes == 8:
                ...
                rank_microbatch_size=2048
        elif model_name == "olmoe-4x7b-1-active":
            model = TransformerConfig.olmoe_nx7b(  # Use MoE configuration
                vocab_size=tokenizer_config.padded_vocab_size(),
                num_experts=4,
                top_k=1,  # Override default of 1
        ...
        # ----------------------------------------------------------------
        # Validate that world_size is divisible by ep_degree
        # ----------------------------------------------------------------
        if world_size % ep_degree != 0:
            raise OLMoConfigurationError(
                f"world_size ({world_size} = {num_nodes} nodes * {GPUS_PER_NODE} GPUs) "
                f"must be divisible by ep_degree ({ep_degree}). "
                f"For ep_degree={ep_degree}, use num_nodes that is a multiple of "
                f"{ep_degree} (e.g. {ep_degree}, {ep_degree*2}, ...)."
            )

        # Compute num_replicas dynamically: dp_world_size = world_size / ep_degree
        num_replicas = world_size // ep_degree
        ...
        config = SFTRouterConfig(
            ...
            train_module=FreezeTransformerTrainModuleConfig(  # Changed class name
                ...
                ep_config=TransformerExpertParallelConfig(
                    degree=ep_degree,  # Split experts across ep_degree GPUs
                ),
                ...
                dp_config=TransformerDataParallelConfig(
                    ...
                    num_replicas=num_replicas,  # dynamically computed: world_size / ep_degree
...
Note: This script requires MoE models. Available model options:
  - olmo2-7b: Dense OLMo2-7B
  - olmo3-7b: Dense OLMo3-7B  
  - olmoe-2x7b[-*]: 2-expert variants
  - olmoe-3x7b[-*]: 3-expert variants (use num_nodes multiple of 3)
  - olmoe-4x7b[-*]: 4-expert variants
  - olmoe-5x7b[-*]: 5-expert variants (use num_nodes multiple of 5)
""",
    )
    ...
    # Parse known args first to get model_name for default batch size
    args, overrides = parser.parse_known_args()

    # Resolve global_batch_size default based on model_name
    # (argparse can't do this natively since the default depends on another arg)
    global_batch_size_default = _get_default_global_batch_size(args.model_name)

    # Re-parse with the model-aware default
    parser.add_argument(
        "--global_batch_size",
        type=int,
        help="The global batch size in tokens. "
             f"Default depends on model: 48*seq_len for 3x7b, 80*seq_len otherwise.",
        default=global_batch_size_default,
    )
    args, overrides = parser.parse_known_args()
