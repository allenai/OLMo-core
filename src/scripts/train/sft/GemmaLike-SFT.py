"""
SFT script for GemmaLike ladder models (65M / 150M / 260M / 709M / 1.3B).

Based on Olmo-3-7B-SFT.py, adapted for the GemmaLike v2 architecture
(sliding window attention + global attention every 5 layers, QK norm, etc.).

Pass ``--size`` to select which model size to SFT (default: 1p3B).

Run the script without any arguments to see usage info.
See src/scripts/train/sft/README.md for full instructions.
"""
...
# Import GemmaLikeTransformerConfig from the ladder script.
sys.path.insert(0, str(Path(__file__).parent.parent / "ladder"))
from gemma_like_ladder import GemmaLikeTransformerConfig  # noqa: E402
...
class BatchSizeConfig:
    ...
    def __post_init__(self):
        ...
        if self.sequence_length > max_tokens_per_rank:
            ...
            log.info(
                f"Sequence length ({self.sequence_length}) exceeds max tokens per rank "
                f"({max_tokens_per_rank}). Setting cp_degree={self.cp_degree}"
            )
        ...
        if rank_batch_size_tokens > max_tokens_per_rank * cp_factor:
            ...
            log.info(
                f"Rank batch size ({rank_batch_size_tokens}) exceeds max tokens per rank "
                f"({max_tokens_per_rank}). Using grad_accum_steps={self.grad_accum_steps}"
            )
        else:
        ...
        assert self.rank_microbatch_size_tokens % self.sequence_length == 0, (
            f"rank_microbatch_size_tokens ({self.rank_microbatch_size_tokens}) must be "
            f"divisible by sequence_length ({self.sequence_length})"
        )
        ...
        assert self.global_batch_size_tokens == total_tokens, (
            f"global_batch_size_tokens ({self.global_batch_size_tokens}) must equal "
            f"rank_microbatch_size_tokens * dp_world_size * grad_accum_steps ({total_tokens})"
        )


def _separate_prefix_and_glob(prefix: str):
...
class SFTConfig(Config):
    ...
    def build(
        ...
        *,
        ...
        if size not in SIZE_TO_FACTORY:
            raise OLMoConfigurationError(
                f"Unknown size '{size}'. Must be one of {list(SIZE_TO_FACTORY)}."
            )
        factory = getattr(GemmaLikeTransformerConfig, SIZE_TO_FACTORY[size])
        model = factory(vocab_size=tokenizer_config.padded_vocab_size())
    ...
    if config.dataset is not None:
        ...
        if not trainer.maybe_load_checkpoint(trainer.save_folder):
            log.info(
                ...
                f"loading from pretraining checkpoint '{checkpoint}'"
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="SFT the GemmaLike 1.3B ladder model.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python %(prog)s dry_run test-run /path/to/checkpoint/model_and_optim ai2/jupiter --dataset_path=/path/to/dataset
  python %(prog)s launch gl-1p3b-sft-v1 /weka/oe-training-default/ai2-llm/checkpoints/kylel/olm4_mixing_calibration/gl-1p3b-dolma-2xc-v2/step42905/model_and_optim ai2/jupiter --num_nodes=2 --launch.priority=high
""",
    )
    ...
    parser.add_argument(
        "--size", choices=list(SIZE_TO_FACTORY), default=DEFAULT_SIZE,
        help="GemmaLike model size to SFT.",
    )
    ...
    parser.add_argument(
        "--global_batch_size", type=int, default=64 * DEFAULT_SEQUENCE_LENGTH
    )
