"""
This script can be used to launch an SFT run for the Qwen3 1.7B model on Beaker.
...
DEFAULT_NUM_NODES = 1
GPUS_PER_NODE = 8
MAX_RANK_MICROBATCH_SIZE_TOKENS = 32_768  # 1.7B fits 32k tokens per GPU on H100
...
class SFTConfig(Config):
    """
    Custom config class for the Qwen3 1.7B SFT run.
    """
        ...
        config = SFTConfig(
            ...
            .with_callback(
                ...
                WandBCallback(
                    name=run_name,
                    entity="ai2-llm",
                    project=f"{user_name}-Qwen3-1.7B-sft",
                    enabled=False,
                    cancel_check_interval=10,
                ),
            ),
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="SFT the Qwen3 1.7B model.",
