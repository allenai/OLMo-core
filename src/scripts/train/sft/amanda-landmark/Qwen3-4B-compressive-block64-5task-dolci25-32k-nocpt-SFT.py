...
**FAST-COMPRESSIVE-LANDMARK** CPT model (block-64 variant) on a 25/75 BLEND of
...
parallelism) is identical for a fair block-size comparison.
  ...
  1. Model: ``fast_compressive_landmark=True, mem_freq=63`` (block 64), with
     ``nonselected_landmark_mass=0.1``. Init from the block-64 (default) compressive CPT base on weka
     (``amandab/q4b-base-fast-compressive-landmark-8node/step2385/model_and_optim``). The landmark-token (151860)
...
comparability across variants and block sizes.
    ...
    PYTHONPATH=src python src/scripts/train/sft/amanda-landmark/Qwen3-4B-compressive-block64-5task-dolci25-32k-nocpt-SFT.py \\
        dry_run q4b-comp-block64-5task-dolci25-32k-nocpt ai2/jupiter
    PYTHONPATH=src python src/scripts/train/sft/amanda-landmark/Qwen3-4B-compressive-block64-5task-dolci25-32k-nocpt-SFT.py \\
        launch  q4b-comp-block64-5task-dolci25-32k-nocpt ai2/neptune --launch.num_nodes=2
"""
...
# ---------------------------------------------------------------------------
# Landmark geometry (block-64 variant)
# ---------------------------------------------------------------------------
MEM_FREQ = 63
...
# Block-64 compressive-landmark CPT base (model+optim) on weka. Loaded weights-only
