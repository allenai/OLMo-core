...
**SHARED-VECTOR LANDMARK** CPT model on a MIX of 5 long-context tasks (contradiction, nq, oolong,
rerank, outlier). NO-CPT variant: cpt_frac=0 (pure downstream FT).

This is identical to ``Qwen3-4B-compressive-blocklocal-5task-32k-nocpt-SFT.py`` except for the
attention mixer and base checkpoint, mirroring what
``Qwen3-4B-base-shared-vector-landmark-dolma3longmino.py`` did relative to the plain fast-landmark
CPT script:

  1. Attention: ``AttentionType.shared_vector_landmark`` (``shared_vector_landmark=True,
     vec_dim=VEC_DIM``) instead of compressive/fast-landmark. This keeps the (non-compressive) fast
     landmark attention weights exactly as fast_landmark, but appends a learned, per-block positional
     vector of length ``VEC_DIM`` to every value before aggregation -- see
     ``olmo_core/nn/attention/landmark_shared_vector.py``. Landmark block structure (mem_freq=63,
     block_size=64) is unchanged, so the same ``LandmarkPackingInstanceSourceConfig`` packing applies.

  2. Base checkpoint: the SHARED-VECTOR-landmark CPT run on the 15B-token dolma3_longmino sample
     (``qwen3-4b-shared-vec-landmark-d3lm15b/step2385``, 10B tokens, final checkpoint). This
     checkpoint already has the shared-vector params (``w_out_vec``, ``weight_landmark``, ``base``)
     trained (the CPT script loaded the dense base non-strictly to init them; here we load the CPT
     checkpoint itself, so a normal strict load applies -- no ``state_dict_load_opts`` override
     needed). Loaded weights-only (``load_optim_state=False``).

Everything else (data pipeline, packing, mixing fractions, geometry, optimizer/schedule) is unchanged
from ``Qwen3-4B-compressive-blocklocal-5task-32k-nocpt-SFT.py`` -- see that script's docstring for the
no-pack -> packed-with-cu_doc_lens data-pipeline notes and the SEQUENCE_LENGTH=40960 window
justification.

    PYTHONPATH=src python src/scripts/train/sft/amanda-landmark/Qwen3-4B-sharedvec-5task-32k-nocpt-SFT.py \\
        dry_run q4b-sharedvec-5task-32k-nocpt ai2/jupiter
    PYTHONPATH=src python src/scripts/train/sft/amanda-landmark/Qwen3-4B-sharedvec-5task-32k-nocpt-SFT.py \\
        launch  q4b-sharedvec-5task-32k-nocpt ai2/neptune --launch.num_nodes=2
"""
...
# Length of the learned per-block vector appended to each value, matching the base CPT checkpoint
# (module default 32; see Qwen3-4B-base-shared-vector-landmark-dolma3longmino.py).
VEC_DIM = 32
...
# SHARED-VECTOR-landmark CPT base (model+optim) on weka, from
# Qwen3-4B-base-shared-vector-landmark-dolma3longmino.py (10B tokens, final checkpoint). Loaded
# weights-only (load_optim_state=False).
BASE_CHECKPOINT = (
...
# Token-matched to the plain-compressive, block-local/partial-RoPE compressive, and dense runs.
TARGET_STEPS = 8550
...
def build_experiment_config(cli_context: CliContext) -> ExperimentConfig:
    ...
    if beaker_launch_config is not None:
        ...
        # The shared-vector tail materializes several ~1 GiB fp32 (T, nb) tensors at long context;
        # reduce allocator fragmentation so those transient spikes can reuse freed segments (same
        # mitigation as the CPT script -- see the sharedvec-tail-64k-oom note).
    ...
    # Qwen3-4B with SHARED-VECTOR LANDMARK attention (fast-landmark weights + a learned per-block
    # positional vector of length VEC_DIM appended to each value before aggregation).
    ...
    # the factory default of False silently falls back to the eager O(T^2) dense path, which OOMs at
    # this script's 40960-token window (a (B, H, T, T) attn matrix -- see the CUDA OOM in
    # landmark_shared_vector.py's _main_dense on the 2026-07-16 run).
    ...
    # landmark kernel supports cu_doc_lens / DOC_MASK, commit ffb2c461).
