...
25/75 BLEND of ``allenai/Dolci-Instruct-SFT`` (Qwen3-tokenized general instruction data) and the
original 5 long-context tasks (contradiction, nq, oolong, rerank, outlier). NO-CPT variant: cpt_frac=0
(pure downstream FT, no continued-pretraining text mixed in).

Mirrors ``Qwen3-4B-dense-5task-dolci50-32k-nocpt-SFT.py`` (dense, YaRN factor 2, flash-2, same base
checkpoint, geometry, and per-task 5-task weighting) but shifts the top-level blend ratio to 25%
Dolci-Instruct-SFT / 75% the 5-task mix (with its existing internal contra/rerank/outlier/nq/oolong
weighting), instead of 50/50. See ``Qwen3-4B-dense-dolci-32k-nocpt-SFT.py`` for the variant that
replaces the 5-task mix entirely (100% Dolci).
...
the drop rate on the Dolci side is unmeasured.
    ...
    PYTHONPATH=src python src/scripts/train/sft/amanda-landmark/Qwen3-4B-dense-5task-dolci25-32k-nocpt-SFT.py \\
        dry_run q4b-dense-5task-dolci25-32k-nocpt ai2/jupiter
    PYTHONPATH=src python src/scripts/train/sft/amanda-landmark/Qwen3-4B-dense-5task-dolci25-32k-nocpt-SFT.py \\
        launch  q4b-dense-5task-dolci25-32k-nocpt ai2/neptune --launch.num_nodes=2
"""
...
# ---------------------------------------------------------------------------
# Mixing fractions WITHIN the 5-task group (this group is then given ratio=0.75 against
...
def build_experiment_config(cli_context: CliContext) -> ExperimentConfig:
    ...
    # Top-level blend: the whole 5-task mix (internally weighted per five_task_specs) at 75%, and
    # Dolci-Instruct-SFT at 25%.
    top_level_specs = [
        MixingDocumentSourceSpecConfig(
