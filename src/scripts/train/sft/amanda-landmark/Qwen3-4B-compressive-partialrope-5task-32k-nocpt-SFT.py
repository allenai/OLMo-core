...
**FAST-COMPRESSIVE-LANDMARK + PARTIAL RoPE** CPT model on a MIX of 5 long-context tasks
...
base checkpoint, mirroring what ``Qwen3-4B-base-fast-compressive-landmark-partialrope-dolma3longmino.py``
did to the plain compressive CPT script:

  1. RoPE: applied to only the leading ``PARTIAL_ROTARY_FACTOR`` (default 25%) fraction of each head's
     dimensions; the rest pass through unrotated. This is a stock feature of ``RotaryEmbedding``
     (``RoPEConfig.partial_rotary_factor``); the fast/compressive landmark kernels only see Q/K *after*
     RoPE, so it is transparent to them (see the CPT script for the full explanation).

  2. Base checkpoint: the PARTIAL-RoPE compressive CPT run on the 15B-token dolma3_longmino sample
     (``qwen3-4b-fclm-partialrope-d3lm15b/step2385``, 10B tokens, final checkpoint), NOT the plain
...
packed-with-cu_doc_lens data-pipeline notes and the SEQUENCE_LENGTH=40960 window justification.
    ...
    PYTHONPATH=src python src/scripts/train/sft/amanda-landmark/Qwen3-4B-compressive-partialrope-5task-32k-nocpt-SFT.py \\
        dry_run q4b-comp-partialrope-5task-32k-nocpt ai2/jupiter
    PYTHONPATH=src python src/scripts/train/sft/amanda-landmark/Qwen3-4B-compressive-partialrope-5task-32k-nocpt-SFT.py \\
        launch  q4b-comp-partialrope-5task-32k-nocpt ai2/neptune --launch.num_nodes=2
"""
...
# Fraction of each head dimension that RoPE is applied to, matching the base CPT checkpoint. The
# leading int(head_dim * PARTIAL_ROTARY_FACTOR) dims are rotated; the rest pass through unchanged.
...
# PARTIAL-RoPE compressive-landmark CPT base (model+optim) on weka, from
# Qwen3-4B-base-fast-compressive-landmark-partialrope-dolma3longmino.py (10B tokens, final
# checkpoint). Loaded weights-only (load_optim_state=False).
BASE_CHECKPOINT = (
    "/weka/oe-training-default/ai2-llm/checkpoints/qwen3-4b-fclm-partialrope-d3lm15b/"
...
def build_experiment_config(cli_context: CliContext) -> ExperimentConfig:
    ...
    # Switch to partial RoPE, matching the base CPT checkpoint: rotate only the leading
    # PARTIAL_ROTARY_FACTOR fraction of each head's dimensions instead of the full head dimension. See
    # Qwen3-4B-base-fast-compressive-landmark-partialrope-dolma3longmino.py for the full rationale.
