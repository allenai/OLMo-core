...
LONGER VARIANT: Doubled epochs (TARGET_STEPS 21400 vs. 10700) for ~1.4B training tokens (~78% of the
1.81B SFT mix), ~8.4h on jupiter H100 (~1.4s/step).
...
EOS-based doc-length detection yields correct block-diagonal (varlen) masking.
    ...
    PYTHONPATH=src python src/scripts/train/sft/amanda-landmark/Qwen3-4B-dense-5task-32k-nocpt-SFT-longer.py \\
        dry_run q4b-dense-5task-32k-nocpt-longer ai2/jupiter
    PYTHONPATH=src python src/scripts/train/sft/amanda-landmark/Qwen3-4B-dense-5task-32k-nocpt-SFT-longer.py \\
        launch  q4b-dense-5task-32k-nocpt-longer ai2/neptune --launch.num_nodes=2
"""
...
# ---------------------------------------------------------------------------
LR = 1e-5
# ~1.4B training tokens: 21400 steps x 2 DP windows x 32768 = 1.4B tokens (~78% of the 1.81B SFT mix),
# ~8.4h on jupiter H100 (~1.4s/step). Doubled from 10700 (original ~700M tokens).
TARGET_STEPS = 21400
GLOBAL_BATCH_SIZE = (
