...
**MULTI-LANDMARK COMPRESSIVE** block-64 CPT model, 4lm-mean arm:
num_landmarks=4, landmark_gate_pool="mean", mem_freq=60 (block_size = 64).

Init (weights-only) from that arm's own CPT run:
qwen3-4b-mcl-block64-4lm-mean/step2385 -- geometry MUST match, and is taken from
_mcl_block64_5task_dolci25_32k_nocpt_common.py's ``_ARMS["4lm-mean"]`` for both the model and the
...
See _mcl_block64_5task_dolci25_32k_nocpt_common.py for all the shared config.
    ...
    PYTHONPATH=src python src/scripts/train/sft/amanda-landmark/Qwen3-4B-mcl-block64-4lm-mean-5task-dolci25-32k-nocpt-SFT.py \
        dry_run q4b-mcl-block64-4lm-mean-5task-dolci25-32k-nocpt ai2/jupiter-cirrascale-2
    PYTHONPATH=src python src/scripts/train/sft/amanda-landmark/Qwen3-4B-mcl-block64-4lm-mean-5task-dolci25-32k-nocpt-SFT.py \
        launch q4b-mcl-block64-4lm-mean-5task-dolci25-32k-nocpt ai2/jupiter-cirrascale-2 \
...
if __name__ == "__main__":
    main(config_builder=partial(build_mcl_experiment, arm="4lm-mean"))
