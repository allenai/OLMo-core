...
using the newer nq pipeline:
  ...
  1. Model: ``gate_temperature=True`` adds a log-parameterized per-layer scalar
     ...
     to the gate-only query at train/prefill (via the kernel's ``q_gate`` input) and to the decode
     gate. Initialized to 0 (``inv_temp = exp(-log_gate_temp) = 1``), a bit-exact no-op at step 0, so
     this run starts identically to the baseline compressive SFT and the parameter is free to move
     from there. See ``analysis/gate_scores/REPORT.md`` for the motivation: the gate softmax is
     peaky and strongly layer-dependent (near-uniform early layers, highly concentrated late layers),
     so a per-layer learnable sharpness may let the model calibrate this better than the raw QK
     magnitudes do implicitly.

  2. Weights load: ``state_dict_load_opts={"strict": False}`` on the train module. ``log_gate_temp``
     is a brand-new parameter absent from the CPT base checkpoint (trained before this feature
     existed); non-strict loading prunes it from the state dict handed to the checkpoint loader and
...
group; ``weight_decay=0.0`` for the whole run anyway, so no decay pressure on it either way).
    ...
    PYTHONPATH=src python src/scripts/train/sft/amanda-landmark/Qwen3-4B-compressive-gate-temp-5task-dolci25-32k-nocpt-SFT.py \\
...
def build_experiment_config(cli_context: CliContext) -> ExperimentConfig:
    ...
    # plus the new per-layer learnable gate temperature (see module docstring).
    ...
    train_module_config = TransformerTrainModuleConfig(
        ...
        # log_gate_temp is new (absent from the CPT base checkpoint); load non-strictly so it's kept
        # at its (no-op) initialized value instead of raising on the missing key. See
        # Qwen3-4B-base-shared-vector-landmark-dolma3longmino.py for the identical precedent.
