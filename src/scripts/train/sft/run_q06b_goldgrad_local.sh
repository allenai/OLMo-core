...
for arm in $ARMS; do
  ...
  torchrun --nproc_per_node="$NGPU" --master_port=$((29100 + RANDOM % 800)) \
    "$REPO/src/scripts/train/sft/Qwen3-0.6B-goldgrad-contradiction-n20-SFT-local.py" \
  ...
  if [ "${EVAL:-1}" = "1" ] && [ -f "$SAVE_ROOT/$RUN/model_and_optim/.metadata" ]; then
    ...
    EVAL_SIZE="$EVAL_SIZE" RUNS="$RUN" TAG="$TAG" CKPT_ROOT="$SAVE_ROOT" NGPU="$NGPU" \
      bash "$REPO/src/scripts/train/sft/eval_q06b_goldgrad_local.sh" "$RUNG" 2>&1 | grep -E "f1=|SKIP|rc=|Traceback"
