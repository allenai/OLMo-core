        ...
        config = TransformerConfig(
            ...
            block=TransformerBlockConfig(
                ...
                feed_forward_moe=MoEConfig(
                    ...
                    shared_mlp=FeedForwardConfig(hidden_size=int(d_model*SHARED_EXPERT_EXPANSION_FACTOR), bias=False) if use_shared_expert else None,
                    lb_loss_weight=0.01,
                    z_loss_weight=0.001,
...
def upcycle_virtual_group_init(
    ...
    for layer in range(NUM_LAYERS):
        # dense weights
        gate = source_model.state_dict()[f"model.layers.{layer}.mlp.gate_proj.weight"].T # (d_model, inter)
        ...
        for expert_idx in range(NUM_EXPERTS):
            ...
            w3[expert_idx, :].copy_(up[:, source_start:source_end] * (G**0.5)) # scale up by sqrt(scale)
    ...
    with torch.no_grad():
        ...
        expert_weights, expert_indices, batch_size_per_expert, aux_loss = target_model.blocks['0'].feed_forward_moe.router(x)  
        ...
        for batch_idx in range(x.shape[0]):
            for token_idx in range(x.shape[1]):
                for local_expert_idx, expert_idx in enumerate(expert_indices[batch_idx][token_idx]):
                    ...
                    moe_out[batch_idx][token_idx] += out * expert_weights[batch_idx][token_idx][local_expert_idx]
                    
        print("‖dense‖:", dense_out.std().item(),
            "‖moe‖:",   moe_out.std().item())
...
if __name__ == "__main__":
    ...
    upcycle_config = UpcycleConfig(
        ...
        target_model=build_model_config(routed_expert_norm=False, shared_expert_norm=False, use_shared_expert=False),
