...
class BaselineModelLadder(ModelLadder):
    ...
    def _get_model_config(self, *, size: ModelSize) -> TransformerConfig:
        ...
        model_config: TransformerConfig = getattr(TransformerConfig, f"olmo2_32Bmup_{size}")(
            vocab_size=self.tokenizer.padded_vocab_size(),
            init_seed=self.init_seed,
            **self.MODEL_OVERRIDES.get(size, {}),
        )

        base_size = ModelSize.size_32B
        base_model_config = getattr(TransformerConfig, f"olmo2_32Bmup_{base_size}")(
            vocab_size=self.tokenizer.padded_vocab_size(),
            init_seed=self.init_seed,
        ...
        model_config = getattr(TransformerConfig, f"olmo2_32Bmup_{size}")(
            vocab_size=self.tokenizer.padded_vocab_size(),
            init_seed=self.init_seed,
            mup=mup_config,
            **self.MODEL_OVERRIDES.get(size, {}),
