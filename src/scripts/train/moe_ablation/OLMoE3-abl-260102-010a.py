"""
Fork from 010: change to 115B token, warmup 5B tokens, decay 10B tokens
...
SHARED_MLP_HIDDEN_SIZE = (
    768  # Hidden size for shared MLP (or dense branch MLP in arctic) in MoE blocks
)
...
)  # keep 3e-4 for 1M tokens, scale up for larger gbs
...
def build_train_module_config(common: CommonComponents) -> MoEV2TransformerTrainModuleConfig:
    ...
    return MoEV2TransformerTrainModuleConfig(
        ...
        scheduler=CosWithWarmupAndLinearDecay(
            ...
            warmup=int((5e9 // GLOBAL_BATCH_TOKENS_IN_M) * GLOBAL_BATCH_TOKENS_IN_M),
            decay=int((10e9 // GLOBAL_BATCH_TOKENS_IN_M) * GLOBAL_BATCH_TOKENS_IN_M),
            decay_fraction=None,
        ),
    )
