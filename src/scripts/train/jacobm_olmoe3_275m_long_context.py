...
from scripts.train.jacobm_olmoe_ladder.v2.moe_v2_core_adapter import (
    build_model_config_from_payload,
)
...
def build_model_config(common: CommonComponents) -> OLMoDDPModelConfig:
    config = build_model_config_from_payload(load_checkpoint_config()["model"])
    ...
    for block_idx in range(config.n_layers):
        ...
        if not uses_sliding_window:
            attention.rope = replace(attention.rope, scaling=ROPE_SCALING)
        overrides[block_idx] = block_config
