"""
Smoke test for the Code Fresh per-language perplexity eval from PR #639.

Verifies that the LM evaluator with DataMix.code_fresh_ppl_validation runs correctly
on a 190M model. This mix covers 42 programming languages as memmap .npy perplexity shards.

Success criteria: LM evaluator reports per-source CE loss and PPL without errors.

Usage:
    # Dry run:
    python src/scripts/train/smoketests/code-fresh-ppl-test.py dry_run test-code-fresh ai2/jupiter

    # Launch on Beaker:
    python src/scripts/train/smoketests/code-fresh-ppl-test.py launch test-code-fresh ai2/jupiter \
        --launch.priority=<priority> --launch.follow=false --launch.step_soft_timeout=null
"""
...
SEQ_LENGTH = 4096  # Code Fresh docs are capped at ~4095 tokens
