...
Drives the full VLM stack through the trainer:
  ...
  PixMo-Cap (streamed from the HuggingFace Hub, or the local /weka cache)
...
class ExperimentConfig(Config):
    ...
    pixmo_cap_source: str = "hub"
    """PixMo-Cap loading mode: ``"hub"`` streams ``allenai/pixmo-cap`` from the
    HuggingFace Hub (downloading images per-URL); ``"local"`` reads the
    pre-downloaded /weka cache."""
    train_dataset_size: int = 2048
    """Number of PixMo-Cap training examples to consume (one epoch)."""
    init_seed: int = 12536
...
def build_config(opts, overrides: List[str]) -> ExperimentConfig:
    ...
    cfg = ExperimentConfig(
        ...
        pixmo_cap_source=opts.pixmo_cap_source,
...
def train(config: ExperimentConfig):
    ...
    # Build the training source: PixMo-Cap, streamed from the HF Hub by default
    # (``--pixmo-cap-source local`` reads the pre-downloaded /weka cache).
    source = PixmoCapDatasetConfig(
        source=config.pixmo_cap_source,
        limit=config.train_dataset_size,
        shuffle=True,
    ).build()
    log.info(
        f"Training on PixMo-Cap [{config.pixmo_cap_source}] "
        f"(up to {config.train_dataset_size} examples)"
    )
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=2048,
        help="Number of PixMo-Cap examples to consume in the training source.",
    )
    parser.add_argument(
        "--fsdp",
        action="store_true",
    ...
    parser.add_argument(
        ...
        choices=["hub", "local"],
        default="hub",
        help="PixMo-Cap loading mode: 'hub' streams allenai/pixmo-cap from the "
        "HuggingFace Hub; 'local' reads the pre-downloaded /weka cache.",
    )
