...
Verifies that FA4 works correctly on Blackwell GPUs. Trains for 20 steps
on a single node.

Note: FA4 backward is not supported on Hopper (sm90). This test must run
on Blackwell (sm100+) GPUs, e.g. B200s on ai2/titan.

Usage:
    # Dry run:
    python src/scripts/train/smoketests/flash4-test.py \
        dry_run test-flash4 ai2/titan

    # Launch on Beaker:
    python src/scripts/train/smoketests/flash4-test.py \
        launch test-flash4 ai2/titan \
