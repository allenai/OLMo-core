...
if __name__ == "__main__":
    USAGE = f"""
LC extend a 32B model.

[yellow]Usage:[/] [i blue]python[/] [i cyan]{sys.argv[0]}[/] [i b magenta]launch|train|dry_run[/] [i b]RUN_NAME CLUSTER[/] [i][OVERRIDES...][/]
...
""".strip()
    ...
    config = LcContTrain.build(
        ...
        cmd="train",
        run_name=run_name,
        load_path="checkpoints/peteish32-anneal/peteish32-from716000-peteish32-from716000-100Banneal-soup/step11921-olmocore/model_and_optim",
        cluster=cluster,
        overrides=overrides,
    )
