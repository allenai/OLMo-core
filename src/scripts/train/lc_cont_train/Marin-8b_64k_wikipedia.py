        ...
        return LcContTrain(
            ...
            train_module = TransformerTrainModuleConfig(
                rank_microbatch_size= 1 * CONTEXT_LENGTH,
                 optim=AdamWConfig(
                    lr= 1.7 * 1e-5, # changed this to reflect the end of starling LR  # or should it be the SFT LR?
                    weight_decay=0.1,
                    betas=(0.9, 0.95),
                    group_overrides=[
                        OptimGroupOverride(params=["embeddings.weight"], opts=dict(weight_decay=0.0)),
                        OptimGroupOverride(params=["attention_norm.weight", "feed_forward_norm.weight"], opts=dict(weight_decay=0.0)) # also don't weight decay layernorms
                    ],
                    fused=True,
                ),
...
""".strip()
    ...
    config = LcContTrain.build(
        ...
        cmd="train",
        run_name=run_name,
        load_path="gs://ai2-llm/checkpoints/amandab/Marin-8B-Base/",
        cluster=cluster,
        overrides=overrides,
    )
