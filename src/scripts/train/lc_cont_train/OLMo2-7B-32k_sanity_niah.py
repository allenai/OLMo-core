        ...
        *,
        ...
        return LcContTrain(
            ...
            dataset=NumpyDatasetConfig.from_data_mix(
                ...
                # docs_per_instance=4,
                # chunks_per_doc=4,
                #seed=1234, #interleaving_exempt_paths=interleaving_exempt_paths,
            ),
            data_loader=NumpyDataLoaderConfig(
                global_batch_size=64 * 2 * CONTEXT_LENGTH,  # NOTE: this is specified in TOKENS, not instances. #NOTE: added *2 because it's 32k now 
...
""".strip()
    ...
    config = LcContTrain.build(
        ...
        load_path="/weka/oe-training-default/ai2-llm/checkpoints/amandab/peteish-olmo2-med/model_and_optim/",
        cluster=cluster,
        overrides=overrides,
    )
