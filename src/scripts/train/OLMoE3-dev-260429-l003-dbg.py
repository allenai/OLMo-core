...
def build_model_config(common: CommonComponents) -> TransformerConfig:
    ...
    config = MoEFusedV2TransformerConfig(
        ...
        block=MoEFusedV2TransformerBlockConfig(
            ...
            rowwise_fp8=MoERowwiseFP8Config(enabled=USE_FP8, fused_autograd=USE_FP8_FUSED_AUTOGRAD)
            if USE_ROWWISE_A2A
            else None,
...
def build_trainer_config(common: CommonComponents) -> TrainerConfig:
    ...
    config = (
        ...
        .with_callback(
            "profiler",
            NvidiaProfilerCallback(
                enabled=USE_NV_PROFILE, profile_ranks=list(range(0, 8 * 8, 8)), start=30, end=34
            ),
        )
        .with_callback(
            "torch_mem_history",
