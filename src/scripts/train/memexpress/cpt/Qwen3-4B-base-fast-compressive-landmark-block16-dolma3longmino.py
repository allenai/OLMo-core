...
# effect during training). Pair with Qwen3-4B-base-fast-landmark-dolma3longmino.py for comparison.
# Block-size sweep variant: landmark block size 16 (default is 64; see
# Qwen3-4B-base-fast-compressive-landmark-dolma3longmino.py). This is the smallest block size the
# fused kernel supports (FastLandmarkAttention requires mem_freq >= 15; tl.dot needs tile dims >= 16).
MEM_FREQ = 15
...
if __name__ == "__main__":
    ...
    (2 nodes, urgent). Block-size sweep variant: landmark block size 16 (mem_freq=15, the smallest
    the fused kernel supports). Same setup as Qwen3-4B-base-fast-compressive-landmark-dolma3longmino.py
    otherwise. dry_run must be run on a GPU node (the fast kernel imports triton).

        python src/scripts/train/Qwen3/Qwen3-4B-base-fast-compressive-landmark-block16-dolma3longmino.py \\
