...
SEQUENCE_LENGTH = 4096  # 4k comparison context (must be divisible by BLOCK_SIZE)
...
GLOBAL_BATCH_SIZE = 32768  # 8 sequences/step at 4k (matches the 0.6B 4k control)
...
def build_experiment_config(cli_context: CliContext) -> ExperimentConfig:
    ...
    beaker_launch_config: Optional[BeakerLaunchConfig] = build_launch_config(
        ...
        budget="ai2/oe-other",
        num_nodes=1,
        num_gpus=2,  # FSDP shard_degree=2 to fit 4B params + Adam states at 4k
    )
    ...
    # Qwen3-4B with landmark attention (block_size=64, fused Triton kernel).
...
if __name__ == "__main__":
    ...
    Examples:
        Render the config and exit:
            python src/scripts/train/memexpress/cpt/Qwen3-4B-landmark.py dry_run my-run ai2/jupiter-cirrascale-2

        Launch on Beaker with 4 nodes:
            python src/scripts/train/memexpress/cpt/Qwen3-4B-landmark.py launch my-run ai2/jupiter-cirrascale-2
