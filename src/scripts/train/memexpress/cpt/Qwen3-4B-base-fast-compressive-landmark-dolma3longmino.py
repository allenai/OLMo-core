...
if __name__ == "__main__":
    ...
    (4 nodes, urgent). Same setup as Qwen3-4B-base-fast-landmark-dolma3longmino.py but with the
    compressive mixer (each block's landmark token contributes its value to the output). dry_run
    must be run on a GPU node (the fast kernel imports triton).

        python src/scripts/train/memexpress/cpt/Qwen3-4B-base-fast-compressive-landmark-dolma3longmino.py \\
