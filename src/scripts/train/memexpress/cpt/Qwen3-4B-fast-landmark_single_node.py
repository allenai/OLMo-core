...
def build_experiment_config(cli_context: CliContext) -> ExperimentConfig:
    ...
    train_module_config = TransformerTrainModuleConfig(
        ...
        compile_model=True,
        # Ulysses CP is supported by FastLandmarkAttention (it gathers the full sequence per rank
        # before the kernel), exactly like the original landmark attention:
...
if __name__ == "__main__":
    """
    Qwen3-4B with the FAST landmark attention mixer (Mohtashami & Jaggi landmark attention with an
    optimized backward). Numerically identical to the original landmark+kernel path, ~17-20x faster
    fwd+bwd. Same data pipeline as Qwen3-4B-landmark_single_node.py; only the attention type differs
    (``fast_landmark=True`` instead of ``landmark=True, landmark_use_kernel=True``).

    Examples:
        python src/scripts/train/memexpress/cpt/Qwen3-4B-fast-landmark_single_node.py dry_run my-run ai2/jupiter-cirrascale-2
        python src/scripts/train/memexpress/cpt/Qwen3-4B-fast-landmark_single_node.py launch my-run ai2/jupiter-cirrascale-2
    """
