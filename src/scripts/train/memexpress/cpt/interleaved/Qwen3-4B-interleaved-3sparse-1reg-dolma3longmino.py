...
# This variant cycles a 3-SPARSE / 1-REGULAR pattern: every 3 sparse-landmark layers are
# followed by one full (normal) attention layer, always ending with a full-attention layer.
# Pattern [sparse_landmark x3, default] cycles evenly across the 36 layers (9 cycles), so the
# final layer is naturally full attention; force_full_attention_on_last_layer guarantees it
# regardless.
#
...
# Pair with the other Qwen3-4B-interleaved-* variants (alternating / 2sparse-1reg / 4sparse-1reg).
MEM_FREQ = 63
...
# Per-layer attention pattern: 3 sparse-landmark layers then 1 full layer, always ending with full.
...
def build_experiment_config(cli_context: CliContext) -> ExperimentConfig:
    ...
    # Qwen3-4B with the INTERLEAVED (3-sparse / 1-full) attention mixer. No YaRN.
...
if __name__ == "__main__":
    """
    Qwen3-4B + INTERLEAVED (3 sparse-landmark / 1 full) attention at 64k on the 15B
    dolma3_longmino sample (2 nodes, urgent). Every three sparse layers are followed by one full
    ...
    (sparse-landmark layers don't support it).
        ...
        python src/scripts/train/memexpress/cpt/interleaved/Qwen3-4B-interleaved-3sparse-1reg-dolma3longmino.py \\
