...
# This variant makes the FIRST HALF of the model full (normal) attention and the SECOND HALF
# sparse-landmark. Idea: let the early layers learn rich contextual information with full global
# attention, then the upper half only needs "reminders" -- each query sees its own chunk plus the
# past chunks' landmark tokens (sub-quadratic).
#
# SparseLandmarkAttention: a query attends fully within its own chunk but sees past chunks only
# through their landmark tokens. num_landmarks=1 matches LandmarkInstanceSource's 1-landmark-per-
# chunk data. The full-attention layers treat the landmark tokens as ordinary tokens.
#
...
# Pair with the other Qwen3-4B-interleaved-* variants (alternating / second-half-full).
MEM_FREQ = 63
...
# Per-layer attention pattern: first half full attention, second half sparse-landmark. The pattern
...
LAYER_TYPES = AttentionTypePatternConfig(
    pattern=[AttentionType.default] * HALF + [AttentionType.sparse_landmark] * (N_LAYERS - HALF),
)
...
def build_experiment_config(cli_context: CliContext) -> ExperimentConfig:
    ...
    # Qwen3-4B: first-half full attention, second-half sparse-landmark. No YaRN.
...
if __name__ == "__main__":
    ...
    First half of the layers are full attention (learn contextual info), second half are sparse-
    landmark (just need reminders). No context parallelism (sparse-landmark layers don't support it).

        python src/scripts/train/memexpress/cpt/interleaved/Qwen3-4B-interleaved-first-half-full-dolma3longmino.py \\
