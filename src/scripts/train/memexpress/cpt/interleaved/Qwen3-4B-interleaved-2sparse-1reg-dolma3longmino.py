...
# This variant cycles a 2-SPARSE / 1-REGULAR pattern: every two sparse-landmark layers are
# followed by one full (normal) attention layer, always ending with a full-attention layer.
# Pattern [sparse_landmark, sparse_landmark, default] cycles evenly across the 36 layers (12
# cycles), so the final layer is naturally full attention; force_full_attention_on_last_layer
...
# Pair with the other Qwen3-4B-interleaved-* variants (alternating / 3sparse-1reg / 4sparse-1reg).
MEM_FREQ = 63
...
# Per-layer attention pattern: 2 sparse-landmark layers then 1 full layer, always ending with full.
...
def build_experiment_config(cli_context: CliContext) -> ExperimentConfig:
    ...
    # Qwen3-4B with the INTERLEAVED (2-sparse / 1-full) attention mixer. No YaRN.
...
if __name__ == "__main__":
    """
    Qwen3-4B + INTERLEAVED (2 sparse-landmark / 1 full) attention at 64k on the 15B
    dolma3_longmino sample (2 nodes, urgent). Every two sparse layers are followed by one full
    layer, and the model always ends with a full-attention layer. No context parallelism
    (sparse-landmark layers don't support it).

        python src/scripts/train/memexpress/cpt/interleaved/Qwen3-4B-interleaved-2sparse-1reg-dolma3longmino.py \\
