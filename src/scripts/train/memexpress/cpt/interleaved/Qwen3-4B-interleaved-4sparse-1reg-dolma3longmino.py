...
# This variant cycles a 4-SPARSE / 1-REGULAR pattern: every 4 sparse-landmark layers are
# followed by one full (normal) attention layer, always ending with a full-attention layer.
# Pattern [sparse_landmark x4, default] cycles across the 36 layers (7 full cycles + 1 layer);
# layer 35 would land on sparse, so force_full_attention_on_last_layer overrides it to full
# attention.
#
...
# here -- each rank processes the full 64k sequence; FSDP shards params across the 16 GPUs.
# Pair with the other Qwen3-4B-interleaved-* variants (alternating / 2sparse-1reg / 3sparse-1reg).
MEM_FREQ = 63
...
# Per-layer attention pattern: 4 sparse-landmark layers then 1 full layer, always ending with full.
...
def build_experiment_config(cli_context: CliContext) -> ExperimentConfig:
    ...
    # Qwen3-4B with the INTERLEAVED (4-sparse / 1-full) attention mixer. No YaRN.
    ...
    train_module_config = TransformerTrainModuleConfig(
        ...
        dp_config=TransformerDataParallelConfig(
            ...
            # Shard params/grads/optim 8-way within each node (replicate across the 2 nodes). There
...
if __name__ == "__main__":
    """
    Qwen3-4B + INTERLEAVED (4 sparse-landmark / 1 full) attention at 64k on the 15B
    dolma3_longmino sample (2 nodes, urgent). Every four sparse layers are followed by one full
    layer, and the model always ends with a full-attention layer (forced on layer 35, which the
    pattern alone would make sparse). No context parallelism (sparse-landmark layers don't
    support it).

        python src/scripts/train/memexpress/cpt/interleaved/Qwen3-4B-interleaved-4sparse-1reg-dolma3longmino.py \\
