...
#
#   bash src/scripts/train/memexpress/goldgrad/reap_q06b_goldgrad_local.sh n100 [max_retries]
set -uo pipefail
RUNG="${1:-n100}"
MAX="${2:-2}"
...
for attempt in $(seq 1 "$MAX"); do
  ...
  ARMS="$missing" bash "$REPO/src/scripts/train/memexpress/goldgrad/run_q06b_goldgrad_local.sh" "$RUNG"
done
