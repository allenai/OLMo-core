...
#
#   bash src/scripts/train/memexpress/goldgrad/run_q06b_goldgrad_local.sh n100
#   bash src/scripts/train/memexpress/goldgrad/run_q06b_goldgrad_local.sh n20
set -uo pipefail
...
for arm in $ARMS; do
  ...
  if [ "${EVAL:-1}" = "1" ] && [ -f "$SAVE_ROOT/$RUN/model_and_optim/.metadata" ]; then
    ...
    EVAL_SIZE="$EVAL_SIZE" RUNS="$RUN" TAG="$TAG" CKPT_ROOT="$SAVE_ROOT" NGPU="$NGPU" \
      bash "$REPO/src/scripts/train/memexpress/goldgrad/eval_q06b_goldgrad_local.sh" "$RUNG" 2>&1 | grep -E "f1=|SKIP|rc=|Traceback"
