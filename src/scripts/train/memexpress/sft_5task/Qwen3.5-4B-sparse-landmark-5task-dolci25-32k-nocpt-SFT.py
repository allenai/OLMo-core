"""
Qwen3.5-4B SPARSE LANDMARK attention on the full-attention layers -- 32k-scale SFT on 75% the 5 long-context tasks (contradiction, nq, oolong,
rerank, outlier) / 25% ``allenai/Dolci-Instruct-SFT``, no CPT text, from the
``q35-4b-sparselm`` CPT checkpoint at step2385.
...
only selects the arm.
    ...
    PYTHONPATH=src python src/scripts/train/memexpress/sft_5task/Qwen3.5-4B-sparse-landmark-5task-dolci25-32k-nocpt-SFT.py \\
        dry_run q35-4b-sparselm-5task-dolci25-32k-nocpt ai2/jupiter
    PYTHONPATH=src python src/scripts/train/memexpress/sft_5task/Qwen3.5-4B-sparse-landmark-5task-dolci25-32k-nocpt-SFT.py \\
        launch  q35-4b-sparselm-5task-dolci25-32k-nocpt ai2/jupiter --launch.num_nodes=1
"""
...
def build_experiment_config(cli_context: CliContext) -> ExperimentConfig:
    """
    Build the sparse-landmark arm's experiment config.
