...
SEQUENCE_LENGTH = 32768  # 32k context (no landmark insertion); examples longer than this are
# dropped by PadToLengthInstanceSource. YaRN (factor 2) is kept so the model stays evaluable to
# 64k (extrapolating from 32k training).
...
# Raw Qwen3-4B-Base (NO CPT) to initialize from (model weights only) -- the same base the
# dolma3longmino CPT runs loaded, so this is a clean "skip the CPT stage" comparison.
...
if __name__ == "__main__":
    ...
    dense pretrain ckpt. The task is selected from the run name:
        ...
        python src/scripts/train/memexpress/sft_longctx/Qwen3-4B-dense-longctx-SFT.py \\
