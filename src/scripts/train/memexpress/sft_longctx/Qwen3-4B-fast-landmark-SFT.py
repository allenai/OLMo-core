...
# ---------------------------------------------------------------------------
# Single-epoch SFT for Qwen3-4B + FAST LANDMARK attention.
#
# Initializes model weights from the fast-landmark *pretrained* checkpoint
# (q4b-fast-landmark-dolma3longmino/step2385) and fine-tunes on the rlhn SFT set.
...
# attend across each other, matching the landmark pretraining regime (ConcatAndChunk).
# ---------------------------------------------------------------------------
...
# Fast-landmark pretrained checkpoint to initialize from (model weights only).
...
# Global batch in *tokens* (incl. landmark tokens). With rank_microbatch=SEQUENCE_LENGTH and
# 4 DP replicas, SEQUENCE_LENGTH * 32 = 2097152 (~2M tokens) -> 8 grad-accum steps. Kept equal
# across the dense/fast/sparse SFT runs for a clean comparison.
...
if __name__ == "__main__":
    """
    Single-epoch SFT of Qwen3-4B + fast-landmark attention from the fast-landmark pretrain ckpt.

        python src/scripts/train/memexpress/sft_longctx/Qwen3-4B-fast-landmark-SFT.py \\
            launch q4b-fast-landmark-sft ai2/jupiter-cirrascale-2
    """
