"""
256k-context Beaker/gantry SFT of the Qwen3.5-4B **interleaved landmark ``reg-sparse``** CPT model on
75% the xlong5 2k-256k 5-task mix (qboth build) / 25% ``allenai/Dolci-Instruct-SFT``.

Arm layout -- Alternating starting reg: reg, sparse, reg, sparse, ... ending ``sparse``. The opposite
phase of ``sparse-reg``.

Continues from ``q35-4b-il-regsparse-256k/step2385``. All shared configuration -- the 75/25 blend, the within-mix
...
This file only selects the arm.
    ...
    PYTHONPATH=src python $S launch_prep q35-4b-il-regsparse-xlong5-dolci25-256k-prep ai2/jupiter-cirrascale-2
