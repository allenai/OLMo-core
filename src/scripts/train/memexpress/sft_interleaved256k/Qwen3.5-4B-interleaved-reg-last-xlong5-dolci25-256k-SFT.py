"""
256k-context Beaker/gantry SFT of the Qwen3.5-4B **interleaved landmark ``reg-last``** CPT model on
75% the xlong5 2k-256k 5-task mix (qboth build) / 25% ``allenai/Dolci-Instruct-SFT``.

Arm layout -- ``sparse`` on the first seven full-attention layers, ``reg`` (fast_compressive_landmark)
on the LAST (31). The 1/7 counterpart to ``reg-first``.

Continues from ``q35-4b-il-reglast-256k/step2385``. All shared configuration -- the 75/25 blend, the within-mix
...
This file only selects the arm.
    ...
    S=src/scripts/train/memexpress/sft_interleaved256k/Qwen3.5-4B-interleaved-reg-last-xlong5-dolci25-256k-SFT.py

    PYTHONPATH=src python $S dry_run q35-4b-il-reglast-xlong5-dolci25-256k ai2/jupiter-cirrascale-2
    ...
    PYTHONPATH=src python $S launch_prep q35-4b-il-reglast-xlong5-dolci25-256k-prep ai2/jupiter-cirrascale-2

    PYTHONPATH=src python $S launch q35-4b-il-reglast-xlong5-dolci25-256k \\
