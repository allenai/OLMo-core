"""
Stage 1a: sample 50 real documents per context-length bucket from each distinct SFT training-data
group in ``models.DATA_GROUPS``, ONCE, and materialize them (token ids + label mask) to weka so
every model that shares a data group scores the identical documents.
...
data" -- it does NOT replay the packer's mixing ratios or windowing, so a group's bucket population
is NOT reweighted to match the training mixture's task proportions. It also does not exercise any
...
N_PER_BUCKET per bucket -- so document identity is fixed independent of how many documents exist
upstream. Only the sampled documents' tokens are then materialized via get_token_range().
...
def build_group_manifest(group_name: str, work_dir: str) -> None:
    ...
    # reservoir[bucket] = list of (task, source_idx, doc_start, doc_end, length)
    reservoir: Dict[str, List[Tuple[str, int, int, int, int]]] = {
        lab: [] for lab in LENGTH_LADDER_LABELS
    }
    seen_count: Dict[str, int] = {lab: 0 for lab in LENGTH_LADDER_LABELS}
        ...
        for src_idx, src in enumerate(sources):
            ...
            for doc_start, doc_end in src.get_document_offsets():
                ...
                if bucket is None:
                    continue
                seen_count[bucket] += 1
                item = (task, src_idx, doc_start, doc_end, length)
                if len(reservoir[bucket]) < N_PER_BUCKET:
                    reservoir[bucket].append(item)
                else:
                    j = rng.randint(0, seen_count[bucket] - 1)
                    if j < N_PER_BUCKET:
                        reservoir[bucket][j] = item
            print(f"    shard {src_idx}: {n_docs} documents scanned", flush=True)

    print(
        f"[{group_name}] offsets scan done in {time.time() - t0:.1f}s. "
        f"bucket counts (population -> sampled): "
        + ", ".join(
            f"{lab}={seen_count[lab]}->{len(reservoir[lab])}" for lab in LENGTH_LADDER_LABELS
        ),
        flush=True,
    )
    ...
    for bucket in LENGTH_LADDER_LABELS:
        for i, (task, src_idx, doc_start, doc_end, length) in enumerate(reservoir[bucket]):
            src = built_sources[(task, src_idx)]
            tr = src.get_token_range(doc_start, doc_end)  # {"input_ids": ..., "label_mask": ...}
            key = f"{bucket}__{i}"
            npz_payload[f"{key}__ids"] = np.asarray(tr["input_ids"], dtype=np.int64)
            label_mask = tr.get("label_mask")
            if label_mask is None:
                raise RuntimeError(
                    f"{group_name}/{task}: source has no label_mask -- every training source in "
        ...
        json.dump(
            {
                ...
                "population_by_bucket": seen_count,
                "sampled_by_bucket": {lab: len(reservoir[lab]) for lab in LENGTH_LADDER_LABELS},
                "examples": index,
            },
            f,
            indent=2,
