    ...
    return TransformerTrainModuleConfig(
        ...
        optim=MuonWithAuxAdamConfig(
            group_overrides=[
                # Muon Params
                OptimGroupOverride(
                    params=["blocks.*.feed_forward.*.weight"], opts=dict(use_muon=True, lr=lr, momentum=0.95)
                ),
                OptimGroupOverride(
                    params=["blocks.*.attention.w_*.weight"], opts=dict(use_muon=True, lr=lr, momentum=0.95)
                ),
                # Adam Params
                OptimGroupOverride(
                ...
                OptimGroupOverride(
                    params=["*norm.weight"], opts=dict(use_muon=False, lr=lr, betas=(0.9, 0.95), weight_decay=0.0)
                ),
            ],
        ),
        compile_model=True,
