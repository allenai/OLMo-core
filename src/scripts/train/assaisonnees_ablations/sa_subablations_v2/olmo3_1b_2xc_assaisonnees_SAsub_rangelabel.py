...
from olmo_core.internal.experiment import CliContext, ExperimentConfig, get_beaker_username
from olmo_core.internal.experiment import main as olmo_core_main
...
DATASET_CONFIG = SourceMixtureList(
    sources=[
        SourceMixtureConfig(
            source_name="sa_rangelabel_filter",
            target_ratio=1.0,
            paths=[
                f"s3://ai2-llm/preprocessed/cc_all_dressed/all_dressed_v5/ablation_pools/suffix_arrays/2rep_5rep_v2/sa_reshard_range-labeled_filtered/allenai/dolma2-tokenizer/*.npy"
            ],
        )
    ]
)
