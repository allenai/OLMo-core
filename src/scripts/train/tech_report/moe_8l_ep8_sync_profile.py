"""Standalone eight-layer, 128-expert synchronized-1D EP8 profile."""
...
def _set_default_triton_cache_dir() -> None:
    ...
    job_id = os.environ.get("JOB_ID", "moe-8l-ep8-sync-profile")
...
def build_trainer_config(common: CommonComponents) -> TrainerConfig:
    return (
        TrainerConfig(
            save_folder=str(
                Path("/workspace/checkpoint/tech_report") / f"{common.run_name}-ep8-sync-profile"
            ),
            save_overwrite=True,
            no_checkpoints=True,
            no_evals=True,
