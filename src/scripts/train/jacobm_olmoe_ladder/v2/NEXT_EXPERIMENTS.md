...
| 8 | LatentMoE | On promoted KDA, project only the routed branch to a smaller latent width while holding expert count/top-k and every non-MoE setting fixed. Start with 2× and paper-standard 4× compression; treat paper-exact full-width routing plus expert/top-k scaling as a separate follow-up. | PR #799's nine-commit implementation is ported and its focused local tests pass. Audited 275M 2×/4× configs and a no-checkpoint EP1 smoke manifest are ready; full runs and validation remain deferred. See [`LATENT_MOE.md`](LATENT_MOE.md). |
