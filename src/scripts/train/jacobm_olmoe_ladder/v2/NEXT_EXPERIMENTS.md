...
| 8 | LatentMoE | On promoted KDA, project only the routed expert branch to a smaller latent width while holding expert count/top-k and every non-MoE setting fixed. Start with 2× and paper-standard 4× compression, and compare PR #799's latent-width routing against paper/Kimi-style full-width routing. | PR #799's nine-commit implementation is ported; the full-width-router correction and audited 275M 2×/4× configs pass focused local tests. Separate commit-pinned, no-checkpoint EP1 qualification pairs cover both router semantics; full runs and validation remain deferred. See [`LATENT_MOE.md`](LATENT_MOE.md). |
