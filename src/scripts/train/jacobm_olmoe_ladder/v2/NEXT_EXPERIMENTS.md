...
| 6 | Gated DeltaNet 2 (GDN2) | Starting from the geometry-matched gated-RoPE parent, replace only GDN v1 with GDN2. Hold attention geometry, MoE, optimization, `expand_v`, convolution, and other model settings fixed; audit the active-parameter increase before launch. | Native adapter and isolated pinned-FLA overlay implemented. Functional smoke, 4/4 MB8/MB16 capacity cells, and the 16/16 GDN1/SWA-matched throughput matrix passed. GDN2 is 11--13% slower in raw TPS than GDN1; an LR sweep remains unlaunched. |
