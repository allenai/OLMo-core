...
| 6 | Gated DeltaNet 2 (GDN2) | Starting from the geometry-matched gated-RoPE parent, replace only GDN v1 with GDN2. Hold attention geometry, MoE, optimization, `expand_v`, convolution, and other model settings fixed; audit the active-parameter increase before launch. | Native adapter and isolated pinned-FLA overlay implemented. The compiled 275M MB1 training smoke passed; MB8/MB16 throughput qualification is in progress before an LR sweep. |
