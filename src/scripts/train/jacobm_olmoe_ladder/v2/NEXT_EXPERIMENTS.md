...
|---:|---|---|---|
| 1 | GDN hybrid | On wide, replace sliding-attention layers with GatedDeltaNet; keep geometry, global-attention placement, RoPE, initialization, and `expand_v=1` fixed. | 275M sweep and 480M/810M scale cells are complete; 1.2B Cx4/Cx8 remain in progress. |
...
| 3 | NoPE | On the aligned-geometry recipe, remove RoPE only from global-attention layers and train from initialization. | 275M sweep complete. Larger wave is 9/12 complete; 810M Cx8 and 1.2B Cx4 are running, while 1.2B Cx8 is stopped after reproducing a non-finite-grad failure. |
| 4 | Full-attention gating, then exact head geometry | First add only the dense ladder's elementwise full-precision gate to the 275M NoPE model while retaining 8-Q/4-KV GQA. If useful, separately test the dense 8-Q/8-KV attention shape. | 275M gated sweep complete. Larger wave is 6/12 complete; five cells are running and 1.2B Cx2 is stopped on a non-finite grad norm. Exact KV-head geometry remains deferred. |
