...
| 3 | NoPE | On the aligned-geometry recipe, remove RoPE only from global-attention layers and train from initialization. | 275M sweep complete. Larger wave has 11/12 original cells complete; the fresh 1.2B Cx8 collapse-monitor reproduction is running at 67.1%. |
...
| 5 | RoPE × attention-gating interaction | Starting from the gated NoPE geometry model, restore RoPE only on full-attention layers 4 and 9. Keep GQA, gating, geometry, initialization, data, and optimization fixed. | 275M sweep complete. Larger wave has 7 finished, 4 running, and 1 repeatedly failed 1.2B Cx2 cell. |
