...
## Live status snapshot (2026-07-20 06:12 UTC)
...
| pretraining | first hybrid 1.2B Cx8 | running | 120.40B / 181.52B tokens (66.3%); ~529 TFLOPs/GPU | [7eemhu7g](https://wandb.ai/ai2-llm/jacobm-olmoe-ladder/runs/7eemhu7g) |
...
| pretraining | larger aligned geometry + NoPE | running | 10/12 finished; 1.2B Cx4 at 75.08B / 92.88B (80.8%); 1.2B Cx8 is stopped after its third non-finite-grad failure | [results](results/pretraining/geometry_gdn_ev2_nope/results.md) |
| pretraining | larger aligned geometry + NoPE + gated attention | running | 8/12 finished; 810M Cx8 at 96.5%, 1.2B Cx4/Cx8 are running, and 1.2B Cx2 is stopped after its second non-finite-grad failure | [results](results/pretraining/geometry_gdn_ev2_nope_gated/results.md) |
| midtraining | first hybrid 275M Cx8 | finished | 100B; final checkpoint `step95368`; validation backfill queued | [1keo2hz6](https://wandb.ai/ai2-llm/jacobm-olmoe-ladder/runs/1keo2hz6) |
| midtraining | first hybrid 480M Cx8 | finished | 100.001B; final checkpoint `step95368`; validation backfill queued | [mnp9rv5l](https://wandb.ai/ai2-llm/jacobm-olmoe-ladder/runs/mnp9rv5l) |
| validation | V2 post-training backfills | running / queued | 33 previous targets finished; 58 newly eligible targets submitted in three batches | [results](results/validation/hybrid_full.md) |

The formal pretraining results and plots use finished runs only and enforce a
complete final-250M-token history. No new pretraining cell finished during this
refresh. The completed NoPE and gated-attention sweeps, eligible larger-scale
cells, evaluator-free first-hybrid scale cells, and both finished hybrid
midtraining checkpoints are now registered in post-training validation
backfills. The completed 480M/810M first-hybrid Cx1/Cx2 runs already contain
the full 178-task validation suite and were not duplicated.
...
strict final-250M CE `2.119848`, versus `2.104939` for wide integration and
`2.095585` for the first hybrid. The 1.2B Cx4 worker is at 75.08B / 92.88B
tokens (80.8%), at about 157 TFLOPs/GPU. The user-requeued 1.2B Cx8 attempt resumed from the existing
checkpoint, reached step 17,644, and failed for a third time on the same
`Non-finite total grad norm` assertion; it is stopped with durable
`step17500`. The all-size fixed-LR
plot includes the ten finished cells and labels the two unresolved cells.
...
At the 2026-07-20 06:12 UTC refresh, all 480M cells, 810M Cx1/Cx2/Cx4, and
1.2B Cx1 are finished, for 8/12 completed cells. Newly collected strict
final-250M CE is `2.191179` for 810M Cx4 and `2.273007` for 1.2B Cx1. The
810M Cx8 worker is at 117.60B / 121.88B (96.5%); 1.2B Cx4 and Cx8 are at
70.64B / 93.72B (75.4%) and 66.88B / 187.44B (35.7%), respectively. All four 1.2B cells were initially
...
The identical retry stopped again at step 20,969 on the same assertion and is
now stopped with durable `step20500`. Exact retry links are in
