...
INTEGRATION_SPECS = {
    ...
    "deep_256e8k": IntegrationSpec(
        ...
        tag="intd256e8k",
        description="Deep integration candidate: 256E/top8, 0.375d routed experts, 0.375d shared expert, one dense prefix layer, depth-expanded.",
    ),
}
...
def get_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default="275m",
        help="MoE A0 active-parameter scale to train. The first integration-test wave uses 275m.",
    )
    parser.add_argument(
        "--integration-config",
        choices=sorted(INTEGRATION_SPECS),
        default="wide_256e8k",
        help="Dedicated integration MoE geometry variant.",
    )
    parser.add_argument(
        "--expert-geometry",
...
def configure_model_size(opts: argparse.Namespace) -> None:
    ...
    integration = INTEGRATION_SPECS[opts.integration_config]
    if (
        opts.expert_geometry != "baseline_48e_top4"
        or opts.total_sparsity != "baseline_48e_top4"
        or opts.shared_expert_config != "baseline"
        or opts.dense_schedule != "dense1_shared"
    ):
        raise ValueError(
            "integration_ladder.py is dedicated to the combined integration-test geometry axis; "
            "leave expert geometry, total sparsity, shared expert, and dense schedule at defaults"
        )

    NUM_EXPERTS = integration.num_experts
    ...
    MOE_HIDDEN_SIZE = int(round(spec.d_model * integration.moe_hidden_mult))
    NUM_SHARED_EXPERTS = integration.num_shared_experts
    SHARED_MLP_HIDDEN_SIZE = int(round(spec.d_model * integration.shared_hidden_mult))
    ...
    DENSE_BLOCK_LAYERS = integration.dense_layers
    NUM_LAYERS = integration.layer_overrides.get(opts.model_size, spec.n_layers)
    EXPERT_GEOMETRY_TAG = integration.tag
    if DENSE_BLOCK_LAYERS and max(DENSE_BLOCK_LAYERS) >= NUM_LAYERS:
...
def consume_script_overrides(opts: argparse.Namespace, overrides: List[str]) -> List[str]:
    ...
    for override in overrides:
        if override.startswith("model_size="):
            ...
            continue
        if override.startswith("integration_config="):
            opts.integration_config = override.split("=", 1)[1]
            continue
    ...
    if opts.integration_config not in INTEGRATION_SPECS:
        raise ValueError(f"--integration-config must be one of {sorted(INTEGRATION_SPECS)}")
