...
LR_SPECS="${LR_SPECS:-8e-4:lr8e-4 1.6e-3:lr1.6e-3 3.2e-3:lr3.2e-3}"
...
launch_one() {
  ...
  local lr="$3"
  local lr_tag="$4"
  local name="${RUN_PREFIX}-${sp_tag}-${lr_tag}-${SWEEP_SUFFIX}"
  ...
  local cmd=(
    ...
    --tag="${sp_tag}-cx8-${lr_tag}-${SWEEP_SUFFIX}"
...
for spec in "${variant_specs[@]}"; do
  ...
  for lr_spec in ${LR_SPECS}; do
    launch_one "${total_sparsity}" "${sp_tag}" "${lr_spec%%:*}" "${lr_spec##*:}"
  done
done
