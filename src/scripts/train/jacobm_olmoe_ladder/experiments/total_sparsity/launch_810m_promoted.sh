...
LOG_DIR="${LOG_DIR:-/tmp/olmoe3-total-sparsity-810m-promoted-launch-logs}"
...
launch_one() {
  ...
  local name="${RUN_PREFIX}-810m-cx${cx}-${sp_tag}-${lr_tag}-${SWEEP_SUFFIX}"
  local log_path="${LOG_DIR}/${name}.log"
  local systems_tag="${batch_tag}-n${NUM_NODES}gpu${gpus}-ep${ep_dim}mb${micro_bsz}"
  ...
  local cmd=(
    ...
    --tag="${sp_tag}-810m-cx${cx}-${lr_tag}-${SWEEP_SUFFIX}"
...
launch_variant() {
  ...
  case "${sp_tag}" in
    sp96e4k)
      launch_if_enabled "${total_sparsity}" "${sp_tag}" 1 b256k 32 8 4 1 5e-4 lr5e-4
      launch_if_enabled "${total_sparsity}" "${sp_tag}" 2 b384k 48 8 2 1 5e-4 lr5e-4
      launch_if_enabled "${total_sparsity}" "${sp_tag}" 4 b512k 64 8 4 1 3.5e-4 lr3.5e-4
      launch_if_enabled "${total_sparsity}" "${sp_tag}" 8 b768k 96 8 4 1 3.5e-4 lr3.5e-4
      ;;
    sp192e4k)
      launch_if_enabled "${total_sparsity}" "${sp_tag}" 1 b256k 32 8 1 1 4e-4 lr4e-4
      launch_if_enabled "${total_sparsity}" "${sp_tag}" 2 b384k 48 8 1 1 4e-4 lr4e-4
      launch_if_enabled "${total_sparsity}" "${sp_tag}" 4 b512k 64 8 1 1 3e-4 lr3e-4
      launch_if_enabled "${total_sparsity}" "${sp_tag}" 8 b768k 96 8 1 1 3e-4 lr3e-4
      ;;
    *)
