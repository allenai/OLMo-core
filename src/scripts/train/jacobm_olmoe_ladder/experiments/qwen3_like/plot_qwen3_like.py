#!/usr/bin/env python
"""Plot Qwen3-like geometry U-curves from W&B history."""
...
EXPERIMENT_TAG = "exp_qwen3_like"
OUTPUT_SUBDIR = "qwen3_like"
PLOT_TITLE = "Qwen3-like geometry"
VARIANT_LABELS = {
...
SUMMARY_VARIANTS = [
    ...
    SummaryVariant("active_matched", ("active_matched",), "Qwen-like active matched 4.5d"),
    SummaryVariant(
        "true_3d_depth_matched",
        ("true_3d_depth_matched",),
        "Qwen-like true 3.0d + depth",
    ),
]
...
def model_from_name(name: str) -> str | None:
    qwen_match = re.search(r"q3-(275m|480m|810m|1p2b)-cx", name)
    if qwen_match:
...
def experiment_variant_name(name: str) -> str | None:
    if "q3am128e8k" in name:
        return "active_matched"
    if "q3td128e8k" in name:
        return "true_3d_depth_matched"
    return None
        ...
        api.runs(
            ...
            filters={
                "$or": [
                    ...
                    {"display_name": {"$regex": "q3-(275m|480m|810m|1p2b)-cx(1|2|4|8)-q3"}},
...
def main() -> None:
    ...
    plot_observed_best_summary(
        ...
        title="Qwen3-like geometry observed best",
