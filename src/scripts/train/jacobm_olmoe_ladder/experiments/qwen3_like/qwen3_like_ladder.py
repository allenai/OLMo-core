...
QWEN3_LIKE_SPECS = {
    "active_matched": Qwen3LikeSpec(
        num_experts=128,
        top_k=8,
        moe_hidden_mult=9 / 16,
        dense_layers=(),
        layer_overrides={},
        tag="q3am128e8k",
        description="Qwen3-style 128E/top8 routing with active FFN width matched to baseline 4.5d.",
    ),
    "true_3d_depth_matched": Qwen3LikeSpec(
        num_experts=128,
        top_k=8,
        moe_hidden_mult=3 / 8,
        dense_layers=(),
...
def get_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default="275m",
        help="MoE A0 active-parameter scale to train. The first Qwen3-like wave uses 275m.",
    )
    parser.add_argument(
        "--qwen3-like",
        choices=sorted(QWEN3_LIKE_SPECS),
        default="active_matched",
        help="Dedicated Qwen3-like MoE geometry variant.",
    )
    parser.add_argument(
        "--expert-geometry",
...
def configure_model_size(opts: argparse.Namespace) -> None:
    ...
    qwen3_like = QWEN3_LIKE_SPECS[opts.qwen3_like]
    if (
        ...
        raise ValueError(
            "qwen3_like_ladder.py is dedicated to the combined Qwen3-like geometry axis; "
            "leave expert geometry, total sparsity, shared expert, and dense schedule at defaults"
        )

    NUM_EXPERTS = qwen3_like.num_experts
    TOP_K = qwen3_like.top_k
    ...
    MOE_HIDDEN_SIZE = int(round(spec.d_model * qwen3_like.moe_hidden_mult))
    NUM_SHARED_EXPERTS = 0
    SHARED_MLP_HIDDEN_SIZE = 0
    EFFECTIVE_MLP = MOE_HIDDEN_SIZE * TOP_K
    MLP_RATIO = EFFECTIVE_MLP / D_MODEL
    DENSE_LAYER_MLP = spec.dense_layer_mlp
    DENSE_BLOCK_LAYERS = qwen3_like.dense_layers
    NUM_LAYERS = qwen3_like.layer_overrides.get(opts.model_size, spec.n_layers)
    EXPERT_GEOMETRY_TAG = qwen3_like.tag
...
def consume_script_overrides(opts: argparse.Namespace, overrides: List[str]) -> List[str]:
    ...
    for override in overrides:
        if override.startswith("model_size="):
            ...
            continue
        if override.startswith("qwen3_like="):
            opts.qwen3_like = override.split("=", 1)[1]
            continue
    ...
    if opts.qwen3_like not in QWEN3_LIKE_SPECS:
        raise ValueError(f"--qwen3-like must be one of {sorted(QWEN3_LIKE_SPECS)}")
