...
RUN_PREFIX="${RUN_PREFIX:-q3-smoke}"
CHECKPOINT_ROOT="${CHECKPOINT_ROOT:-/weka/oe-training-default/ai2-llm/checkpoints/jacobm/olmoe3/qwen3_like_smoke}"
LOG_DIR="${LOG_DIR:-/tmp/olmoe3-qwen3-like-smoke-launch-logs}"
...
launch_one() {
  ...
  if (( global_batch_size_seq % denom != 0 )); then
    echo "Invalid batch settings for ${qwen_variant}: global_batch_size_seq=${global_batch_size_seq} is not divisible by nodes*gpus*micro_bsz=${denom}" >&2
    exit 1
  fi

  local name="${RUN_PREFIX}-${qwen_tag}-${lr_tag}-${SWEEP_SUFFIX}"
  ...
  local cmd=(
    ...
    --tag="${qwen_tag}-smoke-${lr_tag}-${SWEEP_SUFFIX}"
...
for qwen_variant in active_matched true_3d_depth_matched; do
  qwen_tag="$(qwen_tag_for "${qwen_variant}")"
  launch_one "${qwen_variant}" "${qwen_tag}" 2e-3 lr2e-3
done
