...
launch_one() {
  ...
  local systems_tag="b768k-gpu${GPUS}-ep${EP_DIM}mb${micro_bsz}"
  ...
  local cmd=(
    ...
    --tag="${eg_tag}-cx8-${lr_tag}-${SWEEP_SUFFIX}"
...
for spec in coarse_24e_top2:eg24e2k fine_96e_top8:eg96e8k; do
  ...
  launch_one "${expert_geometry}" "${eg_tag}" 8e-4 lr8e-4 "${micro_bsz}"
  launch_one "${expert_geometry}" "${eg_tag}" 1.6e-3 lr1.6e-3 "${micro_bsz}"
  launch_one "${expert_geometry}" "${eg_tag}" 3.2e-3 lr3.2e-3 "${micro_bsz}"
done
