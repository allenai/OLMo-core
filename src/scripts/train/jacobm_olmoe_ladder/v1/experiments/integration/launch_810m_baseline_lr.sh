...
launch_one() {
  ...
  if ! should_launch_cx "${cx}"; then
    echo "Skipping 810m ${INTEGRATION_TAG} Cx${cx}; CX_VALUES=${CX_VALUES}"
    return 0
  fi

  local denom=$((NUM_NODES * gpus * micro_bsz))
  if (( global_batch_size_seq % denom != 0 )); then
    echo "Invalid batch settings for 810m ${INTEGRATION_TAG} Cx${cx}: global_batch_size_seq=${global_batch_size_seq} is not divisible by nodes*gpus*micro_bsz=${denom}" >&2
    exit 1
  fi
  ...
  local cmd=(
    ...
    --tag="${INTEGRATION_TAG}-810m-cx${cx}-${lr_tag}-${SWEEP_SUFFIX}"
...
launch_one 1 b256k 32 8 4 6e-4 lr6e-4
launch_one 2 b384k 48 8 2 5.6e-4 lr5.6e-4
launch_one 4 b512k 64 8 4 4e-4 lr4e-4
launch_one 8 b768k 96 8 4 4e-4 lr4e-4
