...
EVAL_INTERVAL = 2000
SAVE_INTERVAL = 20   # smoke: write a checkpoint mid-run to exercise the checkpointer
...
HEAD_DIM=128
NUM_HEAD = D_ATTN // HEAD_DIM          # 8 heads
NUM_KV_HEAD= NUM_HEAD // 4             # 2 KV heads (GQA 4:1)
MOE_HIDDEN_SIZE = 512
...
# SMOKE TEST: world = DP x EP x PP = 2 x 2 x 1 = 4 GPUs (single node, 4 of 8)
EP_DIM=2
PP_DIM=1

# ref (single node; launch with --launch.num_nodes=1 --launch.num_gpus=4)
REF_NUM_NODES=1
TAG=f'smoke'

LR_ALPHA = 0.53

# smoke: ~38 steps, tiny 0.13M-token global batch so steps are fast
MAX_DURATION = int(5e6)
MICRO_BSZ = 2
GLOBAL_BATCH_SIZE_SEQ=32   # 32 seq x 4096 = 131072 tokens/step (divisible by DP=2 or 4)
# NO LR_REF_BSZ=16M
...
SCHED_WARMUP_TOKENS = int((1e6 // GLOBAL_BATCH_SIZE) * GLOBAL_BATCH_SIZE)
SCHED_FAST_DECAY_TOKENS = int((0e9 // GLOBAL_BATCH_SIZE) * GLOBAL_BATCH_SIZE)
SCHED_LONG_DECAY_TOKENS = int((5e6 // GLOBAL_BATCH_SIZE) * GLOBAL_BATCH_SIZE)
...
USE_MUON = False
USE_PERI_NORM = True
PRODUCTION_RUN = False  # test run: skip the pre-train checkpoint
    ...
    config = (
        ...
        .with_callback(
            ...
            WandBCallback(
                ...
                entity=None,             # use the default entity tied to WANDB_API_KEY (your account)
                project="olmoe-rb-test", # personal test project, off the shared board
                enabled=True,
                cancel_check_interval=cancel_check_interval,
            ),
        )
