#!/usr/bin/env bash
# Download Qwen/Qwen3-4B-Base from HuggingFace to Weka and convert to OLMo-core format via Gantry.
#
...
#
# Output: /weka/oe-training-default/ai2-llm/checkpoints/amandab/Qwen3-4B-Base-olmocore
#
...
WEKA_ROOT="/weka/${WEKA}"
HF_STAGING="${WEKA_ROOT}/ai2-llm/checkpoints/amandab/Qwen3-4B-Base-hf"
OUTPUT_DIR="${WEKA_ROOT}/ai2-llm/checkpoints/amandab/Qwen3-4B-Base-olmocore"

gantry run \
  --name "qwen3-4b-base-hf2olmocore" \
  --description "Download Qwen3-4B-Base from HF and convert to OLMo-core checkpoint" \
  --workspace "${WORKSPACE}" \
  --budget "${BUDGET}" \
  --cluster "${CLUSTER}" \
  --gpus 0 \
