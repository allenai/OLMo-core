...
def vit_pf(nc):
    return (
        nc * P * (2 * m._n_vision_params + 4 * nl * P * d) * 3
...
(d1, a1, v1, t1, s1) = vals["crops=25"]
(d2, a2, v2, t2, s2) = vals["crops=80"]
lm1, lm2 = d1 + a1, d2 + a2
...
print(
    "implied ViT throughput: %.3f PF/s  (%.1fx less efficient)"
    % (1 / inv_vit / 1e15, inv_vit / inv_lm)
)
print()
for name, (dd, aa, vv, tt, ss) in vals.items():
    vt = vv * inv_vit
    print(
        "%-9s predicted ViT time share: %.1f%%  (FLOP share %.1f%%)"
