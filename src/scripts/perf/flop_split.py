...
def vit(nc):
    raw = nc * P
    return raw * (2 * m._n_vision_params + 4 * L * P * d) * 3 + nc*POOLED*6*m._n_connector_params

# (label, crops/pack, packs per rank, real tokens/pack, step seconds)
ARMS = [("crops=25", 24, 16, 5046, 9.62), ("crops=80", 79, 6, 16180, 6.17)]
print("%-10s %8s %10s %10s %10s %10s" % ("arm","PF/pack","PF/s tot","PF/s LM-useful","tok occ","rank packs"))
res={}
for name, nc, packs, real, sec in ARMS:
    ...
    print("%-10s %8.4f %10.3f %14.3f %9.1f%% %10d" % (
        name, (lm_all+vit(nc))/1e15, tot/sec/1e15, useful/sec/1e15, 100*real/S, packs))
a,b = res["crops=25"], res["crops=80"]
print()
print("total FLOP throughput   : %.3f -> %.3f PF/s  = %+.0f%%" % (a[0]/1e15, b[0]/1e15, 100*(b[0]/a[0]-1)))
print("USEFUL LM FLOP throughput: %.3f -> %.3f PF/s  = %+.0f%%" % (a[1]/1e15, b[1]/1e15, 100*(b[1]/a[1]-1)))
print()
lm_all = lm_fpt*S
for nc in (24,79):
    fro = lm_all + nc*P*(2*m._n_vision_params+4*L*P*d) + nc*POOLED*6*m._n_connector_params
    tra = lm_all + vit(nc)
    print("crops=%-3d MFU undercount from frozen-ViT accounting: %.1f%% relative "
