...
class TritonConfig:
    ...
    def name(self) -> str:
        return (
            f"triton_bm{self.block_m}_bn{self.block_n}" f"_rp{self.row_programs}_w{self.num_warps}"
        )
...
def main() -> None:
    ...
    valid_gib = (
        args.valid_rows * args.hidden * 3 * torch.empty((), dtype=dtype).element_size() / 1024**3
    )
    print(
