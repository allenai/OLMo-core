...
def convert_to_hf_checkpoint(
    ...
    if has_qk_norm:
        ...
        if rope_config and "factor" in rope_config:
...
def validate_conversion(hf_model_path, olmo_checkpoint_path, olmo_config):
    ...
    if "rope" in olmo_config_dict["block"]["attention"] and "scaling" in olmo_config_dict["block"]["attention"]["rope"]:
        scaling_data = olmo_config_dict["block"]["attention"]["rope"]["scaling"]
        if isinstance(scaling_data, dict) and "factor" in scaling_data:
            olmo_config_dict["block"]["attention"]["rope"]["scaling"] = RoPELinearScalingConfig(**scaling_data)
