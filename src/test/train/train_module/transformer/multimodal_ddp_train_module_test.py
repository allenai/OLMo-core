...
def test_vision_model_load_synchronizes_only_trainable_optimizer_masters(frozen):
    ...
    calls = []
    module.optim = SimpleNamespace(
        param_groups=[
            {"named_params": {f"vision.{name}": param for name, param in vision.named_parameters()}}
        ],
        _copy_model_params_to_main_params=lambda names: calls.append(("copy", names)),
        _check_model_param_main_param_the_same=lambda names: calls.append(("check", names)),
    )
...
def test_vision_optimizer_mapping_rejects_missing_trainable_parameters():
    module = object.__new__(MultimodalOLMoDDPTrainModule)
    vision = torch.nn.Linear(2, 2)
    module.model_parts = [SimpleNamespace(vision=vision)]
    module.optim = SimpleNamespace(
        param_groups=[{"named_params": {"vision.weight": vision.weight}}]
    )

    with pytest.raises(RuntimeError, match="found 1 of 2"):
        module._trainable_vision_param_names()
