...
def test_attention(
    ...
    if window_size is not None and not use_flash and not use_flex:
...
def test_flex_attention_against_sdpa(device: torch.device, dtype: torch.dtype):
    ...
    flex_att = Attention(use_flex=True, **kwargs)
...
def test_sdpa(
    ...
    if not use_flex and dtype == torch.float32:
    ...
    if not use_flash and not use_flex and window_size is not None:
        pytest.skip("sliding window attention is not supported by torch SDPA")

    if not use_flash and not use_flex and intra_doc_masking:
    ...
    attention = Attention(use_flash=use_flash, use_flex=use_flex, **kwargs)
    block_mask = None
    if use_flex:
        block_mask = get_flex_attn_causal_block_mask(
            seq_len,
