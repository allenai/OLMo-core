...
# backward needs B * (T / 64) * HV >= 1024 of its own. Every experimental-kernel arm below is sized to
...
_EXP_B, _EXP_T, _EXP_HEADS, _EXP_DMODEL = 4, 1024, 16, 256


def _skip_unless_experimental_kernels() -> None:
    ...
    q = torch.empty(_EXP_B, _EXP_T, _EXP_HEADS, 128, **kw)
    v = torch.empty(_EXP_B, _EXP_T, _EXP_HEADS, 256, **kw)
    ok, reason = is_supported(q, v, use_qk_l2norm_in_kernel=True, use_gate_in_kernel=True)
    if not ok:
        pytest.skip(f"the experimental KDA kernels decline this box: {reason}")


def _set_use_experimental_kernels(module, use_experimental: bool) -> None:
    """Flip the one experimental-kernels flag on a built layer, convolutions included.

    ``use_experimental_kernels`` is a single switch over both kernel families, so a test that flips
    it after ``build()`` has to reach the three ``CausalConv1d`` children too — otherwise
    the arm labelled "experimental" would still be running FLA's short conv.
    """
    module.use_experimental_kernels = use_experimental
    for conv in (module.q_conv1d, module.k_conv1d, module.v_conv1d):
        conv.use_experimental_kernels = use_experimental
...
@requires_fla
@requires_gpu
def test_kimi_delta_attention_experimental_kernels_under_torch_compile():
    """The train module compiles each block; the experimental host path must graph-break cleanly
    (dynamo tracing it used to die on torch.cuda.current_stream().cuda_stream)."""
    _skip_unless_experimental_kernels()

    device = "cuda"
    dtype = torch.bfloat16
    d_model, seq_len, batch_size = _EXP_DMODEL, _EXP_T, _EXP_B
    torch.manual_seed(0)
    config = KimiDeltaAttentionConfig(
        n_heads=_EXP_HEADS, head_dim=128, expand_v=2.0, use_experimental_kernels=True
    )
    module = config.build(d_model, layer_idx=0, n_layers=12, init_device=device)
...
@requires_fla
@requires_gpu
def test_kimi_delta_attention_experimental_kernels_extreme_decay():
    ...
    B, T, H, K, V = _EXP_B, _EXP_T, _EXP_HEADS, 128, 256
    torch.manual_seed(0)
    ...
    results = {}
    for use_experimental in (False, True):
        ...
        o, _ = dispatch_chunk_kda(
            ...
            use_experimental_kernels=use_experimental,
        )
        (o.float() * do.float()).sum().backward()
        grads = [t.grad for t in leaves]
        for gr in [o, *grads]:
            assert gr is not None and torch.isfinite(gr).all()
        results[use_experimental] = (o.detach(), grads)

    o_fla, g_fla = results[False]
    o_exp, g_exp = results[True]
    torch.testing.assert_close(o_exp, o_fla, atol=5e-3, rtol=5e-3)
    for gc, gf in zip(g_exp, g_fla):
        torch.testing.assert_close(gc.float(), gf.float(), atol=2e-2, rtol=2e-2)


@requires_fla
@requires_gpu
def test_kimi_delta_attention_experimental_kernels_match_fla():
    _skip_unless_experimental_kernels()
    ...
    torch.manual_seed(0)
    config = KimiDeltaAttentionConfig(
        n_heads=_EXP_HEADS, head_dim=128, expand_v=2.0, allow_neg_eigval=True
    )
    module = config.build(d_model, layer_idx=0, n_layers=12, init_device=device)
    _init_real_weights(module, d_model)
    ...
    results = {}
    for use_experimental in (False, True):
        _set_use_experimental_kernels(module, use_experimental)
        ...
        results[use_experimental] = (y.detach(), xi.grad, module.A_log.grad.clone())

    y_fla, dx_fla, dA_fla = results[False]
    y_exp, dx_exp, dA_exp = results[True]
    torch.testing.assert_close(y_exp, y_fla, atol=5e-3, rtol=5e-3)
    torch.testing.assert_close(dx_exp, dx_fla, atol=1e-2, rtol=1e-2)
    torch.testing.assert_close(dA_exp, dA_fla, atol=1e-2, rtol=1e-2)
