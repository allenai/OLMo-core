...
def _run_optimizer_parity():
    ...
    instrument: ContextManager[Any]
    if cuda:
        instrument = patch("olmo_core.ops.grad_accum.gradient_add", wraps=gradient_add)
    else:
        instrument = nullcontext()
    with instrument as fast_call:
        for step in range(3):
        ...
        if cuda:
            assert fast_call.call_count == 24
