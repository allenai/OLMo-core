...
@pytest.mark.parametrize("masked_dropout", [None, 0.0])
def test_block_accepts_disabled_inherited_masked_dropout(masked_dropout):
    config = _block_config()
    config.masked_dropout = masked_dropout
    _build_block(config)


def test_block_rejects_unsupported_masked_dropout():
    config = _block_config()
    config.masked_dropout = 0.1
    with pytest.raises(OLMoConfigurationError, match="masked_dropout.*not supported"):
        _build_block(config)
