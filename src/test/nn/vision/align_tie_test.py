...
The compiled/distributed case is covered separately and came out the same way: a 2-GPU
holmes run (experiment ``01M2622FH6WTMHZAD6A9WT5D7F``,
``launch_scripts/donovan/dev/align_tie_2gpu_check.py``) with real FSDP2 + ``torch.compile``,
the tie off, rank 0 holding image rows and rank 1 an all-text microbatch, issued ``[10,
10]`` reduce-scatters — symmetric — with no ``None`` gradients on either rank. Note also
that ``apply_compile`` targets the ``lm``/``vision``/``connector`` submodules, not
``MultimodalLM.forward``, so the splice is never inside a compiled region.
"""
...
def test_align_tie_defaults_on():
    """Default remains on as the conservative choice. It is redundant for the current
    forward (see module docstring) and verified so on hardware; turning it off is safe."""
