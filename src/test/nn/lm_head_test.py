...
def test_lm_head_logits_to_keep(head_type, loss_implementation):
    if (
        head_type == LMHeadType.normalized
        and loss_implementation == LMLossImplementation.fused_linear
    ):
        pytest.skip("Normalized head doesn't support fused linear loss")
    ...
    if loss_implementation != LMLossImplementation.fused_linear:
        ...
        assert (
            torch.allclose(output.logits, output_ref.logits) if "output_ref" in locals() else True
        )
