...
def test_rowwise_combine_get_scaled_masks_invalid_routes(monkeypatch):
    ...
    expert_out_q = torch.empty((8, 4), dtype=torch.float32)
    expert_out_scales = torch.empty((8, 1), dtype=torch.float32)
...
def test_rowwise_combine_get_scaled_weighted(monkeypatch):
    ...
    gathered_out = torch.empty((1, 2, 2), dtype=torch.float32)
    ...
    expected_gather = torch.tensor([[[2.0, 4.0], [6.0, 8.0]]], dtype=torch.float32)
