...
def test_router_scores_and_gradients_match_gating_definition(gating_function, normalization):
    torch.manual_seed(6198)
    router = _build(
        gating_function=gating_function,
        normalize_expert_weights=normalization,
    )
    x = torch.randn(2, 4, router.d_model, requires_grad=True)
    weights, indices, _, _ = router(x, False)

    logits = torch.nn.functional.linear(x, router.weight.view(router.num_experts, router.d_model))
    expected_indices = logits.topk(router.top_k, dim=-1).indices
    if gating_function == MoERouterGatingFunction.topk_softmax:
        expected_weights = logits.gather(-1, expected_indices).softmax(dim=-1)
    else:
        scores = (
            logits.sigmoid() + router.sigmoid_stability_epsilon
            if gating_function == MoERouterGatingFunction.sigmoid
            else logits.softmax(dim=-1)
        )
        expected_weights = scores.gather(-1, expected_indices)
    if normalization is not None:
        expected_weights = expected_weights / expected_weights.norm(
            p=normalization, dim=-1, keepdim=True
        )

    assert torch.equal(indices, expected_indices)
    torch.testing.assert_close(weights, expected_weights, rtol=0, atol=0)
    coefficients = torch.linspace(0.3, 1.3, weights.numel()).view_as(weights)
    actual_grads = torch.autograd.grad((weights * coefficients).sum(), (x, router.weight))
    expected_grads = torch.autograd.grad(
        (expected_weights * coefficients).sum(), (x, router.weight)
    )
