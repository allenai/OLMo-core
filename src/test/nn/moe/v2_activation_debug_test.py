...
def test_ep_no_sync_activation_debug_is_disabled_by_default(monkeypatch):
    class _DisabledDebugBlock:
        @property
        def block_idx(self):
            raise AssertionError("disabled activation debug should not read block_idx")

    block = _DisabledDebugBlock()
    x = torch.ones(8, 512, requires_grad=True)

    def unexpected_forward(*_args, **_kwargs):
        raise AssertionError("disabled activation debug should not run the forward")
    ...
    out = activation_debug.maybe_dump_ep_no_sync_saved_activations(
        ...
        loss_div_factor=None,
        forward_kwargs={},
        no_sync_forward=unexpected_forward,
    )
