...
def _olmo_quantize(
    ...
    with _temporary_env(OLMO_MXFP8_Q_BACKEND="olmo"):
        return quantize_rows_to_mxfp8(x, block_size=32, scale_mode=mode)
...
def _torchao_quantize(x: torch.Tensor, mode: str) -> tuple[torch.Tensor, torch.Tensor] | None:
    ...
    scale_mode = (
        ...
        if mode == "rceil"
        else ScaleCalculationMode.FLOOR
    )
    scales, qdata = to_mx(
        x,
        torch.float8_e4m3fn,
...
def _torchao_compiled_quantize(x: torch.Tensor, mode: str) -> tuple[torch.Tensor, torch.Tensor] | None:
    to_mx, _, ScaleCalculationMode, _ = _get_torchao_mxfp_state()
    if to_mx is None:
        return None
...
def main() -> None:
    ...
    if not args.skip_te:
        ...
        if te_result is None:
            print("te            skipped: TransformerEngine MXFP8 path unavailable for this shape/runtime")
        else:
