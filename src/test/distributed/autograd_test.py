...
def _all_to_all_equal_split():
    ...
    input_tensor = torch.full(
        (world_size, 2), float(rank), device="cuda", dtype=torch.float32, requires_grad=True
    )
    ...
    expected = torch.tensor(
        [[float(i)] * 2 for i in range(world_size)], device="cuda", dtype=torch.float32
    )
...
def _all_to_all_unequal_split():
    ...
    if rank == 0:
        ...
        input_tensor = torch.tensor(
            [0.0, 1.0, 1.0], device="cuda", dtype=torch.float32, requires_grad=True
        )
    else:
        input_split_sizes = [2, 1]  # send 2 to rank 0, 1 to rank 1
        output_split_sizes = [2, 1]  # receive 2 from rank 0, 1 from rank 1
        input_tensor = torch.tensor(
            [10.0, 10.0, 11.0], device="cuda", dtype=torch.float32, requires_grad=True
        )
    ...
    if rank == 0:
        ...
        expected = torch.tensor([1.0, 1.0, 11.0], device="cuda", dtype=torch.float32)
    assert torch.allclose(output, expected), f"Rank {rank}: expected {expected}, got {output}"
...
def test_all_to_all_equal_split(backend: str):
    run_distributed_test(_all_to_all_equal_split, backend=backend, world_size=2)


@requires_multi_gpu
@pytest.mark.parametrize("backend", BACKENDS)
def test_all_to_all_unequal_split(backend: str):
    run_distributed_test(_all_to_all_unequal_split, backend=backend, world_size=2)
