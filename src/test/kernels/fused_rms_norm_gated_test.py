...
The vendored kernel keeps fla's fp32 accumulation and full-row reduction semantics, so
``y``/``dx``/``dg`` are expected to match *bitwise*. ``dw`` also matches bitwise as long as the
backward partial-sum partitioning matches fla's; if that partitioning changes during
optimization, relax only the ``dw`` comparison (with a comment explaining why).
"""
...
def test_parity_2d(T: int, dtype: torch.dtype, activation: str):
...
@requires_gpu
@pytest.mark.parametrize("activation", ["sigmoid", "swish"])
def test_parity_4d(activation: str):
...
def test_parity_other_dims(d: int):
...
@requires_fla
@requires_gpu
def test_parity_no_affine():
    _assert_parity((2048, D), torch.bfloat16, "sigmoid", affine=False)


@requires_fla
@requires_gpu
def test_parity_noncontiguous_inputs():
    seed_all(42)
...
@requires_fla
@requires_gpu
def test_module_matches_fla_module():
    import fla.modules
