...
def test_list_remote_directory(tmp_path, s3_checkpoint_dir):
    (tmp_path / "file1.json").touch()
    (tmp_path / "dir1").mkdir()
    (tmp_path / "dir1" / "file2").touch()
    ...
    for path in tmp_path.glob("**/*"):
        ...
        upload(path, f"{s3_checkpoint_dir}/{rel_path}")

    # Should only list immediate children (files and dirs), but not files in subdirs.
    # The paths returned should be full paths.
    assert set(list_directory(s3_checkpoint_dir)) == {
        f"{s3_checkpoint_dir}/file1.json",
        f"{s3_checkpoint_dir}/dir1",
    }
