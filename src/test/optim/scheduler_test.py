...
def test_linear_scheduler():
    ...
    assert scheduler.get_lr(initial_lr, 3_000, max_steps) > scheduler.get_lr(
        initial_lr, 5_000, max_steps
    )
...
def test_inv_sqrt_scheduler():
    ...
    assert scheduler.get_lr(initial_lr, 2_000, max_steps) == 2.0 + 8.0 * math.sqrt(
        1_000.0 / 3_000.0
    )
    assert scheduler.get_lr(initial_lr, 3_000, max_steps) == scheduler.get_lr(
        initial_lr, 3_000, 10 * max_steps
    )
