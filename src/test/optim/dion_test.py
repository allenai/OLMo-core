...
from olmo_core.testing import BACKENDS, DEVICES, run_distributed_test
...
def test_hsdp_dion(backend: str, shard_degree: int, num_replicas: int):
    seed_all(0)
...
@pytest.mark.parametrize("backend", BACKENDS)
def test_tensor_parallel_dion(backend: str):
    seed_all(0)
    run_distributed_test(
        _run_tensor_parallel_dion,
        backend=backend,
        start_method="spawn",
    )
