...
from olmo_core.optim.adamw import SkipStepAdamWForeachConfig
...
def test_skip_step_adamw_foreach(device: torch.device, dtype: Optional[DType]):
    ...
    config = SkipStepAdamWForeachConfig(dtype=dtype)
    model = MyModel().train().to(device)
    optim = config.build(model)
...
def test_skipstep_adamw_equivalence(
    ...
    cfg_common = dict(
        ...
        weight_decay=wd,
        dtype=dtype,
        rolling_interval_length=2,  # keeps the skip-step test short
        sigma_factor=1,
        group_overrides=group_overrides,
    )

    optim1 = SkipStepAdamWConfig(**cfg_common).build(model1)  # type: ignore[arg-type]
    optim2 = SkipStepAdamWForeachConfig(**cfg_common).build(model2)  # type: ignore[arg-type]
        ...
        for t1, t2 in zip(model1.parameters(), model2.parameters()):
            ...
            for key in ("exp_avg", "exp_avg_sq", "step"):
                if key in st1:  # all keys should exist but be defensive
                    ...
                    assert torch.allclose(v1, v2, atol=1e-6, rtol=1e-5)
