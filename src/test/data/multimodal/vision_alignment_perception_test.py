...
def test_perception_registry_identity_binds_imported_adapter_files():
    inventory = vision_alignment_perception_implementation_inventory()
    ...
    assert set(inventory["files"]) >= {
        ...
        "nn/vision/molmo2_image_processor.py",
    }
    assert all(len(value) == 64 for value in inventory["files"].values())
    assert len(vision_alignment_perception_source_registry_sha256()) == 64
