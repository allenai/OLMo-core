...
def test_nvidia_example_is_textocr_tag_and_visual_order(tmp_path):
    tok = _PromptTok()
    ...
    assert [p for p in tok.prompts if p] == ["textocr:"]
...
def test_receipt_example_is_olmocr_tag(tmp_path):
    root = _write_receipts(tmp_path)
    ...
    assert [p for p in tok.prompts if p] == ["olmocr:"]
