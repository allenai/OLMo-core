...
def test_mixture_data_loader_recognizes_pretraining_replay_fingerprint(tmp_path):
    path = tmp_path / "tokens.npy"
    np.arange(16, dtype=np.uint16).tofile(path)
    dataset = PretrainingReplayConfig(
    ...
    assert fingerprint == {
        "type": "olmo_core.data.multimodal.pretraining_replay.PretrainingReplayDataset",
        "version": None,
...
def test_pixmo_cap_strict_source_has_config_bound_content_fingerprint(monkeypatch):
    from olmo_core.data.multimodal import dataset_compat, pixmo_cap
    from olmo_core.data.multimodal.pixmo_cap import PixMoCapDatasetConfig
    ...
    saved = asdict(baseline.config)
    saved.pop("message_weight")
    payload = {
        "adapter": "PixMoCapDataset",
        "config": saved,
        "source": {"arrow_fingerprint": "cap-arrow-v1", "num_rows": 7, "split": "train"},
        "version": "pixmo-cap-adapter-v1",
    }
    encoded = json.dumps(
    ...
    assert (
        ...
        == hashlib.sha256(pixmo_cap._CONTENT_FINGERPRINT_DOMAIN + encoded).hexdigest()
    )
    weighted = replace(baseline.config, message_weight=2.0).build(_FakeTok())
...
@pytest.mark.parametrize("mode", ["caption", "transcript_and_caption", "sft_demo"])
@pytest.mark.parametrize("message_format", ["document", "qwen3"])
