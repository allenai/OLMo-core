...
- Added CuTe DSL kernels for the gated RMS norm (`olmo_core.kernels.gnorm_cute`), vendored from [kernel-fun-2](https://github.com/allenai/kernel-fun-2), plus `olmo_core.ops.gnorm.rms_norm_gated` to dispatch between them and fla's Triton `fused_norm_gate`. `GatedDeltaNet.o_norm` is now `olmo_core.nn.attention.recurrent.GatedRMSNorm` instead of fla's `FusedRMSNormGated` — same parameter name, shape and initialization, so checkpoints are unaffected — and `GatedDeltaNetConfig.kernel_backend` selects its backend, defaulting to `"auto"`, which uses the CuTe kernels on Blackwell (`sm_100+`) with supported shapes and falls back to fla everywhere else. Measured on a B300 at `B=16, T=8192, HV=16, head_v_dim=256` in bf16: 1.10x forward and 1.26x backward over fla, both at ~95% of streaming bandwidth, rising to 1.66-1.77x on the forward at shorter sequences where fla is launch-bound. Requires the new `gnorm-cute` extra. Which backend a shape resolved to is logged once at `INFO` by `olmo_core.ops.gnorm`; a fallback under `"auto"` is logged at `WARNING` with the reason.
