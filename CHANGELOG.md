...
- Added `KimiDeltaAttentionConfig.use_cute_kernel` (**experimental**), which routes both the KDA chunk kernel and the layer's short convolutions to the CuTe/Triton kernels in the `kernel-fun` package, installed with the new `kernel-fun` extra. Both are drop-ins that fall back to FLA off their supported hardware (Blackwell for KDA, Hopper and up for the conv) and shapes, and `KERNEL_FUN_DISABLE=1` forces FLA everywhere at runtime.
