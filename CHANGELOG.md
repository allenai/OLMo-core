...
- Added `DenseMoEFeedForward` (and the `FeedForwardType.dense_moe` / `DenseMoEFeedForwardConfig`), a gated feed-forward whose hidden activations are scaled by a softmax gate over a learned router projection. Despite the name it executes densely (no expert routing) — a cheap dense approximation of an MoE feed-forward.
