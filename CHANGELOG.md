...
- Added `KimiDeltaAttentionConfig.use_cute_kernel` (**experimental**), which routes the KDA chunk kernel to the CuTe/Triton kernels in the `kernel-fun` package, installed with the new `kernel-fun` extra. When that package is installed, `CausalConv1d` also runs its fused short-conv kernels. Both are drop-ins that fall back to FLA off their supported hardware (Blackwell for KDA, Hopper and up for the conv) and shapes, and `KERNEL_FUN_DISABLE=1` forces FLA everywhere at runtime.
