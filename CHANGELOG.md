...
- Added the **EMO** MoE router (`EmoRouter`, registered as `MoERouterType.emo`) and its abstract base `MoETwoLevelRouter`. EMO implements two-level document routing: each document is assigned a per-document expert pool of size randomly sampled from `[emo_min_document_expert_pool, emo_max_document_expert_pool]` (fixed to `emo_eval_document_expert_pool` at eval time), optional always-active shared experts (`emo_num_shared_experts`), and an optional data-parallel-global load-balancing loss (`emo_global_load_balancing`) that all-reduces per-expert token counts across the DP group. Configured via new `emo_*` fields on `MoERouterConfig`. Also adds `unique experts used`, `fraction of experts used`, and `reducedp unique experts used` per-batch router metrics.
- `Transformer` now derives per-document boundaries from EOS token positions (`emo_eos_token_id`) and passes them only to EMO routers. The plumbing is a no-op for non-EMO models — document boundaries are not computed or passed when no EMO router is present.
