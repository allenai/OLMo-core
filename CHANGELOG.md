...
- Added `ProfilerAnnotationCallback`, which emits named `record_function` and/or NVTX ranges around each training phase (data loading, forward, backward, optimizer step) and around each transformer block (e.g. `fwd/block04.attn`, `bwd/block00.gdn`), so profiles of compiled models aren't just anonymous "Torch-Compiled Region" entries. Registered (disabled) for all internal experiment scripts, so it can be turned on with `--trainer.callbacks.profiler_annotations.enabled=true`.
- Added `record_shapes` and `with_flops` options to `ProfilerCallback`, so a trace can attribute GEMM time to specific operator input shapes (previously shapes were only recorded as a side effect of `export_distributed_event_summary`).
