...
- Added `make beaker-image-rma-cu128` / `make beaker-image-rma-cu13` targets that build (torch 2.10) images with reliable support for the MoE-v2 GPU comm kernels — the `nccl_rma_p2p` pipeline transport and the `symm_mem_vdev2d` rowwise-EP backend. They use a CUDA `devel` release base (so the runtime ships the `nvcc`/CUDA headers those runtime-compiled extensions need), pin NCCL to an RMA-capable version (`NCCL_RMA_VERSION`), and install NVSHMEM for `symm_mem_vdev2d`. The underlying `docker-image` build gained `BASE_IMAGE`, `NCCL_PIP_SPEC`, and `NVSHMEM_PIP_SPEC` build args.
