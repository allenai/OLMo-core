...
- Added `KimiDeltaAttentionConfig.use_cute_kernel` (**experimental**), which routes the KDA chunk kernel to the CuTe/Triton kernels vendored from the `kernel-fun` package into `olmo_core.kernel_fun`. `CausalConv1d` runs that package's fused short-conv kernels unconditionally. Both are drop-ins that fall back to FLA off their supported hardware (Blackwell for KDA, Hopper and up for the conv) and shapes, and `KERNEL_FUN_DISABLE=1` forces FLA everywhere at runtime.
