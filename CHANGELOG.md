...
- Added `KimiDeltaAttentionConfig.use_cute_kernel` (**experimental**), which routes both of the layer's hot ops — the KDA chunk kernel and its three Q/K/V causal convolutions — to the CuTe/Triton kernels vendored from the `kernel-fun` package into `olmo_core.kernel_fun`. `CausalConv1d` takes the same flag (default off, so other layers are unaffected). Both families are drop-ins that fall back to FLA off their supported hardware (Blackwell for KDA, Hopper and up for the conv) and shapes, and `KERNEL_FUN_DISABLE=1` forces FLA everywhere at runtime.
