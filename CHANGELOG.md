...
- Added `make beaker-image-rma-cu128` / `make beaker-image-rma-cu13` targets that build (torch 2.10) images with reliable NCCL RMA support for the `nccl_rma_p2p` pipeline transport: they use a CUDA `devel` release base (so the runtime ships the `nvcc`/CUDA headers the JIT-compiled extension needs) and pin NCCL to an RMA-capable version (`NCCL_RMA_VERSION`). The underlying `docker-image` build gained `BASE_IMAGE` and `NCCL_PIP_SPEC` build args.
