...
- Refreshed the `olmo3moe` HF model: `Olmo3MoeConfig` accepts the newer `rope_parameters` syntax (falling back to `rope_scaling`) and orders `layer_types` before RoPE validation; `modeling_olmo3moe` supports per-layer-type RoPE (distinct `rope_parameters` for sliding vs full attention), tolerates transformers mask-helper signature changes, and routes `Olmo3MoeExperts` through `torch grouped_mm` when available (with the eager per-expert loop as the reference fallback and an all-experts loop as the compile-safe path).
...
- Added `OLMoDDPTrainModule` (and `OLMoDDPTrainModuleConfig`), the train module for the fused MoE-v2 transformer. It builds the fused MoE distributed optimizer (`OLMoDDPOptimizerConfig`), wraps model parts in `MultiGroupDistributedDataParallel`, and supports DP/EP/TP/CP/PP parallelism.
...
- Added a JIT loader for CUDA/C++ extensions (`olmo_core.kernels.cuda_extension_utils`: `load_cuda_extension` and the memoizing `LazyCudaExtension`) with arch/ABI-tagged build directories and cross-process build-lock handling.
- Added grouped-matmul kernels in `olmo_core.kernels`: `grouped_mm` (a wrapper over `torch.nn.functional.grouped_mm` adding optional caller-provided forward/backward output buffers for communication-buffer reuse) and `scaled_grouped_mm` (an MXFP8 FP8 grouped matmul). The FP8 path is version-gated on torch ≥ 2.10 and marked as a beta feature.
...
- Added an experimental CPU activation-offloading utility for the fused MoE-v2 stack (`olmo_core.nn.moe.v2.te.get_cpu_offload_context`), which overlaps host<->device copies with compute via a windowed bulk offload/prefetch and a double-buffered reload. It is currently unwired (CPU activation offloading only helps when PCIe bandwidth is not the bottleneck).
