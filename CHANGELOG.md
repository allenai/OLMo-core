...
- `convert_checkpoint_to_hf` now respects an explicit `tokenizer_id` (`-t`/`--tokenizer`) even when a sibling `tokenizer/` directory exists next to the checkpoint (fixes [#609](https://github.com/allenai/OLMo-core/issues/609)).
